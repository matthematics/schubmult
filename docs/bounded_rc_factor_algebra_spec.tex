...
An RC graph $G$ with $k$ rows is \emph{full Grassmannian} if either $\pi(G) = \id$ or $\des(\pi(G)) = \{k\}$ (i.e.\ the associated permutation has at most one descent, occurring at position $k$).
\end{definition}
...
where:
\begin{itemize}
  \item For $1 \leq i \leq n-1$, each $E_i$ is an $i$-row full Grassmannian RC graph whose permutation lies in $\Sym_{i+1}$ (equivalently, $E_i$ is an elementary symmetric RC graph: $\pi(E_i) \in \Sym_{i+1}$ with $\des(\pi(E_i)) \subseteq \{i\}$).
  \item $G_n$ is an $n$-row full Grassmannian RC graph (so $\des(\pi(G_n)) \subseteq \{n\}$).
\end{itemize}
...
The representation of a Schubert polynomial $\Sch_w^{(n)}$ in the bounded RC factor algebra depends on whether $w \in \Sym_n$ or $w$ merely has Lehmer code of length $\leq n$.
...
\[
  \Sch_w^{(n)} = \sum_{\mathbf{a}} c_{\mathbf{a}} \, e_{a_1}^{(1)} \cdot e_{a_2}^{(2)} \cdots e_{a_{n-1}}^{(n-1)},
\]
...
When $w \notin \Sym_n$ but the Lehmer code of $w$ has length $\leq n$, the Schubert element uses the $\Lambda_n$-module representation, computed as follows.

The  $n$-variable polynomial ring $\Z[x_1, \ldots, x_n]$ is a free module over $\Lambda_n$ (the ring of symmetric polynomials in $n$ variables), with basis $\{\Sch_w : w \in \Sym_n\}$.  Concretely, for any $w$ with code length $\leq n$:

\[
  \Sch_w^{(n)} = \sum_{\substack{v \in \Sym_n \\ G \text{ Grass.}}} c_{v,G} \; \Sch_v \cdot [G],
\]
where:
\begin{itemize}
  \item $v$ ranges over elements of $\Sym_n$,
  \item $G$ ranges over $n$-row Grassmannian RC graphs,
  \item each $\Sch_v$ is expanded in the $\Sym_n$ SEM basis (Case~1 above),
  \item $[G]$ is the basis element for the single-factor key $(G)$ of size $n$,
  \item the coefficients $c_{v,G}$ come from the full SEM decomposition.
\end{itemize}
...
\begin{theorem}
For each $u,v$ with code length $\leq n$, 

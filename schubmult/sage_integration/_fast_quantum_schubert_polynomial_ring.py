	...
	def divided_difference(self, i, algorithm='sage'):
		...
		if i in ZZ:
			if algorithm == "sage":
				...
				# The operator `\delta_i` sends the FastQuantumSchubert
				# polynomial `X_\pi` (where `\pi` is a finitely supported
				# permutation of `\{1, 2, 3, \ldots\}`) to:
				# - the FastQuantumSchubert polynomial X_\sigma`, where `\sigma` is
	...
	def scalar_product(self, x):
		...
		EXAMPLES::
			...
			x0^2*x1*x2 + x0*x1^2*x2 + x0*x1*x2^2 + x0^2*x1*x3 + x0*x1^2*x3
			 ...
			 + x0*x1*x3^2 + x0*x2*x3^2 + x1*x2*x3^2
		"""
		if isinstance(x, FastQuantumSchubertPolynomial_class):
			return symmetrica.scalarproduct_schubert(self, x)
		else:
			raise TypeError("x must be a FastQuantumSchubert polynomial")

	def multiply_variable(self, i):
		"""
		Return the FastQuantumSchubert polynomial obtained by multiplying ``self``
		by the variable `x_i`.
...
class FastQuantumSchubertPolynomialRing_xbasis(CombinatorialFreeModule):
	...
	def product_on_basis(self, left, right):
		...
		return sum([self.base_ring()(v)*self(Permutation(list(k))) for k,v in sq.schubmult_db({tuple(left): 1},tuple(right),list(self.base_ring().gens())).items()])

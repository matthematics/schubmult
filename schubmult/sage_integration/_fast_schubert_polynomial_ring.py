...
#def FastSchubertPolynomialRing(R):
#	"""
#	Return the FastSchubert polynomial ring over ``R`` on the X basis.
#
#	This is the basis made of the FastSchubert polynomials.
#
#	EXAMPLES::
#
#		sage: X = FastSchubertPolynomialRing(ZZ); X
...
#	"""
#	return FastSchubertPolynomialRing_xbasis(R)
#	
	...
	def divided_difference(self, i, algorithm='sage'):
		...
		if i in ZZ:
			if algorithm == "sage":
				...
				# The operator `\delta_i` sends the FastSchubert
				# polynomial `X_\pi` (where `\pi` is a finitely supported
				# permutation of `\{1, 2, 3, \ldots\}`) to:
				# - the FastSchubert polynomial X_\sigma`, where `\sigma` is
	...
	def scalar_product(self, x):
		...
		EXAMPLES::
			...
			x0^2*x1*x2 + x0*x1^2*x2 + x0*x1*x2^2 + x0^2*x1*x3 + x0*x1^2*x3
			 ...
			 + x0*x1*x3^2 + x0*x2*x3^2 + x1*x2*x3^2
		"""
		if isinstance(x, FastSchubertPolynomial_class):
			return symmetrica.scalarproduct_schubert(self, x)
		else:
			raise TypeError("x must be a FastSchubert polynomial")

	def multiply_variable(self, i):
		"""
		Return the FastSchubert polynomial obtained by multiplying ``self``
		by the variable `x_i`.
...
class FastSchubertPolynomialRing_xbasis(CombinatorialFreeModule):
	...
	def product_on_basis(self, left, right):
		...
		return sum([v*self(Permutation(list(k))) for k,v in py.schubmult({tuple(left): 1},tuple(right)).items()])

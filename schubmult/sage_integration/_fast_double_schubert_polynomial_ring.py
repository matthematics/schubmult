...
def FastDoubleSchubertPolynomialRing(R, varname):
	"""
	...
	"""
	return FastDoubleSchubertPolynomialRing_xbasis(PolynomialRing(R,100,varname))
	...
	def divided_difference(self, i, algorithm='sage'):
		...
		if i in ZZ:
			if algorithm == "sage":
				...
				# The operator `\delta_i` sends the FastDoubleSchubert
				# polynomial `X_\pi` (where `\pi` is a finitely supported
				# permutation of `\{1, 2, 3, \ldots\}`) to:
				# - the FastDoubleSchubert polynomial X_\sigma`, where `\sigma` is
	...
	def scalar_product(self, x):
		...
		EXAMPLES::
			...
			x0^2*x1*x2 + x0*x1^2*x2 + x0*x1*x2^2 + x0^2*x1*x3 + x0*x1^2*x3
			 ...
			 + x0*x1*x3^2 + x0*x2*x3^2 + x1*x2*x3^2
		"""
		if isinstance(x, FastDoubleSchubertPolynomial_class):
			return symmetrica.scalarproduct_schubert(self, x)
		else:
			raise TypeError("x must be a FastDoubleSchubert polynomial")

	def multiply_variable(self, i):
		"""
		Return the FastDoubleSchubert polynomial obtained by multiplying ``self``
		by the variable `x_i`.

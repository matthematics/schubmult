...
def FastQuantumDoubleSchubertPolynomialRing(R, varname):
	...
	return FastQuantumDoubleSchubertPolynomialRing_xbasis(PolynomialRing(QR,100,varname))
...
class FastQuantumDoubleSchubertPolynomialRing_xbasis(CombinatorialFreeModule):
	...
	def product_on_basis(self, left, right):
		...
		le = tuple(left)
		ri = tuple(right)
		result = yz.schubmult_db({le: 1},ri, r, r,list(self.base_ring().base_ring().gens()))

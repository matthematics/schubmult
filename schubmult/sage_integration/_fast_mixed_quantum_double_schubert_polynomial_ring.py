...
def FastMixedQuantumDoubleSchubertPolynomialRing(R, varname1, varname2):
	"""
	Return the FastMixedQuantumDoubleSchubert polynomial ring over ``R`` on the X basis.

	This is the basis made of the FastMixedQuantumDoubleSchubert polynomials.

	EXAMPLES::

		sage: X = FastMixedQuantumDoubleSchubertPolynomialRing(ZZ); X
	...
	return FastMixedQuantumDoubleSchubertPolynomialRing_xbasis(PolynomialRing(QR,200,vars1+vars2))


class FastMixedQuantumDoubleSchubertPolynomial_class(CombinatorialFreeModule.Element):
	def expand(self):
		"""
		EXAMPLES::

			sage: X = FastMixedQuantumDoubleSchubertPolynomialRing(ZZ)
			sage: X([2,1,3]).expand()
			x0
	...
	def divided_difference(self, i, algorithm='sage'):
		...
		if i in ZZ:
			if algorithm == "sage":
				...
				# The operator `\delta_i` sends the FastMixedQuantumDoubleSchubert
				# polynomial `X_\pi` (where `\pi` is a finitely supported
				# permutation of `\{1, 2, 3, \ldots\}`) to:
				# - the FastMixedQuantumDoubleSchubert polynomial X_\sigma`, where `\sigma` is
	...
	def scalar_product(self, x):
		...
		EXAMPLES::
			...
			x0^2*x1*x2 + x0*x1^2*x2 + x0*x1*x2^2 + x0^2*x1*x3 + x0*x1^2*x3
			 ...
			 + x0*x1*x3^2 + x0*x2*x3^2 + x1*x2*x3^2
		"""
		if isinstance(x, FastMixedQuantumDoubleSchubertPolynomial_class):
			return symmetrica.scalarproduct_schubert(self, x)
		else:
			raise TypeError("x must be a FastMixedQuantumDoubleSchubert polynomial")

	def multiply_variable(self, i):
		"""
		Return the FastMixedQuantumDoubleSchubert polynomial obtained by multiplying ``self``
		by the variable `x_i`.
...
class FastMixedQuantumDoubleSchubertPolynomialRing_xbasis(CombinatorialFreeModule):

	Element = FastMixedQuantumDoubleSchubertPolynomial_class

	def __init__(self, R):
		"""
		EXAMPLES::

			sage: X = FastMixedQuantumDoubleSchubertPolynomialRing(QQ)
			sage: X == loads(dumps(X))
			True
		"""
		self._name = "MixedQuantumDouble Schubert polynomial ring with X basis"
	...
	def product_on_basis(self, left, right):
		...
		le = tuple(left)
		ri = tuple(right)
		result = yz.schubmult_db({le: 1},ri, y, z,list(self.base_ring().base_ring().gens()))
		result = {k:v for k,v in result.items() if v!=0}		
		return sum([self.base_ring()(v)*self(Permutation(list(k))) for k,v in result.items()])

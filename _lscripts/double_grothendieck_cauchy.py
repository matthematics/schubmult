...
from schubmult.symbolic.common_polys import grothendieck_poly, grothendieck_poly_with_ring
...
if __name__ == "__main__":
    ...
    for perm in perms:
        ...
        # neg_y = tuple([-a for a in y[:20]])
        # neg_subs = {y[i]: neg_y[i] for i in range(len(neg_y))}
        # neg_subs = {y[i]: S.Zero}
        # cauchy_groth = sum([Gx._beta**(p1.inv + p2.inv - perm.inv) * grothendieck_poly(p2, x, y, Gx._beta).subs(neg_subs) * grothendieck_poly(p1,y, z, Gx._beta).subs(neg_subs) for p1, p2 in perm_pairs]).expand()
        cauchy_groth = sum([Gx._beta**(p1.inv + p2.inv - perm.inv) * Gx(p2).as_polynomial() * Gy(~p1).as_polynomial() for p1, p2 in perm_pairs])
        dgroth = grothendieck_poly_with_ring(perm, ring, Gx._beta)

...
if __name__ == "__main__":
    ...
    neg_subs = {y[i]: -y[i] for i in range(len(y))}
    for perm in perms:
        if perm.inv == 0:
            continue
        perm_pairs = hecke_pairs(perm)
        
        cauchy_groth = sum([(-Gx._beta)**(p1.inv + p2.inv - perm.inv) * grothendieck_poly_with_ring(p2, ring, Gx._beta) * Gy(~p1).as_polynomial().subs(neg_subs) for p1, p2 in perm_pairs]).expand()
        #dgroth = grothendieck_poly_with_ring(perm, x, y, Gx._beta).expand()
        dgroth = Gx(perm).as_polynomial()

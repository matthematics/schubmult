...
def forest_union(wc1, wc2):

    
    rc1, rc2 = RCGraph(wc1._snap_min()), RCGraph(wc2._snap_min())
    dju = rc1.disjoint_union(rc2)

    #increasing, recording = WCGraph(dju).hecke_invariant

    p_labeling, q_labeling = dju.omega_invariant
...
if __name__ == "__main__":
    ...
    for comp in comps:
        ...
        # print("Hofer gonk")
        print(comp)
        wc_set = WCGraph.grove_wcs(comp)
        spandex = r.from_dict(dict.fromkeys(wc_set, 1))
        pretty_print(spandex.zero_out_last_row())
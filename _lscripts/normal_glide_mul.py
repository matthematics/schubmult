...
def _glide_it_up(comp, bw, n):
    ...
    groth_full = bw.full_groth_elem(uncode(comp), n, 1)
    # groth = bw.full_groth_elem(uncode(comp), n, 1)
    glide = 0
    for key, coeff in groth_full.items():
        wc = bw.key_to_wc_graph(key).resize(n - 1)
        if wc.dst.length_vector == tuple(comp) or (wc.perm != uncode(comp)):
            glide += coeff * bw(key)
    return glide
...
if __name__ == "__main__":
    ...
    for comp1, comp2 in itertools.product(comps, repeat=2):
        ...
        assert real_prod.almosteq(checko_prod), f"Failed for {comp1} * {comp2}: \n{real_prod=}\n{checko_prod=}"

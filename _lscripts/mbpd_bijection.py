...
This file is a standalone prototype (kept in ``_lscripts``) so it can be
validated against the shipped ``WCGraph`` machinery before being integrated
into ``schubmult`` proper.
"""
...
def _selftest():
    ...
    print("=== Rothe MBPD sanity ===")
    for perm_list in ([1, 2, 3], [2, 1, 3], [1, 3, 2], [3, 1, 2], [2, 3, 1], [3, 2, 1], [2, 4, 1, 3]):
        w = Permutation(perm_list)
        D = MBPD.rothe(w)
        errs = D.validity_errors()
        got = D.perm()
        ok = (not errs) and got == w
        print(f"w={perm_list!s:14} valid={not errs} perm={list(got)} {'OK' if ok else 'FAIL'}")
        if errs:
            for e in errs[:5]:
                print("   ", e)
        if not ok:
    ...
    print("\n=== RCP <-> WCGraph round trip + perm/weight (via all_wc_graphs) ===")
    for perm_list in ([2, 1, 3], [1, 3, 2], [3, 1, 2], [2, 3, 1], [3, 2, 1], [1, 3, 4, 2]):
        w = Permutation(perm_list)
        n = len(perm_list)
        wcs = WCGraph.all_wc_graphs(w)
        allok = True
        for wc in wcs:
            rcp = RCP.from_wcgraph(wc, n=n)
            back = rcp.to_wcgraph()
            # WCGraphs are equal up to trailing empty rows; compare canonically
            # via the (word, compatible-sequence) pair.
            roundtrip = (
                back.perm_word == wc.perm_word
                and back.compatible_sequence == wc.compatible_sequence
            )
            good = (
                rcp.is_valid()
                and rcp.perm() == w
                and tuple(rcp.weight()) == tuple(_wc_weight(wc, n))
                and roundtrip
            )
            allok = allok and good
            if not good:
                print(f"  MISMATCH w={perm_list} wc={tuple(wc)}")
                print(f"    rcp={rcp.biletters} valid={rcp.is_valid()} perm={list(rcp.perm())} wt={rcp.weight()}")
                print(f"    wc_wt={_wc_weight(wc, n)} roundtrip_eq={roundtrip}")
        print(f"w={perm_list!s:14} #wc={len(wcs):3d} {'ALL OK' if allok else 'FAIL'}")
...
def _droop_test():
    ...
    print("\n=== droop / undroop: validity, perm-preservation, involution ===")
    perms = [
        [2, 1, 3], [1, 3, 2], [3, 1, 2], [2, 3, 1], [3, 2, 1],
        [1, 3, 4, 2], [2, 4, 1, 3], [4, 1, 3, 2], [3, 4, 1, 2], [4, 3, 2, 1],
    ]
    for pl in perms:
        w = Permutation(pl)
        n = len(pl)
            ...
            for r in range(1, n):
                for b in range(1, n + 1):
                    for d in range(b + 1, n + 1):
                        if D.admits_droop(r, b, d):
                            n_droops += 1
                            try:
                                E_ = D.droop(r, b, d)
                            except ValueError as exc:
                                bad.append(("core-error:" + str(exc), r, b, d, D, D))
                                continue
                            # invariants
                            if E_.validity_errors():
                                bad.append(("droop-invalid", r, b, d, D, E_))
                                continue
                            # NOTE: raw droops do NOT preserve the permutation in
                            # general (only the specific undroops inside F-moves
                            # do). So we check validity + involution only.
                            # involution: E must admit undroop at same window and give D back
                            if not E_.admits_undroop(r, b, d) or E_.undroop(r, b, d) != D:
                                bad.append(("not-involution", r, b, d, D, E_))
                            else:
                                n_invpairs += 1
                            if E_ not in seen:
                                seen.add(E_)
                                frontier.append(E_)
        status = "OK" if not bad else f"FAIL ({len(bad)} issues)"
        print(f"w={pl!s:16} reachable={len(seen):4d} droops={n_droops:4d} inv-pairs={n_invpairs:4d} {status}")
        for kind, r, b, d, D, E_ in bad[:3]:
            print(f"   {kind} at ({r},[{b},{d}])")
            print("   D=\n" + "\n".join("     " + ln for ln in str(D).splitlines()))
            print("   E=\n" + "\n".join("     " + ln for ln in str(E_).splitlines()))


def _reachable_mbpds(w, n):
    """All MBPDs reachable from the Rothe diagram of ``w`` by droops."""
    start = MBPD.rothe(w)
    seen = {start}
    frontier = [start]
    while frontier:
...
def _phi_test():
    ...
    print("\n=== Phi: MBPD -> RCP, validity + perm + weight (droop-reachable) ===")
    perms = [
        [2, 1, 3], [1, 3, 2], [3, 1, 2], [2, 3, 1], [3, 2, 1],
        [1, 3, 4, 2], [2, 4, 1, 3], [4, 1, 3, 2], [3, 4, 1, 2], [4, 3, 2, 1],
        [1, 4, 2, 3], [2, 1, 4, 3],
    ...
    for pl in perms:
        ...
        for D in reach:
            try:
                ...
                bad.append(("phi-exc:" + str(exc), D, None))
                continue
            if not B.is_valid():
                bad.append(("rcp-invalid", D, B))
            elif B.perm() != D.perm():
                bad.append(("perm", D, B))
            elif tuple(B.weight()) != tuple(D.weight()):
                bad.append(("weight", D, B))
            phis[D] = B
        # injectivity on the reachable set
        seen_rcp = {}
        for D, B in phis.items():
            key = (B.biletters,)
            if key in seen_rcp:
                bad.append(("non-injective", D, B))
            seen_rcp[key] = D
        status = "OK" if not bad else f"FAIL ({len(bad)})"
        print(f"w={pl!s:16} #MBPD={len(reach):4d} #distinct-RCP={len(seen_rcp):4d} {status}")
        for kind, D, B in bad[:3]:
            print(f"   {kind}")
            print("   D=\n" + "\n".join("     " + ln for ln in str(D).splitlines()))
            if B is not None:
                print(f"   B={B.biletters} perm={list(B.perm())} wt={B.weight()} vs D perm={list(D.perm())} wt={D.weight()}")


def _psi_test():
    r"""Ψ = Φ^{-1}: check ``Psi∘Phi = id`` on every droop-reachable MBPD and
    ``Phi∘Psi = id`` on every RCP image, plus validity/perm/weight."""
    from schubmult.combinatorics.permutation import Permutation

    print("\n=== Psi: RCP -> MBPD, inverse of Phi (droop-reachable) ===")
    perms = [
        [2, 1, 3], [1, 3, 2], [3, 1, 2], [2, 3, 1], [3, 2, 1],
        [1, 3, 4, 2], [2, 4, 1, 3], [4, 1, 3, 2], [3, 4, 1, 2], [4, 3, 2, 1],
        [1, 4, 2, 3], [2, 1, 4, 3], [4, 2, 3, 1], [3, 1, 4, 2], [4, 3, 1, 2],
    ]
    for pl in perms:
        w = Permutation(pl)
        n = len(pl)
        ...
        for D in reach:
            try:
                ...
                bad.append(("exc:" + str(exc), D, None))
                continue
            if D2 != D:
                bad.append(("psi_phi!=id", D, D2))
                continue
            # Phi∘Psi = id on the RCP as well
            if B.psi().phi().biletters != B.biletters:
                bad.append(("phi_psi!=id", D, D2))
            elif not D2.is_valid():
                bad.append(("psi-invalid", D, D2))
            elif D2.perm() != D.perm():  # Psi preserves the associated perm
                bad.append(("perm", D, D2))
        status = "OK" if not bad else f"FAIL ({len(bad)})"
        print(f"w={pl!s:16} #MBPD={len(reach):4d} {status}")
        for kind, D, D2 in bad[:3]:
            print(f"   {kind}")
            print("   D=\n" + "\n".join("     " + ln for ln in str(D).splitlines()))
            if D2 is not None:
                print("   Psi(Phi(D))=\n" + "\n".join("     " + ln for ln in str(D2).splitlines()))


def _bijection_test():
    r"""Full bijection over all of ``RCP(n)`` (via ``all_wc_graphs`` for every
    ``w in S_n``): ``Phi∘Psi = id`` and ``Psi`` yields a valid MBPD with the
    right permutation and weight."""
    import itertools

    from schubmult.combinatorics.permutation import Permutation
    from schubmult.combinatorics.wc_graph import WCGraph

    print("\n=== Full bijection RCP(n) <-> MBPD(n) via all_wc_graphs ===")
    for n in (3, 4):
        total = 0
        bad = []
        mbpd_seen = {}
        for pl in itertools.permutations(range(1, n + 1)):
            ...
            for wc in WCGraph.all_wc_graphs(w):
                ...
                try:
                    ...
                    bad.append(("exc:" + str(exc), rcp, None))
                    continue
                if back.biletters != rcp.biletters:
                    bad.append(("phi_psi!=id", rcp, D))
                elif not D.is_valid():
                    bad.append(("invalid", rcp, D))
                elif D.perm() != w:
                    bad.append(("perm", rcp, D))
                elif tuple(D.weight()) != tuple(rcp.weight()):
                    bad.append(("weight", rcp, D))
                key = D._key()
                if key in mbpd_seen and mbpd_seen[key] != rcp.biletters:
                    bad.append(("psi-non-injective", rcp, D))
                mbpd_seen[key] = rcp.biletters
        status = "OK" if not bad else f"FAIL ({len(bad)})"
        print(f"n={n}  #RCP={total:4d}  #distinct-MBPD={len(mbpd_seen):4d}  {status}")
        for kind, rcp, D in bad[:5]:
            print(f"   {kind}  rcp={rcp.biletters}")
            if D is not None:
                print("   Psi(rcp)=\n" + "\n".join("     " + ln for ln in str(D).splitlines()))

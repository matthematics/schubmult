...
if __name__ == "__main__":
    ...
    for k in range(1, n):
        for perm in by_desc[k]:
            wcs = WCGraph.all_wc_graphs(perm, k)
            for grass_perm in grass_by_desc[k]:
                grass_poly = GrothendieckPoly(grass_perm, n - 1)
                actual_result = GrothendieckPoly(perm, n - 1) * grass_poly
                result = 0
                ...
                for grass_wc in WCGraph.all_wc_graphs(grass_perm, k):
                    for wc in wcs:
                        ...
                        if squashed.is_principal:
                            result += GrothendieckPoly(squashed.perm, n - 1)

"""MOVED-B0 QUANT!=0: partner = INTRINSIC pieriknotkn1 signature (drop adjacency req).
Partner v = w t_{pp,k} (any pp!=k) with c_{k-1}(p-1;u,v) != 0, u ->_{k-1} v,
and u NOT ->_{k-2} v.
Test: exactly ONE such partner; QUANT + partner_term == 0; remainder (rest) == 0.
Also test whether partner is always pp<k (down) and its dl type (raise/drop).
Run: conda activate schubmult_312 && python _lscripts/moved_partner2.py 5
"""
import sys
...
def run(n):
    ...
    tot=0; pf=0; cf=0; rf=0; dir_hist=Counter()
    for u in perms:
        u=Permutation(u)
        for k in range(2,n):
            ...
            for w in cands:
                ...
                for p in range(1,k+1):
                    ...
                    partners=[]; reflection_total=S.Zero
                    for pp in range(1,N+1):
                        if pp==k: continue
                        ...
                        # intrinsic partner signature
                        if c_nz(u,vv,k-1,p-1,Rkm1) and not reachable(u,vv,k-2,N):
                            partners.append((pp,typ,tval))
                    if len(partners)!=1: pf+=1; continue
                    pp,typ,tval=partners[0]
                    dir_hist[(typ, "down" if pp<k else "up")]+=1
                    if expand(quant+tval)!=0: cf+=1
                    if expand(diag+rec+reflection_total-tval)!=0: rf+=1
    print(f"moved-B0 QUANT!=0 instances = {tot}")
    print(f"intrinsic partner count != 1 : {pf}")
    print(f"QUANT + partner != 0         : {cf}")
    print(f"remainder != 0               : {rf}")
    print("partner (type, dir) histogram:")
    for kk,vv in sorted(dir_hist.items(),key=lambda x:-x[1]): print(f"    {kk}: {vv}")

...
def main(n):
    ...
    for perm in Permutation.all_permutations(n):
        ...
        for wc in WCGraph.all_wc_graphs(perm, length):
            hw_set.add(wc.to_highest_weight()[0])
        
        result = Gx(perm).expand()
        test_result = 0
        for hw_wc in hw_set:
           print("Trying highest weight WCGraph", hw_wc)
           test_result += Gx._beta**(len(hw_wc.perm_word) - perm.inv) * KeyPoly(*hw_wc.extremal_weight).expand()
        
        assert (result - test_result).expand() == 0, f"Grothendieck polynomial for {perm} does not match sum over highest weight WCGraphs, got {result}, expected {test_result}"

...
def worker(nn, shared_recording_dict, lock, task_queue):
    ...
    from schubmult import CrystalGraphTensor, FreeAlgebra, Permutation, RCGraph, RCGraphRing, RootTableau, SchubertBasis, Sx, uncode
                ...
                if rc_w.is_principal:
                        ...
                        for t_elem in coeff:
                            #for t_elem0 in coeff0:
                                # these crystals have a product?
                                def sub_vector(v1, v2):
                                    return tuple([a-b for a,b in zip(v1, v2)])
                                def add_vector(v1, v2):
                                    return tuple([a+b for a,b in zip(v1, v2)])
                                test_schub = Sx(u) * Sx(rc_w.perm)
                                #hw_u = t_elem0.to_highest_weight()[0].factors[1]
                                for w1, coeff in test_schub.items():
                                    the_rc = RCGraph.principal_rc(w1, n)
                                    low_weight = sub_vector(sub_vector(the_rc.length_vector, w0_prin.length_vector),t_elem.to_lowest_weight()[0].factors[1].length_vector)
                                    high_weight = sub_vector(sub_vector(the_rc.to_highest_weight()[0].length_vector, w0_prin.length_vector),t_elem.to_highest_weight()[0].factors[1].length_vector)
                                    for u_rc in RCGraph.all_rc_graphs(u, n):
                                        if u_rc.to_highest_weight()[0].length_vector == high_weight and u_rc.to_lowest_weight()[0].length_vector == low_weight:
                                            sm += coeff * Sx(uncode(add_vector(u_rc.length_vector, v_rc.length_vector)))#t_elem.to_lowest_weight()[0].factors[1].length_vector)))

...
\begin{frame}{A warm-up: dual key polynomials}
  ...
  \begin{itemize}
    \item The \emph{Coxeter-Knuth equivalence}  on reduced words \cite{morse2016crystal} as well as the $Q$ tableau \cite{hamaker2014relating} are preserved by $\star$. Using this, we define $R\sim A$ iff $Q(R)=Q(A)$, $\wtt(R)=\wtt(A)$, and $\mathrm{word}(R)$, $\mathrm{word}(A)$ are Coxeter-Knuth equivalent.
    \item Quotient the ring by this equivalence relation to obtain $\Key{}\RC$ with $[R]\mapsto\Key{\mathrm{wt}(R)}^*$.
  \end{itemize}
  \begin{theorem}[S, 2026+]

...
\begin{itemize}
\item Forest polynomials (Nadeau and Tewari 2024 \cite{nadeau2024forest}): $\Forest{F}$ are polynomials indexed by ``indexed forests'' $F$ (bijectively related to weak compositions) and can be defined by a certain equivalence relation on reduced words/compatible sequences
...
\item It was shown that the Schubert polynomials expand positively in the forest basis. Using the equivalence relation, we can define a ``forest weight'' $F(R)$ for each pipe dream $R$, and obtain
\end{itemize}
\begin{theorem}[S, 2026+]
Let $F$ be an indexed forest with $\mathrm{code}(F) = \mathrm{code}(w)$ for a permutation $w$. Then
$$\Forest{F}= \sum_{R\in\RC(w), F(R) = F} \mathrm{wt}(R)$$
\end{theorem}
\end{frame}

\begin{frame}{New result: forest polynomial branching formula}
  \begin{theorem}[S, 2026+]
    Write
    $$\Forest{F}(x_1,\ldots,x_{n_1+n_2}) = \sum_{G,H} d_{F}^{G,H}\,\Forest{G}(x_1,\ldots,x_{n_1})\Forest{H}(x_{n_1+1},\ldots,x_{n_1+n_2}).$$
    Then, fixing bounded pipe dreams $R_G, R_H$ with $\hr(R_G) = n_1$, $\hr(R_H) = n_2$, $F(R_G) = G = \wtt(R_G)$ and $F(R_H) = H = \wtt(R_H)$,  $d_{F,G}^{H}$ is the number of bounded pipe dreams $R$ with $\hr(R)=n_1+n_2$ and $F(R) = F$ such that $R$ has a coefficient of $1$ in the product $R_G\star R_H$.
  \end{theorem}
  \begin{corollary}
  Dual forest polynomials $\Forest{F}^*$ have nonnegative structure constants for the product in $\dcoma$.
  \end{corollary}
\end{frame}
...
\begin{frame}{Dual forest polynomial coproduct?}
  ...
  \begin{itemize}
  \item It is highly likely that a positive coproduct formula for $\Forest{F}^*$ in $\dcoma$ can be obtained (at least as compared to the much harder problem for Schubert polynomials) by analyzing the $\star$ product of pipe dreams with $F(R) = F$.
  \end{itemize}
\end{frame}

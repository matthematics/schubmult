...
if __name__ == "__main__":
    ...
    #     perm=Permutation((1, 4, 2, 5, 3, 6, 8, 7))
        ...
        for i, a in enumerate(A):
            ...
            while len(perm_stack) != 0:
                ...
                for index_list, new_perm in pull_out_var(a - len([bi for bi in used if bi < a]), perm0):#pull_out_var(a - sum([1 for bi in A[:i] if bi < a]), perm0):
                    ...
                    if build_perm.inv != up_perm.inv - new_perm.inv:
                        ...
                        # full_ref_prod = Permutation.ref_product(*list(range(max(the_index+3,len(perm0)+2), 0, -1)))
                        # shifted_new_perm = Permutation([1] + [jj+1 for jj in new_perm])
                        # lower_ref_prod = Permutation.ref_product(*list(range(1, the_index)))
                        # up_perm = full_ref_prod * shifted_new_perm# *(lower_ref_prod)

                        build_perm = full_ref_prod

                        vals = []
                        for i in range(1, the_index):
                            vals += [shifted_new_perm[i]]
                        print(f"{vals=}")
                        # raise Exception
                        ...
                        # up_perm = new_perm * (full_ref_prod)
                        build_perm = (up_perm * (~((shifted_new_perm))))
                    if build_perm.inv != up_perm.inv - shifted_new_perm.inv:
                        print(f"Pacos {build_perm=}")
                        raise Exception
                    print(f"{up_perm=}")
                    print(f"{shifted_new_perm=} {the_index=}")
                    print(f"{reduced_word(build_perm)=}")
                    ...
                    print(f"{word0=}")
                    outperm = (~build_perm)*outperm0
                    print(f"{outperm=}")
                    if outperm[0] != 1:
                        print("Bingledinget")
                        #outperm = Permutation([ap - 1 for ap in outperm])
                    else:
                        outperm = Permutation([ap - 1 for ap in outperm if ap > 1])
                    new_perm_stack.append(new_perm)
                    new_out_perm_stack.append(outperm)
                    #new_perm_stack.append(new_perm)
            ...
            out_perm_stack = new_out_perm_stack
        print(f"{perm.code=}")
        for ref, seq, new_perm, bongo, outperm in zip(ref_stack,seq_stack,perm_stack,bongo_stack,out_perm_stack):
            print(f"{new_perm=}")
            print(seq)
            print(bongo)
            print(ref)

...
#         return Permutation()
#     m_index = d.index(m)
#     jumps = [1] + [i + 1 for i in range(1, m_index + 1) if d[i] != d[i - 1]]
#     late_jumps = [i + 1 for i in range(m_index + 1, len(d)) if d[i] != d[i - 1]]
#     late_jumps.reverse()
#     partial_perm = []
#     for j, L in enumerate(late_jumps):
#         if L - 1 - j >= len(partial_perm):
#             partial_perm += [None] * (L - j - len(partial_perm))

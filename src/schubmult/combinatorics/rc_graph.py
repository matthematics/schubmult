...
class RCGraph(WCGraph, CrystalGraph):
    ...
    def left_squash(self, other_rc):
        ...
        anti_result = a_other.squash_product(a_self)
    ...
    def left_squash_decomp(self):
            ...
            while stack:
                ...
                if residue.inv == 0 and len(min_cos.descents()) <= 1:
                    ...
                    if min_cos_ret.perm.inv == 0 or min_cos_ret.perm.descents() == {n - 1}:
                        return RCGraph([()]).resize(n), AntiRCGraph.from_rc_graph(working_rc).vertical_cut(working_rc.rows - n)[1].to_rc_graph()
        ...
        base_hw, grass = decomp_hw()
        tensor = CrystalGraphTensor(base_hw, grass).reverse_raise_seq(raise_seq)
        return tuple(reversed(tensor.factors))
    ...
    def _sympyrepr(self, printer=None) -> str:
        rows = list(self)
        if printer is None:
            return f"RCGraph({rows!r})"
        return f"RCGraph({printer._print(rows)})"
    ...
    def _rebuild(self, rows=()) -> RCGraph:
        ...
        construction state only have to override this one method.
        """

...
if __name__ == "__main__":
    ...
    for perm in perms:
        ...
        ideal_generators.extend([r.from_tensor_dict(cem[rc], size=n-1) for rc in cem if not rc.perm == perm])
    
    ideal_generators = [gen for gen in ideal_generators if not gen.almosteq(r.zero)]
    for perm in perms:
        cem = RCGraph.full_CEM(perm, n)
        for rc, cem_dict in cem.items():
            elem = r.from_tensor_dict(cem_dict, size=n-1)
            if not elem.almosteq(r.zero):
                ideal.extend([(elem * gen).to_rc_graph_ring_element() for gen in ideal_generators if not (elem * gen).to_rc_graph_ring_element().almosteq(g.zero)])
                ideal.extend([(gen * elem).to_rc_graph_ring_element() for gen in ideal_generators if not (gen * elem).to_rc_graph_ring_element().almosteq(g.zero)])
    
    for ideal_elem in ideal:
        pretty_print(ideal_elem)
    # g = GrassRCGraphRing()
    # T = r @ g

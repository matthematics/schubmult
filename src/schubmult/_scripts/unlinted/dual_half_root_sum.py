...
if __name__ == "__main__":
    ...
    for perm2 in perms:
        ...
        # # print(f"piss2 = {piss2}")
        # # print(f"{}")
        the_dual = sympy.sympify(PA.from_dict(ASx(perm2, n).change_basis(WordBasis)).expand())
        #the_dual = PA.from_dict({k: (v*prod([sympy.factorial(a) for a in k])) for k, v in PA.from_expr(the_dual.subs({sympy.sympify(x[i]): sum([sympy.sympify(y[j]) for j in range(1,i + 1)]) for i in range(1,50)})).items()})
        the_dual = sympy.expand(the_dual.subs({sympy.sympify(x[i]): -(sympy.sympify(x[0]) + sum([sympy.sympify(y[j]) for j in range(1,i + 1)])) for i in range(1,50)}))

...
if __name__ == "__main__":
    ...
    for perm in perms:
        for rc in RCGraph.all_rc_graphs(perm):
            ...
            spittoon = t_elem.coproduct()
            real_cprd = spittoon.ring.zero#t_elem.ring.zero
            for ((perm1, a), rc1, (perm2, b), rc2), coeff in spittoon.items():
                if coeff == 0:
                    continue
                # if len(rc2.perm.descents()) > 1:
                #     continue
                if not any(rc0.grass == rc1 for rc0 in RCGraph.all_rc_graphs(perm1, len(rc1))):
                    continue
                if not any(rc0.grass == rc2 for rc0 in RCGraph.all_rc_graphs(perm2, len(rc2))):
                    continue
                real_cprd += coeff * (spittoon.ring((((perm1, a), rc1,(perm2, b), rc2))))
            pretty_print(real_cprd)

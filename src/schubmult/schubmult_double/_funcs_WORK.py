...
def posify_generic_partial(val, u2, v2, w2):
    ...
    if expand(val - val2) != 0:
        raise Exception(f"{val=} {val2=} {u2=} {v2=} {w2=} {u2.code=} {v2.code=} {w2.code=}")
    return val
...
def xreplace_genvars(poly, vars1, vars2):
    ...
    return sympify(poly).xreplace(subs_gen1)
...
def posify(
    ...
    if len([i for i in code(v) if i != 0]) == 1:
        ...
        for i in range(len(cv)):
            if cv[i] != 0:
                ...
                p = cv[i]
                # p = cv[i]
            # if i == len(cv) - 1 or (flag and cv[i] == 0):
            #     p = i
        # k = inv(v)
        print(f"{p=} {k=} {code(~v)=}")
        # k = p - pmk
    ...
    if is_reducible(v):
        ...
        for new_w in coeff_dict:
            ...
            boink_dict = schubmult_one(new_w, permtrim(uncode(newc)), var2, var3)
            print(f"{boink_dict=} {w=}")
            newval = boink_dict.get(
                w,
                0,
            )
            print(f"{newval=}")
        ...
        print(f"{val=} {expand(val-oldval)=} FASO")
        ...
        # print("BOOGLE")
        # val = 0
        # print(f"{expand(val-oldval)=} OAFS")
        oldvalunky = val
        val = schubmult_one(permtrim(u3), permtrim([*v]), var2, var3).get(
            permtrim(w3),
            0,
        )
        print(f"{expand(val-oldvalunky)=} OAFasf", file=sys.stderr)
        # TODO: ADDED
        ...
        print(f"{expand(val-oldval0)=} FOINKLE", file=sys.stderr)
        # TODO: I REMOVED THIS
        for i in range(varl):
            val = permy(val, i + 1,var2)
        print(f"{expand(val-oldval)=} FOINKLE", file=sys.stderr)
        return val
    elif c1[0] == c2[0]:
        #print()
        if sign_only:
            return 0
        #
        # print(f"FLEFLL? {c1=} {v=} {c2=}",file=sys.stderr)
        print("FOD")
        ...
        for arr, v3 in vp:
            ...
            print(f"BAGELS {expand(val2-old_val2)=}")
            print(f"HERE {expand(val - oldval)=}")
            val += tomul * shiftsub(val2,var2)

        print(f"HERE {expand(val - oldval)=}")
    elif not sign_only:
        import sys
        print("FAGEL", file=sys.stderr)
        if optimize:

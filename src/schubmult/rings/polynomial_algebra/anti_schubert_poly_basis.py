...
class AntiSchubertPolyBasis(PolynomialBasis):
    ...
    def coproduct(self, key):
        ...
        for len0 in range(length + 1):
            ...
            res = add_perm_dict(res, {((k2, length - len0), (k1, len0)): v for (k1, k2), v in cprd.items()})
        return res

    def printing_term(self, k):
        return GenericPrintingTerm(k, "AntiSchub")
    ...
    # #     if not isinstance(other_basis, AntiSchubertPolyBasis):
    ...
    def transition_key_key(self, key):
        ...
        print("spin")
        for rc in RCGraph.all_hw_rcs(key[0], key[1]):
            # tuptup = pad_tuple(k.length_vector, key[1])
            ext_vect = rc.extremal_weight
            # the_perm = Permutation.sorting_perm(ext_vect, reverse=True)
            # sputnik = the_perm.apply(ext_vect)
            # assert sputnik == tuple(sorted(sputnik, reverse=True))
            # apply_perm =  (~the_perm) *Permutation.w0(key[1])
            # new_key = tuple(apply_perm.apply(sputnik))
            new_key = tuple(reversed(ext_vect))
            res[new_key] = res.get(new_key, S.Zero) + S.One
        return res
    ...
    def transition_forest_key(self, key):
        raise NotImplementedError("AN ANTIFOREST IS A FOREST")
        # from schubmult.combinatorics.rc_graph import RCGraph

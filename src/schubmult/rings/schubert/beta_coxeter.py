...
# pi_i s_i = -partial^i (1 + beta x_{i})
# pi_i = -beta + \partial^i + x_i\partial^i
# pi_is_i = -beta s_i - (1+x_i)\partial^i
# = -beta (1 - s_i) - \pi_i
...
class BetaCoxeterRing(Ring, CompositeDomain):
    ...
    def __init__(self, genset, domain=None):
        ...
        self.dtype = type("BetaCoxeterElement", (BetaCoxeterElement,), {"ring": self})
    ...
    def new(self, x):
        if isinstance(x, BetaCoxeterElement):
            return x
    ...
    def domain_new(self, element, orig_domain=None):  # noqa: ARG002
        ...
        if isinstance(element, BetaCoxeterElement) or isinstance(element, BaseSchubertElement):
...
df = BetaCoxeterRing(GeneratingSet("x"))

...
from schubmult.perm_lib import NilPlactic, Permutation, Plactic, uncode
...
# key is (NilPlactic, Plactic) of the same shape, but compatible with the bottom key
# <= bottom key

class RCGraphElement(DomainElement, DefaultPrinting, dict):
    _op_priority = 1e200
    precedence = 40
    ...
    def __add__(self, other):
        if isinstance(other, RCGraphElement):
...
class RCGraphRing(Ring, CompositeDomain):
    def __str__(self):
    ...
    def __init__(self, genset, domain=None):
        ...
        self.dtype = type("RCGraphElement", (RCGraphElement,), {"ring": self})
    ...
    def new(self, x):
        if isinstance(x, RCGraphElement):
            return x
    ...
    def domain_new(self, element, orig_domain=None):  # noqa: ARG002
        ...
        if isinstance(element, RCGraphElement):
...
df = RCGraphRing(GeneratingSet("x"))

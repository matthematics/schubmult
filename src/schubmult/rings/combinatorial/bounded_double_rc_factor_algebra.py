...
from schubmult.combinatorics.rc_graph import DecoratedRCGraph, RCGraph
...
def _elem_factor_from_rc(rc):
    ...
    return to_lower
    #real_weight = tuple([len([w for j, w in enumerate(tensor)) if tensor[j][i] == 1) for i in range(n - 1)])
    #target_perm = rc.perm
...
class BoundedDoubleRCFactorPrintingTerm(PrintingTerm):
    is_commutative = False
    precedence = 50

    def __new__(cls, key):
        return BoundedDoubleRCFactorPrintingTerm.__xnew_cached__(cls, key)
    ...
    def __xnew_cached__(_class, key):
        return BoundedDoubleRCFactorPrintingTerm.__xnew__(_class, key)

    def __hash__(self):
        return hash((self._key, "BoundedDoubleRCFactorPrintingTerm"))
...
class BoundedDoubleRCFactorAlgebraElement(CrystalGraphRingElement):
    """Element of BoundedDoubleRCFactorAlgebra: finite linear combinations of Grass tensors."""
    ...
    def to_rc_graph_ring_element(self, generating_set) -> RCGraphRingElement:
        ...
        for key, coeff in self.items():
            if coeff == 0:
                continue
            rc = self.ring.key_to_rc_graph_ring_element(key, generating_set)
            result += coeff * rc
        result = r.from_dict({k: v for k, v in result.items() if expand(v) != 0})
        return result
...
class BoundedDoubleRCFactorAlgebra(CrystalGraphRing):
    ...
    _id = 0
    # zero_monom = BoundedDoubleRCFactorAlgebra._key((),0)

    @property
    def make_key(self):
        return BoundedDoubleRCFactorAlgebra._key
    ...
    class _key(CrystalGraphTensor):
        ...
        def __eq__(self, other):
            return isinstance(other, BoundedDoubleRCFactorAlgebra._key) and self.size == other.size and self.factors == other.factors

    def __init__(self, domain=None, post_normalize=False):
        super().__init__(domain=domain)
        self.post_normalize = post_normalize
        self.zero_monom = self.make_key((), 0)
        self._ID = BoundedDoubleRCFactorAlgebra._id
        BoundedDoubleRCFactorAlgebra._id += 1

        self.dtype = type("BoundedDoubleRCFactorAlgebraElement", (BoundedDoubleRCFactorAlgebraElement,), {"ring": self})
    ...
    def _schub_elem_cached(self, perm, size, partition, genset):
        ...
        for _, cem_dict in dct.items():
            for key, coeff in cem_dict.items():
                ...
                #     continue
                #print(f"DEBUG {coeff=} {key[-1].generating_set=} {new_key=} from CEM of {perm} at size {size}")
    ...
    def schub_elem(self, perm, size, genset=None, partition=None):
        from schubmult.abc import y
        from schubmult.combinatorics.permutation import Permutation

        if genset is None:
            genset = tuple([y[0] for i in range(len(y))])
        if partition is None:
            # partition = tuple((~(perm.strict_mul_dominant(size))).trimcode)
            partition = tuple((~(perm.strict_mul_dominant(size))).trimcode)

        return self._schub_elem_cached(Permutation(perm), size, partition, tuple(genset))
    ...
    def __hash__(self):
        return hash(("BoundedDoubleRCFactorAlgebra", self._ID))
    ...
    def printing_term(self, key):
        return BoundedDoubleRCFactorPrintingTerm(key)

    def key_to_rc_graph_ring_element(self, key, generating_set) -> DecoratedRCGraph:
        ...
        if len(key) == 0:
            return ring(DecoratedRCGraph([], generating_set).resize(key.size))
        ...
        if not isinstance(key[0], DecoratedRCGraph):
            raise TypeError(f"Key factors must be DecoratedRCGraph, got {type(key[0])} in key_to_rc_graph")
        acc = ring(key[0])
        for factor in key[1:]:
            ...
            new_acc = 0
            for acc_rc, acc_coeff in acc.items():
                if not isinstance(acc_rc, DecoratedRCGraph):
                    raise TypeError(f"Accumulator must be DecoratedRCGraph, got {type(acc_rc)} in key_to_rc_graph")
                new_acc += acc_coeff * ring.from_dict(DecoratedRCGraph(acc_rc, generating_set=generating_set).elem_squash(factor))
            acc = new_acc
    ...
    def _sort_only(self, key):
        factors = list(key)
        if len(factors) <= 1:
            return factors
        sorted_factors = sorted(factors, key=len)
        if tuple(sorted(sorted_factors, key=len)) != tuple(sorted_factors):
            raise ValueError(f"Sorting failed to produce length-sorted key: {sorted_factors} from {key}")
    ...
    def _normalize_key(self, key):
        ...
        while True:
            ...
            factors = self._sort_only(list(factors))
            factors = [rc.normalize() for rc in factors if rc.perm.inv != 0]
    ...
    def mul(self, a, b):
        # if not isinstance(b, BoundedDoubleRCFactorAlgebraElement):
        #     return super().mul(a, b)
        if isinstance(a, BoundedDoubleRCFactorAlgebraElement):
            if isinstance(b, BoundedDoubleRCFactorAlgebraElement):
                accum = self.zero
        ...
        #raise NotImplementedError("Left multiplication by non-algebra element is not implemented")

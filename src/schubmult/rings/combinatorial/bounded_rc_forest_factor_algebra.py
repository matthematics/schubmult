...
def _to_forest(the_squash):
    return next(iter([rc for rc in RCGraph.all_forest_rcs(the_squash.forest_weight) if rc.length_vector == the_squash.length_vector and rc.omega_invariant[1] == the_squash.omega_invariant[1]]))
...
class BoundedRCForestFactorAlgebra(CrystalGraphRing):
    ...
    _id = 0
    # zero_monom = BoundedRCForestFactorAlgebra._key((),0)

    @property
    def make_key(self):
        return BoundedRCForestFactorAlgebra._key
    ...
    class _key(CrystalGraphTensor):
        ...
        def __eq__(self, other):
            return isinstance(other, BoundedRCForestFactorAlgebra._key) and self.size == other.size and self.factors == other.factors

    def __init__(self, domain=None, post_normalize=False):
        super().__init__(domain=domain)
        self.post_normalize = post_normalize
        self.zero_monom = self.make_key((), 0)
        self._ID = BoundedRCForestFactorAlgebra._id
        BoundedRCForestFactorAlgebra._id += 1

        self.dtype = type("BoundedRCForestFactorAlgebraElement", (BoundedRCForestFactorAlgebraElement,), {"ring": self})
    ...
    def full_schub_elem(self, perm, size):
        ...
        for (comp, _), coeff in schub_elem.items():
            ...
            schub += coeff * term
        return schub#self.from_dict({key: value for key, value in schub.items() if rr.key_to_rc_graph(rr.make_key(key, key.size)).perm.pad_code(size) == rr.key_to_rc_graph(rr.make_key(key, key.size)).forest_weight})
    ...
    def __hash__(self):
        return hash(("BoundedRCForestFactorAlgebra", self._ID))
    ...
    def key_to_rc_graph(self, key) -> RCGraph:
        ...
        if acc.forest_weight != acc.perm.pad_code(len(acc)):
            return None
    ...
    def mul(self, a, b):
        # if not isinstance(b, BoundedRCForestFactorAlgebraElement):
        #     return super().mul(a, b)
        if isinstance(a, BoundedRCForestFactorAlgebraElement):
            accum = self.zero

...
from schubmult.rings.rc_graph_module import RCGraph, RCGraphModule, RCGraphTensor, TensorModule, all_fa_degree, lr_module, try_lr_module
...
def lr_elem(perm):
    if perm.inv == 0:
        return ASx.one.coproduct()
    lower_perm = uncode(perm.trimcode[1:])
    elem = ASx(lower_perm)
    ret_elem = ASx(uncode([perm.trimcode[0]]), 1).coproduct() * lr_elem(lower_perm)
    up_elem = ASx(uncode([perm.trimcode[0]]), 1)*elem
    keys = set(up_elem.keys())

    for key, coeff in up_elem.items():
        if key[0] != perm:
            ret_elem -= coeff * lr_elem(key[0])
    
    return TensorModule({RCGraphTensor(rc1, rc2): v for (rc1, rc2), v in ret_elem.items() if rc1.perm.bruhat_leq(perm) and rc2.perm.bruhat_leq(perm)})
...
def main():
    ...
    for perm in perms:
        ...
        try:
            ...
            print("CORRECT")
            print(correct_mod)
            trim_module = TensorModule({RCGraphTensor(rc1, rc2): v for (rc1, rc2), v in try_mod.items() if RCGraphTensor(rc1, rc2) not in correct_mod})
            print("TRIM")
            print(trim_module)
            other_trim_module = TensorModule({RCGraphTensor(rc1, rc2): v for (rc1, rc2), v in correct_mod.items() if RCGraphTensor(rc1, rc2) not in try_mod})
            print("TRIM2")
            print(other_trim_module)
            raise

...
The iteration in Definition \ref{definition:clptrm} is legitimate: all crossings of $S$ lie in rows $1,\ldots,p$, and by Lemma \ref{lemma:zeroweight} each application of $\zeromap$ preserves the weight vector, so every intermediate bounded RC graph again has all of its crossings in rows $1,\ldots,p$ and in particular has an empty last row, which is the hypothesis under which $\zeromap$ is defined.

Two immediate consequences of this definition will be used constantly.

\begin{lemma}[The clip is insensitive to the number of rows]\label{lemma:clipstable}
Let $R$ be a bounded RC graph, let $0\leq p\leq \hr(R)$, and let $S$ and $h$ be as in Definition \ref{definition:clptrm}. If instead we regard $S$ as a bounded RC graph with $h'$ rows for any $h'\geq h$, then
$$\zeromap^{\,h'-p}(S)=\clp^p(R).$$
\end{lemma}
\begin{proof}
It suffices to treat $h'=h+1$ and to induct. Since $\maxd(\wof S)\leq h<h'$ and all crossings of $S$ lie in rows $1,\ldots,p$, the last row of $S$ is empty and deleting it leaves a valid bounded RC graph, namely $S$ with $h'-1$ rows. Hence the first branch of Algorithm \ref{algorithm:zero} applies, and $\zeromap$ returns exactly that. The remaining $h'-1-p$ iterations then compute $\clp^p(R)$ by definition.
\end{proof}
...
\end{lemma}
\begin{proof}
Definition \ref{definition:clptrm} constructs $\clp^p(R)$ out of the set $S$ and the integer $p$ alone, making no further reference to $R$ or to $\hr(R)$; the hypothesis says precisely that $R$ and $R'$ give rise to the same $S$, so the first assertion follows. For the second, suppose $\hr(R)=p$. Then every crossing of $R$ lies in rows $1,\ldots,p$, so $S=R$ as sets, and $\maxd(\wof R)\leq p$ since $R$ is a bounded RC graph with $p$ rows. Hence $h=p$, so $S$ is regarded as having $p$ rows and no iteration is performed, giving $\clp^p(R)=R$.
\end{proof}
...
Associativity of $\sqcupplus$ rests on two structural identities relating $\clp^p$ and $\trm^p$: a commutativity of $\clp$ with $\trm$, and a functoriality of $\clp$ under iteration. The first is a formal consequence of Lemma \ref{lemma:ztrim} and Lemma \ref{lemma:clipstable}; the second carries genuine combinatorial content.
...
\end{lemma}
\begin{proof}
We induct on $h$. Suppose first that $p=h-1$. Since row $h$ of $S$ is empty, every crossing of $S$ lies in rows $1,\ldots,h-1$, so the set arising in Definition \ref{definition:clptrm} for $\clp^{h-1}(S)$ is all of $S$, and the associated integer is $\max\{h-1,\maxd(\wof S)\}$. As $S$ is a bounded RC graph with $h$ rows we have $\maxd(\wof S)\leq h$, so this integer is $h$ if $\maxd(\wof S)=h$ and is $h-1$ otherwise. In the first case $\clp^{h-1}(S)=\zeromap(S)$ directly from the definition. In the second case $\clp^{h-1}(S)$ is $S$ regarded as having $h-1$ rows, and the first branch of Algorithm \ref{algorithm:zero} shows $\zeromap(S)$ is the same. Either way $\clp^{h-1}(S)=\zeromap(S)$, and since $\zeromap(S)$ has $p=h-1$ rows, Lemma \ref{lemma:cliplocal} gives $\clp^{p}(\zeromap(S))=\zeromap(S)$. So the case $p=h-1$ holds.
...
Moreover $\wtt(X)=\wtt(Y)$: by Lemma \ref{lemma:zeroweight} every application of $\zeromap$ preserves the weight of each surviving row, so each of $\wtt(X)$ and $\wtt(Y)$ equals $(\wtt_1(S),\ldots,\wtt_p(S))$. By Theorem \ref{theorem:modactrc} a bounded RC graph with $p$ rows is determined by its trim together with its permutation, so it remains only to check that $\wof X=\wof Y$. Both are obtained from the common permutation $\wof{\trm(X)}=\wof{\trm(Y)}$ by adjoining a first row of the common size $\wtt_1(S)$, so this is the assertion that the two chains of transition steps produced by Lemma \ref{lemma:zerotransition}, namely the one carrying $\wof S$ to $\wof Y$ and the one carrying $\wof{\zeromap(S)}$ to $\wof X$, have the same endpoint.
\end{proof}
...
\end{lemma}
\begin{proof}
Collecting coefficients, the coefficient of $\dsch_w^n$ on the right-hand side is the number of WC graphs $W$ with $\wof{W}=w$, $\hr(W)=n$, and $\wtt(W)=a$. By definition, this coefficient is the coefficient of the monomial $x^a$ in $\dsch_w^n$, which is equal to $\langle \sch_w^n, [a]\rangle$.
\end{proof}
...
\begin{proof}
  ...
  Suppose $A$ is an RC graph with $k$ rows, and let $W_1 - W_2$ be a generator of $\Omega$, so that $\wof{W_1}=\wof{W_2}$, with $\hr(W_1)=\hr(W_2)=m$. Suppose $W_1'$ and $W_2'$ satisfy 
  $$\clp^{k}(W_1') = A = \clp^{k}(W_2')$$ 
  and $\trm^{k}(W_1') = W_1$, $\trm^{k}(W_2') = W_2$. Assume also that the first $k$ rows of $W_1'$ and $W_2'$ are identical. Let this set of rows be denoted $A'$. Then 
$$\wof{W_1'}=\wof{A'}\wof{W_1}=\wof{A'}\wof{W_2}=\wof{W_2'}$$ 
For each such $A'$ that is the unaltered first $k$ rows of a RC graph $W'$ with $\hr(W')=m+k$, $\clp^k(W')=A$, with empty last $m$ rows, we obtain exactly one pair of $W_1'$ and $W_2'$ satisfying the above conditions. Therefore, 
$$A\sqcupplus(W_1-W_2) = \sum_{\substack{\clp^k(W_1')=\clp^k(W_2')=A\\\trm^k(W_1')=W_1\\\trm^k(W_2')=W_2}} (W_1' - W_2') \in \Omega.$$
For the right ideal property, let $W_1$, $W_2$, and $A$ be as above. Then
$$(W_1-W_2)\sqcupplus A = \sum_{\substack{\clp^m(W_1')=W_1\\\clp^m(W_2')=W_2\\\trm^m(W_1')=\trm^m(W_2')=A}} (W_1' - W_2')$$
Assume first that $A$ is a single empty row.  We know that
$$\zeromap:\RC_{m+1}(w')\to \bigcup_{w'\downvar{m + 1} w}\RC_{m}^0(w)$$
is a bijection, so that for each $w'\in S_\infty$ there is at most one $W_1'\in \RC_{m+1}(w')$  satisfying $\clp^{m}(W_1')=W_1$ and $\trm^{m}(W_1')=A$, and this is true if and only if there is a (unique) $W_2'\in \RC_{m+1}(w')$ with $\clp^m(W_2')=W_2$ and $\trm^m(W_2')=A$. For each such pair, we have $\wof{W_1'}=\wof{W_2'}$, so $W_1'-W_2'\in\Omega$. Therefore, the sum is of differences of pairs of WC graphs, both with the same permutation, each with coefficient $1$, so that $(W_1-W_2)\sqcupplus A\in\Omega$. 

Though we have proved this only for a single empty row, the result follows if we can show it is still true for $A$ consisting of $k>1$ empty rows for any $k$, which is easily seen by associativity and an inductive argument. For arbitrary $A$ the product is identical to that of $A$ empty, except for the last $k$ rows being replaced with $\shup^{m}A$, with the stipulation that the RC graph must remain reduced, which depends only on the permutation of the first $m$ rows. Thus, $\Omega$ is a two-sided ideal of $\BRC$.
\end{proof}

The left-hand half of the argument just given isolates a structural feature of $\sqcupplus$ that we will use repeatedly in Sections \ref{section:crystals}--\ref{section:slide}, and which we now record separately. It says that in a product $A\sqcupplus W$ the second factor enters only as an inert block of rows sitting below an index set of ``top blocks'' that is determined by $A$ and by the permutation of $W$ alone.
...
\begin{enumerate}
\item $\clp^{k}(A')=A$, where $A'$ is regarded as having $\max\{k,\maxd(\wof{A'})\}$ rows, and
...
\begin{proof}
  ...
  \emph{Right ideal.} Let $R_1-R_2$ be a generator of $\Theta$ with $\hr(R_1)=\hr(R_2)=m$, and let $A$ be a bounded RC graph with $\hr(A)=k$. Suppose first that $A$ is a single empty row, so $k=1$. By Theorem \ref{theorem:zero_bijection} the terms of $R_j\sqcupplus A$ are exactly the $R_j'\in\RC_{m+1}^0$ with $\zeromap(R_j')=R_j$, and there is at most one such $R_j'$ for each permutation; as in the proof for $\Omega$, this matches the terms of $R_1\sqcupplus A$ with those of $R_2\sqcupplus A$ so that corresponding terms satisfy $\wof{R_1'}=\wof{R_2'}$. A raising operator $e_i$ moves a crossing out of row $i+1$ and into row $i$, so it can never create a crossing in the last row; consequently $\RC_{m+1}^0$ is stable under every $e_i$, and $\mathbb{Y}(R_j')\in\RC_{m+1}^0$. By Corollary \ref{corollary:crystaliso}, $\zeromap$ intertwines the raising operators on $\RC_{m+1}^0$, hence
  $$\zeromap\bigl(\mathbb{Y}(R_j')\bigr)=\mathbb{Y}\bigl(\zeromap(R_j')\bigr)=\mathbb{Y}(R_j).$$
  Since $\mathbb{Y}(R_1)=\mathbb{Y}(R_2)$ and $\zeromap$ is injective on $\RC_{m+1}^0$ by Lemma \ref{lemma:zero_injective}, we conclude $\mathbb{Y}(R_1')=\mathbb{Y}(R_2')$, so $R_1'-R_2'\in\Theta$ and $(R_1-R_2)\sqcupplus A\in\Theta$. The case of $A$ consisting of $k>1$ empty rows follows by induction, using associativity of $\sqcupplus$.

  For general $A$, Lemma \ref{lemma:blockdecomp} shows that the terms of $R_j\sqcupplus A$ are the bounded RC graphs $B_j\cup\shup^{m}A$ with $m+k$ rows, where $B_j$ satisfies condition (1) of that lemma with respect to $R_j$, and that the terms of $R_j\sqcupplus\mathrm{empty}(k)$ are the $B_j$ themselves regarded as having $m+k$ rows, subject to the same condition (1) and the instance of condition (2) with the identity permutation in place of $\wof A$. Hence the bijection $B_1\leftrightarrow B_2$ produced in the previous paragraph, which satisfies $\wof{B_1}=\wof{B_2}$, restricts to a bijection between the terms of $R_1\sqcupplus A$ and those of $R_2\sqcupplus A$. It therefore suffices to know that $\mathbb{Y}(B_1)=\mathbb{Y}(B_2)$ implies $\mathbb{Y}(B_1\cup\shup^{m}A)=\mathbb{Y}(B_2\cup\shup^{m}A)$, which is Lemma \ref{lemma:lowerblockattach} below.
\end{proof}
...
\begin{proof}
  ...
  \emph{Right ideal.} Let $R_1-R_2$ be a generator of $\Gamma$ with $\hr(R_j)=m$ and let $A$ have $\hr(A)=k$. Suppose first that $A$ is a single empty row. As in the proof for $\Omega$, Theorem \ref{theorem:zero_bijection} matches the terms of $R_1\sqcupplus A$ with those of $R_2\sqcupplus A$ so that corresponding terms $R_1',R_2'$ satisfy $\wof{R_1'}=\wof{R_2'}$ and $\zeromap(R_j')=R_j$. Since $R_j'$ has empty last row, $\mathrm{word}(R_j')$ is obtained from $\mathrm{word}(R_j)$ by the sequence of Little bumps of Proposition \ref{proposition:little_bump} run in reverse, and by Theorem \ref{theorem:littlecrystal} such bumps preserve the recording tableau. As forest equivalence is likewise a statement about the pair $(P,Q)$ of the $\Omega$-insertion, with the forest class determined by $P$, the correspondence $R_j\mapsto R_j'$ preserves forest classes, so $R_1'\equiv_\forest R_2'$ and $R_1'-R_2'\in\Gamma$. The case of $k>1$ empty rows follows by induction using associativity of $\sqcupplus$.

  For general $A$, Lemma \ref{lemma:blockdecomp} shows that the terms of $R_j\sqcupplus A$ are the bounded RC graphs $\bigl(B_j\cup\shup^{m}A,\ m+k\bigr)$ where $B_j$ satisfies condition (1) of that lemma with respect to $R_j$, subject to a condition (2) that depends only on $\wof{B_j}$ and $\wof A$; since the bijection $B_1\leftrightarrow B_2$ of the previous paragraph satisfies $\wof{B_1}=\wof{B_2}$, it restricts to a bijection between the terms of $R_1\sqcupplus A$ and those of $R_2\sqcupplus A$. Corresponding terms have words $\mathrm{word}(B_1)\cdot(\mathrm{word}(A)+m)$ and $\mathrm{word}(B_2)\cdot(\mathrm{word}(A)+m)$ with $\mathrm{word}(B_1)\equiv_\forest\mathrm{word}(B_2)$, so they are forest equivalent by \cite[Lemma~5.9]{nadeau2024forest} (suffix invariance, the mirror of the prefix invariance used above). Hence $(R_1-R_2)\sqcupplus A\in\Gamma$.
\end{proof}
...
\emph{Right ideal.} Suppose first that $A$ is a single empty row. As in the proof for $\Omega$, Theorem \ref{theorem:zero_bijection} matches the terms of $R_1\sqcupplus A$ with those of $R_2\sqcupplus A$ so that corresponding terms $R_j'$ satisfy $\zeromap(R_j')=R_j$ and $\wof{R_1'}=\wof{R_2'}$. By Proposition \ref{proposition:zerocrystal} and Proposition \ref{proposition:little_bump}, $\mathrm{word}(R_j')$ is determined by $\mathrm{word}(R_j)$ together with $\wof{R_j'}$, since the passage from $R_j'$ to $R_j=\zeromap(R_j')$ is a sequence of Little bumps whose starting position is determined by the permutation. As $\mathrm{word}(R_1)=\mathrm{word}(R_2)$ and $\wof{R_1'}=\wof{R_2'}$, we get $\mathrm{word}(R_1')=\mathrm{word}(R_2')$, so $R_1'-R_2'\in\Sigma$. The case of $k>1$ empty rows follows by induction using associativity of $\sqcupplus$.

For general $A$, Lemma \ref{lemma:blockdecomp} shows as in the proof of Lemma \ref{lemma:forestideal} that the bijection $B_1\leftrightarrow B_2$ just constructed restricts to a bijection between the terms of $R_1\sqcupplus A$ and those of $R_2\sqcupplus A$, and corresponding terms have words $\mathrm{word}(B_j)\cdot(\mathrm{word}(A)+m)$. Since $\mathrm{word}(B_1)=\mathrm{word}(B_2)$ these agree, so $(R_1-R_2)\sqcupplus A\in\Sigma$.
\end{proof}

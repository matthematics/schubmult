...
\emph{Trims agree.} By Lemma \ref{lemma:ztrim}, $\trm(\zeromap(R))=\zeromap(\trm(R))$ and $\trm(\zeromap^2(S))=\zeromap^2(\trm(S))$. Moreover $\trm(\lmap(R))=\lmap(\trm(R))$, by the argument of Lemma \ref{lemma:ztrim}: trimming preserves a suffix of the word, and the deletion of the crossing at the last descent followed by the re-insertion may equally be carried out before or after that suffix is taken.\todo{GAP: $\trm\circ\lmap=\lmap\circ\trm$ is asserted by analogy with Lemma \ref{lemma:ztrim}, not proved. Verified computationally for $N\leq 6$ (2219 instances where both sides defined, no failures). Should be stated and proved as its own lemma alongside Lemma \ref{lemma:ztrim}.} Applying the present lemma inductively to $\trm(R)$, which has height $n-1$ and empty last row, gives $\zeromap(\trm(R))=\zeromap^2(\lmap(\trm(R)))=\zeromap^2(\trm(S))$, so the two sides have the same trim.

\emph{Permutations agree.} By Theorem \ref{theorem:modactrc} a bounded RC graph of height $n-1$ is determined by its trim together with its permutation, so it remains to check that the two sides have the same permutation. Both are obtained from $\wof R$ by the transition steps of Lemma \ref{lemma:zerotransition}: the left side by a single step at index $n$, the right side by the step of $\lmap$ followed by steps at indices $n+1$ and $n$. These two chains of transition steps have the same endpoint.\todo{GAP: the equality of endpoints is asserted, not proved. \cite[Lemma~6]{little2003combinatorial}, which states that a bump replaces $w$ by $wt_{r,s}t_{j,r}$ for some $j$, controls how a bump moves the permutation and is the natural tool. This is the same residual as in Lemma \ref{lemma:clipzero}.}
\end{proof}
...
Note that Lemma \ref{lemma:cliplmap} refers to $\clp$, defined in Section \ref{section:ringproduct} below; it is placed here because its proof is a direct reading of Lemma \ref{lemma:bumpstep}, and it is not used until Section \ref{section:crystals}.
...
\end{lemma}
\begin{proof}
We may assume $\maxd(\wof R)=n$, since otherwise $\lmap(R)$ is undefined. Write $S=\lmap(R)$, so that $\hr(S)=n+1$, and recall that all crossings of $S$ lie in rows $1,\ldots,n-1$: the re-insertion is performed with parameter $n-1$ and the parked crossing at $(n,1)$ is removed.

\emph{Reduction to $r=n-1$.} Since $r\leq n-1\leq \hr(R)$ and $r\leq n-1\leq\hr(S)$, Lemma \ref{lemma:clpfunc} gives
$$\clp^{r}(R)=\clp^{r}\bigl(\clp^{n-1}(R)\bigr),\qquad \clp^{r}(S)=\clp^{r}\bigl(\clp^{n-1}(S)\bigr),$$
so it suffices to treat $r=n-1$.

\emph{Identification of the two sides.} All crossings of $R$ lie in rows $1,\ldots,n-1$, so the set $S$ of Definition \ref{definition:clptrm} for $\clp^{n-1}(R)$ is all of $R$; since $R$ is a bounded RC graph with $n$ rows, the prescribed number of rows is $\max\{n,\maxd(\wof R)\}=n$, and hence
$$\clp^{n-1}(R)=\zeromap(R).$$
Let $p$ be the length of the chain located in step 7 of Algorithm \ref{algorithm:zero} for $R$.

\emph{Case $p=1$.} Here the \textbf{for} loop of Algorithm \ref{algorithm:insertion} makes a single pass, which is precisely the insertion performed by $\lmap$, so $\zeromap(R)$ is $S$ with its trailing empty rows deleted down to height $n-1$. In particular $\maxd(\wof S)=\maxd(\wof{\zeromap(R)})\leq n-1$. Since $S$ has $n+1$ rows, all of its crossings lying in rows $1,\ldots,n-1$, Lemma \ref{lemma:clipempty} gives $\clp^{n-1}(S)=\zeromap^{2}(S)$; and as $\maxd(\wof S)\leq n-1$, each of these two applications falls into the first branch of Algorithm \ref{algorithm:zero} and merely deletes an empty row. Hence $\clp^{n-1}(S)$ is $S$ truncated to $n-1$ rows, and $\clp^{n-1}(S)=\zeromap(R)=\clp^{n-1}(R)$.

\emph{Case $p\geq 2$.} The re-insertion performed by $\lmap$ uses a reflection $t_{a,b}$ with $a\leq n-1<b$, so it does not disturb the value $\wof R(n)$, which is carried to position $n+1$ by the deletion of the crossing at root $(n,n+1)$. Hence $\maxd(\wof S)=n+1$, and since $S$ has $n+1$ rows with all crossings in rows $1,\ldots,n-1$, Lemma \ref{lemma:clipempty} gives
$$\clp^{n-1}(S)=\zeromap^{2}(S).$$
So the assertion to be proved is
\begin{equation}\label{eq:onetwo}
\zeromap(R)=\zeromap^{2}\bigl(\lmap(R)\bigr),
\end{equation}
an equality of bounded RC graphs of height $n-1$. We verify the three data determining such a graph.

\emph{Weights agree.} Each application of $\zeromap$ preserves the weight vector by Lemma \ref{lemma:zeroweight}, and $\lmap$ does as well: it deletes one crossing from row $i$ and, by Lemma \ref{lemma:insertworks}, the insertion $\ins{n-1}{(i)}$ restores exactly one crossing to row $i$ and alters no other row weight, while the parked crossing contributes only to row $n$. Hence both sides of \eqref{eq:onetwo} have weight $(\wtt_1(R),\ldots,\wtt_{n-1}(R))$.

\emph{Trims agree.} By Lemma \ref{lemma:clptrm},
$$\trm\bigl(\clp^{n-1}(R)\bigr)=\clp^{n-2}\bigl(\trm(R)\bigr),\qquad \trm\bigl(\clp^{n-1}(S)\bigr)=\clp^{n-2}\bigl(\trm(S)\bigr).$$
Moreover $\trm(\lmap(R))=\lmap(\trm(R))$, by the argument of Lemma \ref{lemma:ztrim}: trimming preserves a suffix of the word, and the deletion of the crossing at the last descent followed by the re-insertion may equally be carried out before or after that suffix is taken.\todo{GAP: $\trm\circ\lmap=\lmap\circ\trm$ is asserted by analogy with Lemma \ref{lemma:ztrim}, not proved. Verified computationally for $N\leq 6$ (2219 instances where both sides defined, no failures). Should be stated and proved as its own lemma alongside Lemma \ref{lemma:ztrim}.} Applying the present lemma inductively to $\trm(R)$, which has height $n-1$, with $n-2\leq (n-1)-1$, gives $\clp^{n-2}(\trm(R))=\clp^{n-2}(\lmap(\trm(R)))=\clp^{n-2}(\trm(S))$, so the two sides of \eqref{eq:onetwo} have the same trim.

\emph{Permutations agree.} By Theorem \ref{theorem:modactrc} a bounded RC graph of height $n-1$ is determined by its trim together with its permutation, so it remains to check that the two sides of \eqref{eq:onetwo} have the same permutation. Both are obtained from $\wof R$ by the transition steps of Lemma \ref{lemma:zerotransition}: the left side by a single step at index $n$, the right side by the step of $\lmap$ followed by steps at indices $n+1$ and $n$. These two chains of transition steps have the same endpoint.\todo{GAP: the equality of endpoints is asserted, not proved. Same residual as in Lemma \ref{lemma:clipzero}. Little's Lemma~6 \cite{little2003combinatorial}, already cited in Proposition \ref{proposition:zerocrystal}, controls how a bump moves the permutation and is the natural tool.}
\end{proof}
...
Let $R$ be a bounded RC graph with $\hr(R)=r$ and empty last row, and put $w=\wof R$. If $\maxd(w)\leq r-1$ then the first branch of Algorithm \ref{algorithm:zero} returns $R$ with its last row deleted, while the iteration defining $\lzeromap$ performs no bump at all and returns the same graph; so we may assume $\maxd(w)=r$. Let
$$s=\max\{t>r \mid w(r)>w(t)\},$$
which exists because $r$ is a descent of $w$. We induct on $s-r$.

By the exchange property of Coxeter groups there is a unique pair $(i,j)$ with $\rtt_R(i,j)=(r,r+1)$, and this pair is simultaneously the first term $(i_1,j_1)$ of the chain located in step 7 of Algorithm \ref{algorithm:zero} and the crossing appearing in the definition of $\lmap$. Both maps begin by deleting it, parking a crossing in the last row, and re-inserting into row $i$ by Algorithm \ref{algorithm:insertion} with parameter $r-1$: for $\lmap$ this is the definition, and for $\zeromap$ it is the first pass of the \textbf{for} loop of Algorithm \ref{algorithm:insertion} on the sequence $I=(i_1,\ldots,i_p)$, at which moment $L$ and $U$ are still empty, so that the search in step 5 and the rectification in steps 10--18 are exactly those performed by $\lmap$. Write $S=\lmap(R)$ for the common result of this first step. Note that all crossings of $S$ lie in rows $1,\ldots,r-1$, since the re-insertion is carried out with parameter $r-1$ and the parked crossing is removed, while $S$ is assigned $r+1$ rows.

If $s=r+1$, then $w(r)<w(t)$ for every $t>r+1$, the chain of step 7 has length $p=1$, and the \textbf{for} loop makes a single pass; both maps return $S$ with its trailing empty rows deleted down to height $r-1$, and we are done. This is the base case.

Suppose now $s>r+1$. The re-insertion performed by $\lmap$ uses a reflection $t_{a,b}$ with $a\leq r-1<b$, so it does not disturb the value $w(r)$, which is carried to position $r+1$ by the deletion of the crossing at root $(r,r+1)$. Consequently
$$\maxd(\wof S)=r+1=\hr(S),$$
while the index $s$ attached to $\wof S$ in the same way is unchanged. Thus $S$ again satisfies the hypotheses of the proposition, with inductive parameter $s-(r+1)=(s-r)-1$, and the inductive hypothesis gives
$$\lzeromap(S)=\zeromap(S)=:T,$$
a bounded RC graph with $\hr(T)=r$. Setting $w'=\wof{T}$, we can assert that $\zeromap(T) = \lzeromap(T)$ provided that $s' - (r + 1) < s - r$, where $s' = \max\{t>r \mid w'(r)>w'(t)\}$. This follows from \cite[Lemma~6]{little2003combinatorial}, which states that
$$w' = wt_{r,s}t_{j,r}$$
for some $j$, and hence $w'(r) = w(j) < w(s) = w'(s)$, so that $s' \leq s$ and $s' - (r + 1) < s - r$ as desired. We conclude that 
$$\lzeromap(R)=\lzeromap(T) = \zeromap(T) = \clp^{r-1}(S) = \clp^{r-1}(\lmap(R)) = \clp^{r-1}(R) = \zeromap(R),$$
where the first equality is by definition, the second is the inductive hypothesis, the third equality follows by definition of $\clp$, the fourth identifies $S=\lmap(R)$, and the last two equalities are Lemma \ref{lemma:cliplmap} and the definition of $\clp$. This completes the induction and proves the proposition.
\end{proof}
...
\begin{proof}
  ...
  These two words have a common prefix and suffixes obtained from $\mathrm{word}(R_1)$ and $\mathrm{word}(R_2)$ by adding the constant $k$ to every letter. Forest equivalence is generated by local swaps of adjacent letters, and these swaps are shift-invariant \cite[Proposition~5.8]{nadeau2024forest} and prefix-invariant \cite[Lemma~5.9]{nadeau2024forest}. Hence $\mathrm{word}(R_1)\equiv_\forest\mathrm{word}(R_2)$ implies that the two displayed words are forest equivalent, so each matched difference lies in $\Gamma$ and $A\sqcupplus(R_1-R_2)\in\Gamma$.
  ...
  By Lemma \ref{lemma:topmatch}(3) it is enough to treat a single application of $\zeromap$, the general case following by iteration. So suppose $\zeromap(B)=R_1$ and $\zeromap(\beta(B))=R_2$ with $\wof B=\wof{\beta(B)}$. Since $B$ has empty last row, $\mathrm{word}(B)$ is obtained from $\mathrm{word}(R_1)$ by the sequence of Little bumps of Proposition \ref{proposition:little_bump} run in reverse, and by Theorem \ref{theorem:littlecrystal} such bumps preserve the recording tableau; the same applies to $\beta(B)$ and $R_2$. As forest equivalence is a statement about the pair $(P,Q)$ of the $\Omega$-insertion, with the forest class determined by $P$, and as $R_1\equiv_\forest R_2$, we obtain $\mathrm{word}(B)\equiv_\forest\mathrm{word}(\beta(B))$. Hence $(R_1-R_2)\sqcupplus A\in\Gamma$.
\end{proof}
...
due to the rigidity of crystal graph isomorphisms.\todo{GAP: matching weights and components does not by itself pin down the isomorphism; one must anchor highest weight elements. As written this does not establish $P_n(T_1)\boxtimes P_n(T_2)=P_n(T_1T_2)$, on which associativity of $\boxtimes$ then depends.} Since $P_n$ is therefore a homomorphism of (potentially nonassociative) algebras as well as bijective, it is an isomorphism, from which it follows that $\boxtimes$ itself is associative.
\end{proof}
...
is equal to the coefficient of $x^c$ in $\sch_{uw^{-1}}(x)$.\todo{GAP: two issues. (1) Index/complementation mismatch: this proposition requires $|\beta_i|=c_i$, but the definition of $\mathrm{lift}_n$ immediately below sets $|E_i|=\lambda_{N+1-i}-c_{N+1-i}$. Tracing the Cauchy argument suggests the proposition is the one to correct; check against \texttt{schubmult}. (2) The proof does not establish the \emph{uniqueness} that $\mathrm{lift}_n$ requires, namely that the relevant coefficient is exactly $1$.}
\end{proposition}
\begin{proof}
...
(R_1^{\mathrm{left}}*R_2^{\mathrm{left}})*R_3*(R_1^{\mathrm{right}}*R_2^{\mathrm{right}}),
$$
so they are equal. The key point is that the corresponding lift factors commute, so this regrouping is canonical.\todo{GAP: the displayed expression $(R_1^{\mathrm{left}}*R_2^{\mathrm{left}})*R_3*(R_1^{\mathrm{right}}*R_2^{\mathrm{right}})$ is written without brackets, i.e. it already presumes the associativity being proved. Note also that Step~1 of Proposition \ref{proposition:quasiprod} gives a \emph{different} justification for the same fact; the two should be reconciled and only one kept.}
...
\end{theorem}
\begin{proof}
By construction $\ddot{e}_i$ and $\ddot{f}_i$ preserve $\mathrm{word}(R)$, so all elements of $\mathcal{C}$ share a common word $\mathbf{r}=\mathrm{word}(R_0)$. Conversely, an arbitrary $R\in\RC_n(w)$ with $\mathrm{word}(R)=\mathbf{r}$ is reached from $R_0$ by a sequence of crystal operators $e_i,f_i$ that fix the word: each such step changes $\wtt$ by moving a single crossing between rows $i$ and $i{+}1$ without altering inversions, hence is realized by the corresponding $\ddot{e}_i$ or $\ddot{f}_i$. Thus $\mathcal{C}=\{R\in\RC_n(w):\mathrm{word}(R)=\mathbf{r}\}$.\todo{GAP: the inclusion $\supseteq$ assumes that any two RC graphs with the same word are joined by a sequence of \emph{word-preserving} single crystal operators, which is what is being proved.}
...
Write $\mathrm{lift}_n(B) = E_1\otimes\cdots\otimes E_k$ in weakly increasing-height order (Proposition~\ref{proposition:elemfactor}). The lift product $*$ on $\BRC$ is associative, since it is the projection under $\RC_n$ of the associative multiplication on $\mathcal{M}_n$ and Proposition~\ref{proposition:elemfactor} guarantees that $\RC_n$ respects this multiplication on lift factorizations.\todo{GAP: this reads as ``$\RC_n$ is multiplicative'', which sits badly with the Remark stating that $\ker\RC_n$ is not an ideal. Note $\mathrm{lift}_n(R_1)\mathrm{lift}_n(R_2)$ is generally not in canonical weakly-increasing-height order, so $\mathrm{lift}_n(R_1*R_2)\neq\mathrm{lift}_n(R_1)\mathrm{lift}_n(R_2)$ --- which is exactly what makes associativity non-trivial. Duplicates the separate argument given for the associativity theorem above.} Likewise the quasi-tensor product $\ddot\otimes$ is associative on seminormal quasi-crystals by \cite[Theorem~5.1]{cain2023quasicrystals}. Hence
...
\]
so the multiset of slide classes appearing in $\mathsf{Sh}(W_A,W_B)$ is determined by the slide-polynomial product $\mathfrak{F}(W_A)\,\mathfrak{F}(W_B)$.\todo{GAP: distinct slide classes can carry the same slide polynomial, since $\mathfrak{F}_a$ depends only on the quasi-Yamanouchi weight. A character identity therefore recovers only the multiset of $\mathfrak{F}_a$, not the multiset of classes. Same conflation in Lemma \ref{lemma:quasitensorswap}.}
...
\]
slide-equivalent to $\overline{\mathrm{word}(R)}$, where $\Phi_A,\Phi_B$ denote the forest classes of $\overline{\mathrm{word}(A)},\overline{\mathrm{word}(B)}$. Slide equivalence is finer than forest equivalence (slide-equivalent words have identical slide polynomial and hence belong to the same forest class),\todo{GAP: this parenthetical is a non sequitur. Equal slide polynomial does not imply same forest class. The conclusion may still hold, but needs a different argument.} so $X_0\equiv_\forest\overline{\mathrm{word}(R)}\equiv_\forest\overline{\mathrm{word}(R')}$. The shuffle-stability statement of Nadeau--Tewari applied to $X_0\in\mathsf{Sh}(\Phi_A,\Phi_B)$ then gives
\[
...
where $m_{[R]}$ is the (constant) number of pairs $(A',B')$ with $A'\equiv_\forest A$, $B'\equiv_\forest B$, and $A'*B'$ landing in the class $[R]$ at any specified representative; this multiplicity is independent of the choice of representative by the quasi-crystal isomorphism of Proposition \ref{proposition:quasiprod}, which in particular ensures that isomorphic classes have equal cardinality.\todo{GAP, and the most load-bearing one in the paper: constancy of $m_{[R]}$ across a forest class. Proposition \ref{proposition:quasiprod} controls \emph{slide} classes, but a forest class is a union of slide classes, and Lemma \ref{lemma:foreststable} gives only set-stability, not equality of fibre cardinalities across the slide classes inside a forest class. Alternative route worth trying: extract $\beta_{ab}^c$ from the coefficient of $x^c$ using triangularity of $\{\forest_c\}$, avoiding the constancy claim entirely.}

...
Being the inverse of the surjective homomorphism $\iota$, $\alpha$ is an isomorphism of rings.
...
Assume first that $A$ is a single empty row. Then we know that for each $w'\in S_\infty$ there is at most one $W_1'$ with $\wof{W_1'}=w'$, $\clp^{m}(W_1')=W_1$, and $\trm^{m}(W_1')=A$, and similarly there is at most one $W_2'$ with $\wof{W_2'}=w'$, $\clp^m(W_2')=W_2$, and $\trm^m(W_2)=A$. We know that
$$\zeromap:\RC_{m+1}(w')\to \bigcup_{w'\downvar{m + 1} w}\RC_{m}^0(w)$$
is a bijection, so for each $w'$ there is exactly one such $W_1'$ and exactly one such $W_2'$ if and only if either $W_1'$ or $W_2'$ exists. In either case, we have $\wof{W_1'}=\wof{W_2'}$, so $W_1'-W_2'\in\Omega$. Therefore, the sum is of differences of pairs of WC graphs, both with the same permutation, each with coefficient $1$, so that $(W_1-W_2)\sqcupplus A\in\Omega$. 

Though we have proved this only for a single empty row, the result follows if we can show it is still true for $A$ consisting of $k>1$ empty rows for any $k$, which is easily seen by associativity and an inductive argument. For arbitrary $A$ the product is identical to that of $A$ empty, except for the last $k$ rows being replaced with $\shup^{m}A$. Thus, $\Omega$ is a two-sided ideal of $\BRC$.
\end{proof}
...
\end{theorem}
\begin{proof}
  For RC graphs $R_1,R_2$, we have that $\alpha(R_1)=\alpha(R_2)$ if and only if $\wof{R_1}=\wof{R_2}$ and $\hr(R_1)=\hr(R_2)$. Therefore the kernel of $\alpha$ is precisely $\Omega$. We thus have an exact sequence

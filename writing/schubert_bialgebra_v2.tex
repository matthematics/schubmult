...
\end{lemma}
\begin{proof}
Since $B$ has empty last row, $\mathrm{word}(B)$ is obtained from $\mathrm{word}(\zeromap(B))$ by the sequence of Little bumps of Proposition \ref{proposition:little_bump} run in reverse, and by Proposition \ref{proposition:zerocrystal} the starting position of each bump is determined by the permutation; the same applies to $B'$ and $\zeromap(B')$, with the same sequence of positions because $\wof{B}=\wof{B'}$. By Theorem \ref{theorem:littlecrystal} a Little bump preserves the Edelman--Greene recording tableau. As forest equivalence is a statement about the pair $(P,Q)$ of the $\Omega$-insertion, with the forest class determined by $P$, and as $\zeromap(B)\equiv_\forest\zeromap(B')$, we conclude $\mathrm{word}(B)\equiv_\forest\mathrm{word}(B')$, that is $B\equiv_\forest B'$.
\end{proof}

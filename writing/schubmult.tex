...
This is Lemma~\ref{lemma:outputsupport}; the proof is a Cauchy-formula factorization of $\sch_{w_1}$ into two copies of $\sch_{w_0}$, followed by linear independence of the Schubert basis. The point is that the doubled staircase $w_1$, and not $S_n$ or $S_{2n-1}$, is what confines the computation.
...
\begin{lemma}[Output support bound of $(2n-1)!!$] \label{lemma:outputsupport}
...
a dominant element of $S_{2n-1}$ with $\ell(w_1)=2\binom{n}{2}$. For any $u,v\in S_n$, every permutation $w$ occurring in the product $\sch_u\cdot\sch_v$, or in any intermediate product formed during its computation, satisfies $w\leq_L w_1$. The number of such permutations is at most $(2n - 1)!!$.
\end{lemma}
\begin{proof}
  Consider that
...
\begin{lemma}[Exact size of a dominant weak order ideal] \label{lemma:dominantinterval}
Let $\sigma\in S_N$ be dominant and write $\lambda=\code(\sigma)$. Then
$$\bigl|[e,\sigma]_R\bigr|=\frac{N!}{\prod_{i=1}^{N}\bigl(\sigma(i)-\lambda_i\bigr)},$$
and consequently, for dominant $\mu$,
$$\bigl|[e,\mu]_L\bigr|=\bigl|[e,\mu^{-1}]_R\bigr|=\frac{N!}{\prod_{i=1}^{N}\bigl(\mu^{-1}(i)-\code_i(\mu^{-1})\bigr)},$$
the inverse of a dominant permutation being again dominant. Both expressions are unchanged by padding $\sigma$ with fixed points, so $N$ may be taken to be any value at least the support size.
\end{lemma}
\begin{proof}
...
with $M=M(u,v)$ as in Lemma~\ref{lemma:instancesupport}, so that every intermediate and output $u$-side permutation is counted by $P_u$, and
$$P_u\ \leq\ \bigl|[e,M]_L\bigr|\ \leq\ (2n-1)!!.$$
...
$$V\ \leq\ \bigl|[e,v\mu_v^{-1}]\bigr|\ \leq\ n!,$$
...
Write $L:=\min(n-1,\ell_v)$. The number of coefficient-ring operations performed by $\textbf{\textsc{schubmult}}(u,v)$ is
$$O\!\left(\,n^2\,\Phi_v\;+\;L\cdot P_u\cdot B\cdot n^3\;+\;L\cdot P_u\cdot V\cdot B\cdot\Delta\cdot E_n\,\right),$$
where $P_u$, $V$, $B$, $\Delta$, $E_n$ are as in Definition~\ref{def:complexityquantities} and $\Phi_v$ is the $v$-side precomputation cost of Proposition~\ref{prop:kdowncomplexity}. Writing that bound as $2^{\,f(n)+O(\log n)}$,
$$f(n)=2n\log_2 n+(3-2\log_2 e)\,n+\Theta(\log^2 n),$$
...
$$O\!\left(\,n^2\,\Phi_v\;+\;L\cdot P_u\cdot B\cdot n^3\;+\;L\cdot P_u\cdot V\cdot B\cdot\Delta\,\right)$$
...
\begin{proof}
  ...
  For the converse, suppose for some $v$ that $c_{u,v'}^w(y;z)\neq 0$ for all $v'\geq_L v$. It follows that not only do we have that $c_{u,v}^w(0;z)\neq 0$, but also that $c_{u,v}^w(0;z)$ is homogeneous of degree $\ell(u) + \ell(v) - \ell(w)$, which we can see from the fact that this is true for $v=w_0(n)$ and the coefficient is obtained from successive applications of divided differences on the $z$ variables, which maintain homogeneity. In particular, since some Schubert polynomial $\sch_{\widetilde{v}}(-z)$ with $\ell(u)+\ell(\widetilde{v})-\ell(w)=\ell(u)+\ell(v)-\ell(w)$ has a nonzero coefficient, there exists an $i$ such that 
  $$\partial^i_{(z)}(c_{u,v}^w(0;z))=-c_{u,s_iv}^w(0;z)\neq 0$$ whence $c_{u,s_iv}^w(y;z)\neq 0$ and $c_{u,s_iv}^w(0;z)$ is homogeneous of degree $\ell(u) + \ell(s_iv) - \ell(w) = \ell(u)+\ell(v)-\ell(w) - 1$. Continuing inductively, it follows that there exists some $v''\leq_L v$ with $\ell(u)+\ell(v'')=\ell(w)$ such that $c_{u,v''}^w(y;z)=c_{u,v''}^w(0;0)\neq 0$. This proves the direction $\Rightarrow$, and we are done.
\end{proof}

\begin{remark}[Reduction of the double vanishing problem] \label{remark:doublevanishing}
The length condition in Theorem~\ref{prop:doublesupport} determines the rank of $v''$: only the elements of $[e,v]_L$ of length exactly $\ell(w)-\ell(u)$ can witness $w$. In particular the double support meets every degree $d$ with $\ell_u\leq d\leq \ell_u+\ell_v$, since $[e,v]_L$ is graded by length and a product of ordinary Schubert polynomials never vanishes.

The two directions of Theorem~\ref{prop:doublesupport} behave quite differently. The union is over the left weak order ideal $[e,v]_L$, so no cancellation between its members can occur: the ordinary structure constants are nonnegative integers, and a permutation lying in the support of $\sch_u\sch_{v''}$ for even one $v''\leq_L v$ already forces $c_{uv}^{w}(y;z)\neq 0$. The content of the theorem is the converse, that the polynomial $c_{uv}^{w}(y;z)$ cannot be nonzero for any other reason. Consequently, deciding whether a double structure constant vanishes is a disjunction of instances of the ordinary vanishing problem, the problem placed in $\Sigma_2^p$ by \cite{pak2024vanishing}; no separate theory of vanishing for the double constants is needed. The length condition bounds that disjunction by the number of elements of $[e,v]_L$ of length $\ell(w)-\ell(u)$, a single rank of the ideal rather than all $|[e,v]_L|\leq n!$ of it.

The degree bookkeeping is what makes the union spread. Assigning degree $1$ to each of $x$, $y$, and $z$, the polynomial $\sch_u(x;y)$ is homogeneous of degree $\ell(u)$, so the coefficient $c_{uv}^{w}(y;z)$ is homogeneous of degree $\ell(u)+\ell(v)-\ell(w)$ in $y$ and $z$. A permutation $w$ with $\ell(w)<\ell(u)+\ell(v)$ is thus admitted not by any failure of homogeneity but by its coefficient carrying positive degree in the coefficient variables---which is exactly what the ordinary constants, being scalars, cannot do. This is why the ordinary support is confined to the single degree $\ell(u)+\ell(v)$ while the double support is not: each $v''\leq_L v$ contributes in degree $\ell_u+\ell(v'')$ alone, and the decomposition of the double support by degree is precisely the decomposition of $[e,v]_L$ by rank, \end{remark}

\begin{proposition}[Output lower bound, uniform in $u$] \label{prop:outputlower}
For all $u,v\in S_n$,
$$\bigl|\operatorname{supp}\bigl(\sch_u(x;y)\,\sch_v(x;z)\bigr)\bigr|\ \geq\ \bigl|[e,v]_L\bigr|,$$
...
Proposition~\ref{prop:outputlower} therefore counts no degree individually, and in particular never counts the degree-$0$ terms, which are what Remmel and Whitney were facing, the Grassmannian case being Littlewood--Richardson. The entire bound is carried by the terms of positive degree, and the case $u=e$ shows how completely. There the ordinary product $\sch_e\sch_v=\sch_v$ is a single term, and it accounts for everything in degree $0$; yet the double product $\sch_e(x;y)\sch_v(x;z)$ has exactly $\bigl|[e,v]_L\bigr|$ terms, so all but one of them lie in positive degree. No estimate of the degree-$0$ count was needed to see this, and none could have helped: a bound extracted from degree $0$ alone would have returned $1$.

This is why no such bound is available for the ordinary product. Its support lies entirely in degree $\ell(u)+\ell(v)$, so counting it means counting exactly the coefficients that resist counting; there is no second family to appeal to. This is also why the question Remmel and Whitney raised is not settled here: their $|C(\lambda*\mu)|$ counts tableaux for a product of two Schur functions, with multiplicity and in a single degree, and the mechanism just described is unavailable to it. The double product supplies one. It contains the whole independent family $\{\sch_u\sch_{v''}:v''\leq_L v\}$ within a single support, spread across the ranks of $[e,v]_L$ by Theorem~\ref{prop:doublesupport}, and it is the size of that family---a purely order-theoretic quantity, available for free---that the argument converts into a term count. The coefficient variables are not an inconvenience of the double case but the source of the count.
\end{remark}

\begin{corollary}[Worst-case output-polynomiality, for each fixed $u$] \label{cor:outputpoly}
...
where $T(u,v)$ is the bound of Theorem~\ref{thm:complexity} and $N(u,v)$ is the number of terms of $\sch_u(x;y)\,\sch_v(x;z)$ in the Schubert basis. Then for all sufficiently large $n$ and every $u\in S_n$,

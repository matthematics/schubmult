...
\end{theorem}
\begin{proof}
Note that, by the Cauchy formula,
$$c_{u,v}^w(y;z) = \sum_{v'v''=v}c_{u,v''}^w(y;0)\sch_{v'}(-z).$$
Set $y=0$. Then the result is a linear combination of ordinary Schubert polynomials
$$\sum_{v'v''=v}c_{u,v''}^w(0;0)\sch_{v'}(-z) = \sum_{v'v''=v}c_{u,v''}^w\sch_{v'}(-z).$$
By linear independence, the direction $\Leftarrow$ is immediate.

We prove the converse for $v$ dominant. By the positive formula for $c_{u,v}^w(y;z)$ in \cite[Theorem~8.1]{samuelmolev}, we obtain the fact that $c_{u,v}^w(0;z)\neq 0$ if $c_{u,v}^w(y;z)\neq 0$. Then, by the Cauchy formula, we have
$$c_{u,v}^w(0;z) = \sum_{v'v''=v}c_{u,v''}^w(0;0)\sch_{(v')^{-1}}(-z).$$
For this to be nonzero, there must exist some $v''\leq v$ with $c_{u,v''}^w(0;0)\neq 0$, as desired.

For general $v$, if $c_{u,v}^w(y;z)\neq 0$ then we also have $c_{u,\mu}^w(y;z)\neq 0$ for $\mu=\dom(v^{-1})^{-1}$, whence there is a $v''\leq_L \mu$ with $c_{u,v''}^w(0;0)\neq 0$. By \cite[Theorem~1.1]{reading2007sortable}, it follows that 
$$v''\leq_L \dom((v'')^{-1})^{-1}\leq_L v\leq_L \mu$$ 
as well, so the result follows.
\end{proof}

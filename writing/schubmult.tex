...
$$O\!\left(n^4\Lambda(\dom(v^{-1})^{-1})N_{\ell(v)}(u,\dom(v^{-1})^{-1})E_n\right)$$
...
% the two counts and branchings contribute $\frac{n^4}4\Lambda(dom(v^{-1})^{-1})N(u,\dom(v^{-1})^{-1})$; the elementary-symmetric construction adds a factor of $E_n$.
...
We account for the work in two parts. First, the entire $v$-side is precomputed once by $\textbf{\textsc{elemrepr}}(v)$ (Method~\ref{method:elemrepr}) at cost $O(n^2\Lambda(\dom(v^{-1})^{-1}))$ ring operations (Proposition~\ref{prop:kdowncomplexity}); thereafter each $v$-side transition is a table lookup carrying no combinatorial work.



% Second, the live $\textbf{\textsc{pieri}}$ enumeration. Because every $u$-side cover strictly raises length, the $u$-side transitions form a graded acyclic graph, so within a fixed layer no permutation recurs; hence $\textbf{\textsc{pieri}}(w,\theta_i,\theta_i)$ is invoked at most once per distinct $u$-side permutation present in that layer (Method~\ref{method:schubmult}, step~1), which all sum to $N(u,\dom(v^{-1})^{-1})$. The number of such invocations is
...
times, where $\beta_i(w):=|\textbf{\textsc{pieri}}(w,\theta_i,\theta_i)|$ and $\delta_i(\lambda)$ is the number of kdown triples carried by $\lambda$ in column $i$. Bounding $L=O(n)$, $\beta_i(w)=O(n^3)$, $\delta_i=O(\Lambda(\dom(v^{-1})^{-1}))$, and $|\Phi_i|\leq N(u,\dom(v^{-1})^{-1})$ gives 
$$W = O(n^4\Lambda(\dom(v^{-1})^{-1})N(u,\dom(v^{-1})^{-1}))$$
...
$$O(W\cdot E_n)=O(n^4\Lambda(\dom(v^{-1})^{-1})N(u,\dom(v^{-1})^{-1})E_n)$$
...
$$O\!\left(n^4\Lambda(\dom(v^{-1})^{-1})N_{\ell(v)}(u,\dom(v^{-1})^{-1})\right)$$
% where $P_u$, $V$, $B$, $\Delta$, $E_n$ are as in Definition~\ref{def:complexityquantities} and $\Phi_v$ is the $v$-side precomputation cost of Proposition~\ref{prop:kdowncomplexity}. 
% Writing that bound as $2^{\,f(n)+O(\log n)}$,
% $$f(n)=2n\log_2 n+(3-2\log_2 e)\,n+\Theta(\log^2 n),$$
% and, when $v$ is dominant, $f(n)=n\log_2 n+(2-\log_2 e)\,n+\Theta(\log^2 n)$, where $\log_2 e=1/\ln 2\approx 1.4427$.
When $v$ is dominant, this simplifies to
$$O\!\left(n^4N(u,v)\,\right),$$
\end{corollary}
\begin{proof}
...
\begin{lemma}
  Suppose $v\in S_n$ has at most $k$ right descents. Then
$$\Lambda(\dom(v^{-1})^{-1})\leq n^k N(u,v).$$
\end{lemma}

\begin{corollary}[Output-polynomiality, $312$-avoiding case]
Let $u,v\in S_n$ and suppose $v$ is $312$-avoiding with at most $k$ descents. Then there is a polynomial $P_k(n,N)$ such that the runtime of schubmult is
$$O(E_nP_k(n,N(u,v)))$$
and in the ordinary case this simplifies to
$$O(P_k(n,N(u,v)))$$
\end{corollary}
\begin{proof}
For a $312$-avoiding permutation $v$, $N_{\ell(v)}(u,\dom(v^{-1})^{-1})=N(u,v)$. Using the previous lemma, take $P_k(n,N) = n^{k+4}N^2$, since $n^kN(u,v)\geq \Lambda(\dom(v^{-1})^{-1})$.
\end{proof}

\begin{lemma}
For any $u,v\in S_n$, we have that
$$N_{\ell(v)}(u,\dom(v^{-1})^{-1})\leq \sum_{\substack{\ell(v')=\ell(v)\\v'\leq_L \dom(v^{-1})^{-1}}}N(u,v')$$
\end{lemma}


\begin{corollary}
For each $u,v\in S_n$ with $v$ having at most $k$ right descents, we have
$$N(u,\dom(v^{-1})^{-1})\leq n^kN(u,v).$$
\end{corollary}

\begin{corollary}[Output-polynomiality] \label{cor:outputpoly}
  Suppose $v\in S_n$ has at most $k$ right descents. Then we have that
  $$T(u,v)=O(n^{4k + 3}N(u,v)^4E_n)$$
  and
  $$T_0(u,v)=O(n^{4k + 3}N_0(u,v)^4)$$

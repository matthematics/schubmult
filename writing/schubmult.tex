...
$$P_u:=\#\{\,w\mid u\leq w\leq w_1 \text{ and } \ell(w)\leq \ell_u+\ell_v\,\},$$
with $w_1=w_1(n)$ as in Lemma~\ref{lemma:outputsupport}. By that lemma every intermediate and output $u$-side permutation is counted by $P_u$, and the number of permutations occurring, intermediates included, is at most $(2n-1)!!$; we write $P_u\leq(2n-1)!!$. Let $V$ be the number of distinct $v$-side labels produced by $\textbf{\textsc{elemrepr}}(v)$ across all columns; these are permutations, and, as in Lemma~\ref{lemma:outputsupport}, we take as given that at most $n!$ of them occur, so $V\leq n!$.
...
where $P_u$, $V$, $B$, $\Delta$, $E_n$ are as in Definition~\ref{def:complexityquantities} and $\Phi_v$ is the $v$-side precomputation cost of Proposition~\ref{prop:kdowncomplexity}. The three summands are, respectively, the one-time $v$-side precomputation, the live $\textbf{\textsc{pieri}}$ enumeration (run once per distinct $u$-side permutation per layer), and the matched-pair ring work; the third dominates in the worst case. Writing that bound as $2^{\,f(n)+O(\log n)}$,
...
\end{theorem}
\begin{remark}
Here $B$ and $\Delta$ are the $u$-side (Pieri) and $v$-side (kdown-triple) per-layer branchings and $E_n$ is the per-iteration elementary-symmetric construction cost. Each term of $f(n)$ is a multiplicative factor of the running time, so all are retained. The dominant contribution is the product of the two support counts: the at most
...
integer operations, with $L=\min(n-1,\ell_v)$. Writing the bound as $2^{\,g(n)+O(\log n)}$, the worst case is
$$g(n)=2n\log_2 n+(3-2\log_2 e)\,n,$$
and, when $v$ is dominant, $g(n)=n\log_2 n+(2-\log_2 e)\,n$. These are the bounds of Theorem~\ref{thm:complexity} with the term $\Theta(\log^2 n)$ deleted.
\end{corollary}
\begin{proof}
...
with $H_2$ the binary entropy function; since $0\leq H_2\leq 1$ this recovers $\log_2 G_k\leq 2n-k$. Symmetric bounds on the binomial, $\binom{2n-k}{k}\leq(2(n-k)+1)^{k}$ and $\binom{2n-k}{k}\leq(k+1)^{2(n-k)}$, give the sharper reading
$$\log_2 G_k=O\!\big(\min(k,\,n-k)\,\log n\big),$$
which vanishes to $O(\log n)$ as $k\to 0$ or $k\to n$ and peaks at $\Theta(n)$ near $k=n/2$: the running time is governed by the two box dimensions $k$ and $n-k$, never by $n$ alone. Finally, every intermediate shape lies in the same $k\times 2(n-k)$ box, so a single Pieri step produces at most $G_k$ distinct covers; hence the per-step branchings obey $B,\Delta\leq G_k$, and the number of layers is at most $n-k$ (the code of $v^{-1}$ has at most $n-k$ nonzero parts).
\end{proof}
...
with $E_n$ as in Definition~\ref{def:complexityquantities}. Writing this bound as $2^{f_{\mathrm{Gr}}(n,k)+O(\log n)}$, the worst case is the explicit function of $n$ and $k$
$$f_{\mathrm{Gr}}(n,k)=4(2n-k)\,H_2\!\left(\frac{k}{2n-k}\right)+\Theta(\log^2 n),\qquad H_2(x)=-x\log_2 x-(1-x)\log_2(1-x),$$
with $H_2$ the binary entropy function (Lemma~\ref{lemma:grassoutput}). This exponent is $O\!\big(\min(k,n-k)\log n\big)$: it depends genuinely on both box dimensions $k$ and $n-k$, vanishes to $\Theta(\log^2 n)$ as $k\to 0$ or $k\to n$, and is largest, $\Theta(n)$, near $k=n/2$. For fixed $k$ it is $O(k\log n)$, so the $G_k$-dependent factors are polynomial in $n$ of degree $O(k)$; the leading exponent $\Theta(n\log n)$ of the general Theorem~\ref{thm:complexity} never appears.
\end{theorem}
\begin{proof}
...
$$f_{\mathrm{Gr}}(n,k)=4(2n-k)\,H_2\!\left(\frac{k}{2n-k}\right)+\Theta(\log^2 n)=O\!\big(\min(k,n-k)\log n\big).$$
The exponent is controlled by the box dimensions $k$ and $n-k$; it collapses to $\Theta(\log^2 n)$ at the extremes $k\to 0$ and $k\to n$ and reaches $\Theta(n)$ only near $k=n/2$.
\end{proof}

\begin{remark}
The general bound of Theorem~\ref{thm:complexity} has dominant exponent $\log_2((2n)!)-n=\Theta(n\log n)$, contributed by the $P_u\leq(2n-1)!!$ distinct $u$-side permutations and the $V\leq n!$ distinct $v$-side labels. In the Grassmannian case both counts, and every per-step branching, are confined to the $k\times 2(n-k)$ box and hence bounded by $G_k=\binom{2n-k}{k}$ (Lemma~\ref{lemma:grassoutput}), so the whole exponent collapses to $4\log_2 G_k=4(2n-k)H_2\!\left(\frac{k}{2n-k}\right)+O(\log n)$. This is $O\!\big(\min(k,n-k)\log n\big)$, a genuine function of both box dimensions: it is largest, $\Theta(n)$, at the balanced point $k=n/2$, and shrinks to $\Theta(\log n)$ as $k\to 0$ or $k\to n$, mirroring the shrinking of $\mathrm{Gr}(k,n)$ at the extremes. The improvement over the general algorithm is a factor $n^{\Theta(n)}=2^{\Theta(n\log n)}$ in the worst case, and for fixed $k$ the collapse is total: the count is polynomial in $n$ of degree $O(k)$.
\end{remark}

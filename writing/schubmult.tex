...
$$\code(\dom(u,v))=\code(\dom(u^{-1})^{-1})+\code(\dom(v^{-1})^{-1})$$
...
$$O\!\left(n^3N(u,\dom(v^{-1})^{-1})^4E_n\,\right),$$
...
$$O\!\left(n^3N(u,\dom(v^{-1})^{-1})^4\,\right),$$
% where $P_u$, $V$, $B$, $\Delta$, $E_n$ are as in Definition~\ref{def:complexityquantities} and $\Phi_v$ is the $v$-side precomputation cost of Proposition~\ref{prop:kdowncomplexity}. 
% Writing that bound as $2^{\,f(n)+O(\log n)}$,
% $$f(n)=2n\log_2 n+(3-2\log_2 e)\,n+\Theta(\log^2 n),$$
% and, when $v$ is dominant, $f(n)=n\log_2 n+(2-\log_2 e)\,n+\Theta(\log^2 n)$, where $\log_2 e=1/\ln 2\approx 1.4427$.
When $v$ is dominant, this simplifies to
$$O\!\left(n^3N(u,v)^2\,\right),$$
\end{corollary}
\begin{proof}
...
Let $u,v\in S_n$ and suppose $v$ is dominant. Let
$$N(u,v) = \{w\in S_\infty\mid [\sch_w(x;y)](\sch_u(x;y)\sch_v(x;z))\neq 0\}$$
Then there is a polynomial $P(n,N)$ such that the runtime of schubmult is
$$O(E_nP(n,N(u,v)))$$
...
$$N(u,\dom(v^{-1})^{-1})\leq |[v,\dom(v^{-1})^{-1}]|N(u,v)$$

% \begin{lemma}
% Suppose $v,v'\in S_\infty$ both satisfy $v\leq_L \dom(v)$ and $v'\leq_L \dom(v)$. If $v'\nleq_L v$, then $\dom(v)=\dom(v')$.
% \end{lemma}
% \begin{proof}
% % Note that if $\dom(v')\neq \dom(v)$, then $\dom(v') <_L \dom(v)$. Consider the element $v'' = v\wedge \dom(v')$. Then $\dom(v'') = \dom(v')$ since $\dom:S_n\to S_n$ is a lattice homomorphism, and $v''\leq_L v$, so there exists a $v''\leq_L v$ with $\dom(v'')=\dom(v')<_L\dom(v)$. 
% Consider the element $v'' = v\vee v'$. If $v'\nleq_L v$, then $v <_L v''\leq_L \dom(v)$, hence $\dom(v'')=\dom(v)$. Since $v'\cap v'' = v'

% % Suppose that $v'\nleq_L v$. Consider $v''=v\wedge v'$. We then have that $v'' <_L v'$ and $v'' <_L v$. Also, $\dom(v'') = \dom(v)\wedge \dom(v')=\dom(v')$. If $\dom(v')\neq \dom(v)$, then $\dom(v') <_L \dom(v)$, hence $\dom(v')\leq_L v$ since $v''\leq_L\dom(v'')\leq_L v$, contradicting the assumption that $v'\nleq_L v$. Thus, we may conclude that $\dom(v)=\dom(v')$, as desired.
% \end{proof}

% \begin{lemma} \label{lemma:equivvanish}
% The following are equivalent.
%   \begin{enumerate}
% \item $\ell(s_iw)<\ell(w)$ and $c_{u,v}^w(y;z)\neq 0$.
% \item $\ell(s_iu) > \ell(u)$ and $\partial^{s_i}(c_{u,v}^{s_iw}(y;z))\neq 0$, or $\ell(s_iu)<\ell(u)$ and $\partial^{s_i}(c_{u,v}^{s_iw}(y;z)) \neq -s_i(c_{s_iu,v}^{s_iw}(y;z))$.
% \end{enumerate}

% \end{lemma}
% % \begin{proof}
% % $$\sch_u(x;y)\sch_v(x;z) = \sum_{w}c_{u,v}^{w}(y;z)\sch_w(x;y)$$
% % $$\sch_{us_i}(x;y)\sch_v(x;z)+\sch_u(x;y)\sch_{v}(x;z)-\alpha_i\sch_{us_i}(x;y)\sch_{vs_i}(x;z)$$
% % $$=\sum_w c_{u,v}^w(y;z)\sch_{ws_i}(x;y)$$
% % $$=c_{us_i,v}^{ws_i}(y;z)+c_{u,vs_i}^{ws_i}(y;z)
% % \end{proof}

% \begin{theorem}[Equivariant support]
% Let $u,v\in S_n$ and $w\in S_\infty$. Then
% $$c_{u,v}^w(y;y)\neq 0 \Longleftrightarrow c_{u,v}^w(y;z)\neq 0\text{ and }c_{v,u}^w(y;z)\neq 0$$
% \end{theorem}
% \begin{proof}
% Note that $c_{u,v}^w(y;y)\neq 0$ is equivalent to
% $$\partial^w(\sch_u(x;y)\sch_v(x;y))|_{x=y}\neq 0$$
% By the fact that $c_{u,v}^w(y;y)=c_{v,u}^w(y;y)$, the implication $\Longrightarrow$ follows easily. We therefore must prove the opposite implication.

% %We prove this by induction on $\ell(u,w)$. If $u=w$, the result is $\sch_v(u(y);y)$, which is nonzero if and only if $u\geq v$ in Bruhat order. Thus $c_{u,v}^u(y;y)\neq 0$ if $c_{v,u}^u(y;z)\neq 0$, because this would require that $v\leq u$.

% % For the inductive step, we utilize the characterization in Lemma \ref{lemma:equivvanish}. Let $i$ be such that $\ell(s_iw) < \ell(w)$. Assume that $c_{v,u}^w(y;z)\neq 0$. We split this into two cases.

% For the inductive step, assume $c_{v,u}^w(y;z)\neq 0$. This is equal to
% $$\sum_{u_1u_2=u}c_{v,u_2}^w(y;y)\sch_{u_1}(y;z)$$
% By the theorem above there is a $\ell(v)+\ell(u_2)=\ell(w)$ and $c_{v,u_2}^w\neq 0$. By the inductive hypothesis, for any $\ell(u_1)>0$ we have $c_{v, u_2}^w(y;y)\neq 0$.
% % In the case that $\ell(s_iv) > \ell(v)$, we have that $\partial^{s_i}(c_{v,u}^{s_iw}(y;z))\neq 0$, and by the inductive hypothesis we have that $c_{v,u}^{s_iw}(y;y)\neq 0$. Assume on the contrary that $c_{u,v}^w(y;y)=0$. 
% \end{proof}

% \begin{proposition}[Output lower bound, uniform in $u$] \label{prop:outputlower}
% For all $u,v\in S_n$,
% $$\bigl|\operatorname{supp}\bigl(\sch_u(x;y)\,\sch_v(x;z)\bigr)\bigr|\ \geq\ \bigl|[e,v]_L\bigr|\ \geq \Lambda(v^{-1})$$
% with equality at $u=e$. In particular, taking $v=w_0(n)$, the product $\sch_u(x;y)\,\sch_{w_0}(x;z)$ has at least $n!$ terms for every $u\in S_n$.
% \end{proposition}
% \begin{proof}
% Work in $\mathbb{Q}(y,z)[x]$. The ordinary Schubert polynomials $\sch_{v''}(x)$ form a $\mathbb{Q}$-basis of $\mathbb{Q}[x]$, hence a $\mathbb{Q}(y,z)$-basis after extension of scalars, so they are linearly independent over $\mathbb{Q}(y,z)$. Since $\mathbb{Q}(y,z)[x]$ is an integral domain and $\sch_u\neq 0$, multiplication by $\sch_u$ is an injective $\mathbb{Q}(y,z)$-linear map, and therefore the $\bigl|[e,v]_L\bigr|$ polynomials
% $$\bigl\{\,\sch_u(x)\,\sch_{v''}(x)\ :\ v''\leq_L v\,\bigr\}$$
% are linearly independent. By Theorem~\ref{prop:doublesupport} every one of them is supported, in the Schubert basis, inside $\operatorname{supp}(\sch_u(x;y)\sch_v(x;z))$; so that set indexes a spanning family for a space of dimension at least $\bigl|[e,v]_L\bigr|$, and being itself linearly independent it has at least that many elements. Equality at $u=e$ is the case $\sch_u\sch_{v''}=\sch_{v''}$ of Theorem~\ref{prop:doublesupport}. For $v=w_0$ the ideal $[e,w_0]_L$ is all of $S_n$.
% \end{proof}

% \begin{remark}[What the lower bound needs from degree $0$] \label{remark:lowerboundmechanism}
% In a paper introducing a similar algorithm for multiplying ordinary Schur functions, Remmel and Whitney \cite{remmel1984multiplying} end with a complexity discussion that arrives at an output-sensitive bound and halts there. Their algorithm traverses a tree whose leaves are the tableaux $C(\lambda*\mu)$ enumerating the product, at a cost of $K|\mu|\,\bigl|C(\lambda*\mu)\bigr|$ steps; having noted that the constant might be improved, they add that this ``is not the main problem to determine the complexity of this algorithm,'' since
% \begin{quote}
...
% Theorem~\ref{prop:doublesupport} is what makes that tractable here, and it does so as a vanishing criterion rather than as a way around one. Sort the terms of the double product by the degree $\ell(u)+\ell(v)-\ell(w)$ of the coefficient $c_{uv}^{w}(y;z)$, which is determined by $\ell(w)$. A witness $v''$ contributes in degree $\ell(v)-\ell(v'')$, so each $w$ has a single degree attached to it, and degree $0$ comes from the single witness $v''=v$: the coefficients of degree $0$ are the ordinary structure constants $c_{uv}^{w}$, $\#P$-hard to compute and with vanishing known only to lie in $\Sigma_2^p$ \cite{pak2024vanishing}. The theorem gives necessary and sufficient conditions for $c_{uv}^{w}(y;z)\neq 0$, and thereby reduces vanishing in every positive degree to vanishing in degree $0$, the case that has actually been studied. The positive-degree coefficients are not a separate difficulty; they are governed by the classical one.

% What Proposition~\ref{prop:outputlower} draws from degree $0$ is the least that could be drawn: not which ordinary constants vanish, nor how many, but only that in each ordinary product $\sch_u\sch_{v''}$ at least one does not. That much is free, $\mathbb{Q}[x]$ being a domain. Theorem~\ref{prop:doublesupport} then propagates a single nonvanishing degree-$0$ coefficient of $\sch_u\sch_{v''}$ into a nonvanishing coefficient of the double product in degree $\ell(v)-\ell(v'')$, for every $v''\leq_L v$ at once, and the $\bigl|[e,v]_L\bigr|$ witnesses so obtained are what the bound counts. The proof is phrased through linear independence over $\mathbb{Q}(y,z)$ only because distinct witnesses of equal length contribute in the same degree, so that counting one term apiece requires an argument; the mechanism is the propagation.

% Proposition~\ref{prop:outputlower} therefore counts no degree individually, and never counts the degree-$0$ terms, which are the ones Remmel and Whitney faced, the Grassmannian case being Littlewood--Richardson. The bound is carried by the positive-degree terms, as the case $u=e$ shows: there the ordinary product $\sch_e\sch_v=\sch_v$ is a single term accounting for all of degree $0$, yet the double product $\sch_e(x;y)\sch_v(x;z)$ has $\bigl|[e,v]_L\bigr|$ terms, so all but one lie in positive degree. A bound extracted from degree $0$ alone would have returned $1$.

% No such bound is available for the ordinary product: its support lies entirely in degree $\ell(u)+\ell(v)$, so counting it means counting the coefficients that are themselves hard to count, with no second family to appeal to. For the same reason the question Remmel and Whitney raised is not settled here: their $|C(\lambda*\mu)|$ counts tableaux for a product of two Schur functions, with multiplicity and in a single degree, where the present mechanism does not apply. The double product does contain the independent family $\{\sch_u\sch_{v''}:v''\leq_L v\}$ within a single support, spread across the ranks of $[e,v]_L$ by Theorem~\ref{prop:doublesupport}, and it is the size of that family---an order-theoretic quantity---that the argument converts into a term count.
% \end{remark}

Suppose $\alpha,\beta\in S_n$ are dominant. Then
$$\Lambda(\alpha + \beta)=\frac{(2n - 1)!}{((\alpha+\beta)(i) - \code_i(\alpha) - \code_i(\beta))}$$

$$\Lambda(\alpha)\Lambda(\beta) = \frac{(n!)^2}{(\alpha(i) - \code_i(\alpha)(\beta(i) - \code_i(\beta)))}$$


$$(2n-1)!\sqrt{\Lambda(\alpha)\Lambda(\beta)} \geq 2^{n-1}n!\Lambda(\alpha + \beta)$$

...
$$O\!\left(n\cdot 4^nn!\Lambda(\dom(u,v))E_n\,\right),$$
...
$$O\!\left(n\cdot 4^nn!\Lambda(\dom(u,v))\right)$$
Writing the bound as $2^{\,g(n)+O(\log n)}$, the worst case is bounded by
...
$$N(u,v)\ \leq\ T(u,v)\leq\ n 2^{n+1}(2n-1)!\sqrt{\Lambda(\dom(u^{-1}))\Lambda(\dom(v^{-1}))}$$
and consequently

...
and
$$R_i^+f = f(x_1,\ldots,x_{i},t_i,x_{i+1},\ldots)$$

By (REFERENCE) above, we have
$$R_i^-\sch_v(x;t) = \sum_{v\downvar{i} v'}\sch_{v'}(x;t)\prod_{b\in Q_i(v',v)}(t_i - t_b)$$
and
$$R_i^+\sch_v(x;t) = \sum_{v\downvar{i+1} v''}\sch_{v''}(x;t)\prod_{b\in Q_{i+1}(v'',v)}(t_i - t_b)$$

$$R_i^-\sch_v(x;t) = \sum_{v\downvar{i} v'\downvar{i}\widetilde{v}}\sch_{\widetilde{v}}(x^{(i)};t)\prod_{a\in Q_i(\widetilde{v},v')}(x_i - t_b)\prod_{b\in Q_i(v',v)}(t_i - t_b)$$
$$R_i^+\sch_v(x;t) = \sum_{v\downvar{i+1} v''\downvar{i}\widetilde{v}}\sch_{\widetilde{v}}(x^{(i)};t)\prod_{a\in Q_i(\widetilde{v},v'')}(x_i - t_a)\prod_{b\in Q_{i+1}(v'',v)}(t_i - t_b)$$

$$R_i^- = \mathtt{varop}_i(t_i)R_i$$

We have
$$E_i\sch_w(x;t) = R_i^-\partial^i = \prod_{a\in Q_i(v',vs_i)}(t_i - t_a)\sch_{ws_i}(x;t)$$
Vine marked forest
...
\subsection{The Schubert bialgebra over the symmetric functions}

Consider the algebra $\coma$ as a left module over $\Lambda$. $\Lambda$ has a homomorphism $\pi_n:\Lambda\to\mathbb{Z}[x_1,\ldots,x_n]$ for each $n$, and we may define a module structure on $\coma$ by
$$f\cdot g = \pi_n(f)g$$

Consider the algebra $\hom_\Lambda(\coma,\Lambda)$ of graded linear functionals $\coma_n\to\Lambda_n$.

\subsection{The double Schubert basis}

In $\coma(y)$, each $\coma_n$ has a basis of double Schubert polynomials $\sch_u^{(n)}(y)$, where the largest right descent of $u$ is at most $n$, ensuring that $\sch_u(x)$ has at most $n$ variables. Schubert polynomials limited to a specific number of variables have well-defined structure constants $c_{u,v}^w$, independent of $n$, given by
$$\sch_u^{(n)}(x;y)\sch_v^{(n)}(x;y) = \sum_{w}c_{u,v}^w(y;y)\sch_w^{(n)}(x;y)$$
These are known to be Graham-nonnegative by \cite{graham1995positivity}. The coproduct of $\sch_w^{(n)}(x;y)$ is given by
$$\Delta(\sch_w^{(n)}(x;y)) = \sum_k\sum_{u,v}c_{u\mu_{[k]},v\mu_{[k]^c}}^{w\mu}(-y;-y)\sch_u^{(k)}(x;y)\otimes \sch_v^{(n-k)}(x;y)$$
...
$$\sch_w^{(n)} = \sum_{u\cdot v=w} \sch_v^{(n)}(x;y)\sch_u(y)$$
\end{lemma}

\begin{corollary}
$$\langle \sch_w^{(n)},\dsch_v^n(y)\rangle = \begin{cases}
\sch_{wv^{-1}}(y) & \text{if } \ell(wv^{-1}) = \ell(w) - \ell(v)\\
0 & \text{otherwise}
\end{cases}$$
\end{corollary}

\subsection{The generic elementary basis}

Recall the top factorial elementary symmetric polynomial in $x$ and $t_1,\ldots,t_n$ is defined by
$$E_{n,n}(x;t) = \prod_{i=1}^n(x_i-t_1)$$
Lower degrees are given by
$$(-1)^{n-k}\partial_t^{s_{n-k}\cdots s_{1}}(E_{n,n}(x;t)) = E_{k,n}(x;t)$$
where $\partial_t^{s_{n-k}\cdots s_{1}}$ is the divided difference operator in the $t$ variables. We may introduce a new doubly-indexed set of indeterines $t_{i,j}$ for $i\geq 1$ and $1\leq i\leq j$ and define the generic factorial elementary symmetric polynomial $E_{i}^j$ by
$$E_i^j = E_{i,j}(x;t_{1,j},\ldots,t_{j + 1 - i,j})$$

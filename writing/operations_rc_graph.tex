...
\begin{definition}
Let $\mathrm{lift}:\BRC_n\to\mathcal{M}_n$ be the lift map, and let $\mathcal{L}<\mathcal{M}_n$ be the subring generated by the image. Let $Z\leq \mathcal{L}$ be the ideal that is the kernel of $\RC:\mathcal{L}\to\BRC_n$. Then $Z$ is the two sided ideal generated by the elements
$$\{D - \mathrm{lift}(\RC(D))\mid D\in \mathcal{L}\}$$
for all $D\in\mathcal{L}$. Consider the two-sided ideal $\overline{Z}\leq \mathcal{M}_n$ generated by the elements
$$R - \mathrm{lift}(\RC(R))$$
for all $R\in\BRC_n$.
\end{definition}

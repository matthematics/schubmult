...
\begin{proof}
  ...
  $$R'' = ((R\setminus \{(i,j)\})\cup \{(i, j - 1)\})^{\leq(i, j - 1)}$$
...
We first address the special case where $\mathrm{word}(R)$ and $\mathrm{word}(S)$ differ by a single swap. Suppose $\mathrm{word}(R) = a_1\cdots a_N$ and $\rtt_r(a_1\cdots a_N) = \alpha_n$ for some $r$. Let $q$ be an index such that there are distinct $\overline{a},\overline{b}\in\mathrm{rl}(\mathrm{word}(R),q)$ satisfying $\overline{a_q}<a^{[b]}<c^{[d]}<\overline{a_{q+1}}$. Without loss of generality, we may assume that $q=1$ by \cite[Lemma~5.9]{nadeau2023permutahedral}. Necessarily, $|a_1 - a_2|\geq 2$, since otherwise no such commutation move would exist.  Assume without loss of generality that $a_1 < a_2$ and that $a^{[b]}$ is the minimal element of the rootlist. If $r>2$, the only way the Little bump at position $r$ could potentially invalidate the swap is if $a_1 = a - 1$ and $a^{[b]}$ is bumped. After the bump, we obtain $\mathrm{word}(S)$, and when we injectify the bumped word, if $(a-1)^{[j]}$ occurs in the right portion then $\overline{a_1}\mapsto (a-1)^{[j+i]}$ for some $i>0$. This would indeed reduce the count of the rootlist; however, we may assume without loss of generality that $a$ occurs at position $3$, since stripping out this middle portion would remain reduced aosibgsaoibsogiabsIABGSIGBS. In that case, $a_1a_2a_1$ fails to be reduced, so the bumping sequence would continue and subtract $1$ from the first index, resolving the loss.

% The Little bump at position $r$ decrements $a_r$ to $a_r - 1$, and by Lemma \ref{lemma:littlebumprecurse} any subsequent bumps occur strictly to the left. Thus, the open interval between $\overline{a_q}$ and $\overline{a_{q+1}}$ still contains at least two elements of $\mathrm{rl}(\mathrm{word}(\lmap(R)),q)$, so that the swap is still valid. By symmetry, the argument for $S$ is identical.
%If $q\geq r - 1$, then by Lemma \ref{lemma:littlebumprecurse} the letters weakly to the right of index $q$ are not affected, so that in particular 
%$$\mathrm{rl}(\mathrm{word}(\lmap(R)),q) = \mathrm{rl}(\mathrm{word}(R),q)$$
%It follows in the case where $q>r$ that 
%$$\ltb{\mathrm{word}(R)}{r} \sim_\forest \ltb{(a_1\cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_{q-1}a_{q+1}a_qa_{q+2}\cdots a_n$$
%so that
%$$\lmap(R)\equiv_\forest \lmap(S)$$
% If $q=r$, assume that $|a_q - a_{q+1}|>2$. Assume first also that $a_{q} < a_{q+1}$. Then the distance between $\overline{a_q}$ and $\overline{a_{q+1}}$ increases, so the open interval between them still contains at least two elements of $\mathrm{rl}(\mathrm{word}(\lmap(R)),q)$. If on the contrary $a_q > a_{q+1}$, any element of $\mathrm{rl}(\mathrm{word}(R),q)$ of the form $i^{[0]}$ remains less than $\overline{a_q-1}$. We therefore need to address the case where $(a_q-1)^{[j]}\in \mathrm{rl}(\mathrm{word}(R),q)$ for some $j\geq 1$. With the reindexing obtained by re-injectifying the bumped word, we obtain that 
% $$\overline{a_q - 1} = (a_q-1)^{[j+1]}$$
% and the count of the rootlist is maintained. By symmetry, the argument for $q=r-1$ is identical. The cases where $q\in \{r-1,r\}$ and $|a_q - a_{q+1}|> 2$ are therefore closed.

% Assume now that $q = r$ and $|a_q - a_{q+1}|=2$. If $a_q > a_{q+1}$, the hypotheses of the lemma do not hold, since there is no $w'$ satisfying $\lmap(R),\lmap(S)\in \RC_{n+1}(w')$. If $a_q < a_{q+1}$, then the Little bump at position $r$ decrements $a_q$ to $a_q - 1$, and the letter $a_{q+1}$ is unaffected. The open interval between them still contains at least two elements of $\mathrm{rl}(\mathrm{word}(\lmap(R)),q)$, so that the swap is still valid. For $q=r-1$ and $|a_q - a_{q+1}|=2$, the argument is symmetrical.

% Now we address the case where $q<r-1$. Observe by Lemma \ref{lemma:littlebumprecurse} that
% $$\mathrm{word}(\lmap(R)) = \ltb{(a_1\cdots a_{q-1}a_q a_{q+1} \cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_n$$
% By \cite[Lemma~5.9]{nadeau2024forest}, stripping off a suffix does not affect the validity of a swap. Thus,
% $$a_1\cdots a_{q-1}a_q a_{q+1} \cdots a_{r-1}\sim_\forest a_1\cdots a_{q-1}a_{q+1} a_{q} \cdots a_{r-1}$$
% There are RC graphs $R',S'$ that have these respective words, and $R'\equiv_\forest S'$ by Proposition \ref{proposition:forestcommute}. It is possible, however, that the Little bump at position $p$ is not on the correct letter to achieve $\lmap(R')$ and $\lmap(S')$, thwarting a direct induction. If that is the case, we use the reduction technique of Lemma \ref{lemma:descentchop}; that is, let $(i,j)$ be the coordinates corresponding to the index $p$ in the word. Then
%   $$\ltb{\mathrm{word}(R^{\leq i})}{p'} = \mathrm{word}(\{(a,b)\in R\mid a\leq i\text{ and }b> j\}\cup \lmap(R^{\leq(i, j)}))$$
% for some corresponding $p'$. If the remapped swap occurs within one of the two sets in the union, since they are strictly smaller than the original word, the special case where the swap is achieved by a chute move ensures that the indexes remain adjacent in the larger word, and we may apply induction. If the remapped swap occurs between the two sets, then it is still valid since the open interval between the swapped letters contains at least two elements of $\mathrm{rl}(\mathrm{word}(\lmap(R)),q)$, and we may apply induction to the two sets separately. In all cases, we conclude that
\end{proof}

...
\begin{lemma}
Let $w\in S_\infty$ satisfy $\maxd(w)=n$. Then $w$ admits an RC graph of height $n$ with row $n$ empty if and only if there is an $i<n$ such that $\code_i(w)=0$.
\end{lemma}
\begin{proof}
...
\begin{lemma} \label{lemma:littlebumprecurse}
  Let $a_1\cdots a_n$ be a reduced word for a permutation $w$ and suppose $r$ is an index such that $\rtt_r(a_1\cdots a_n)=(m, m+1)$ (that is, the right root of the word $a_1\cdots a_n$ at position $r$ is the simple root $\alpha_m$). If the initial step of a Little bump at position $r$ does not result in a reduced word, then there is a unique index $p<r$ such that
  $$\ltb{(a_1\cdots a_n)}{r} = \ltb{(a_1\cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_n$$
\end{lemma}
\begin{proof}
Assume the hypotheses of the lemma, which implies that the word $a_1\cdots a_{r-1}(a_r - 1)a_{r+1}\cdots a_n$ is not reduced. Thus, there is some unique index $p$, not necessarily less than $r$, such that the root at position $p$ is equal to the root at position $r$ after decrementing $a_r$. Consider the word of length $n+2$
$$a' = a_1\cdots a_{r-1}a_r(a_{r}-1)a_ra_{r+1}\cdots a_n$$
We claim that
$$\rtt_{r}(a') = \rtt_r(a_1\cdots a_{r-1}(a_r -1)a_{r+1}\cdots a_n)$$
This is because
$$s_{a_r}s_{a_r - 1}(\alpha_{a_r}) = \alpha_{a_r - 1}$$ so that
$$\rtt_{r}(a') = s_{a_n}\cdots s_{a_{r+1}}s_{a_r}s_{a_r - 1}(\alpha_{a_r}) = s_{a_n}\cdots s_{a_{r+1}}(\alpha_{a_r - 1}) = \rtt_r(a_1\cdots a_{r-1}(a_r - 1)a_{r+1}\cdots a_n)$$
However, the word
$$a_1\cdots a_{r-1}a_r(a_{r}-1)a_{r+1}\cdots a_nm$$
has the same right root at $r$, since the exchange property allows us to delete the rightmost $a_r$ and place the descent $m$ at the end of the word, leaving roots at indices to the left unchanged. This root is equal to 
$$s_m\rtt_r(a_1\cdots a_{r-1}(a_r - 1)a_{r+1}\cdots a_n)$$
which is still $\rtt_r(a_1\cdots a_{r-1}(a_r - 1)a_{r+1}\cdots a_n)$ by assumption. However,
$$\langle\rtt_r(a_1\cdots a_{r-1}a_r(a_{r}-1)a_ra_{r+1}\cdots a_n)\mid \alpha_m\rangle < 0$$
due to the fact that $\alpha_m$ is the root at position $r+2$ in this word, which is a contradiction. Thus, $p<r$.

Now, consider the reduced word $a_1\cdots a_{r-1}$. Since adding $a_r - 1$ is no longer reduced, we have that the root at position $p$ is equal to $\alpha_{a_r - 1}$, and the next Little bump is at position $p$. We may repeat the argument, yielding a recursive process that ensures each additional bump is to the left by induction on the length of the word.
\end{proof}
...
\begin{proof}
  ...
  If $t$ is within the $V$ portion, Lemma \ref{lemma:bumprecurse} allows us to rewrite this as

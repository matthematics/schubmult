...
\begin{proof}
  ...
  for some $q$. Adding on this right-hand border past column $j-1$, we see that the root at $p'$ in $R''$ expands to a root at the same position in $R^{<i}$ which therefore is at some position $q$ in $R^{<i}$, and the Little bump on position $q$ in $R^{<i}$ is exactly the same as the Little bump on position $p'$ in $R''$, which by induction can be embedded as an RC graph to the left of column $j$, and the claim follows by taking the resulting word.
...
Two reduced words are \emph{commutation equivalent} if one is obtained from the other by a sequence of interchanges of adjacent letters differing by at least $2$. Commutation classes are \emph{not} preserved by bumping: a bump decrements a letter, and if the decrement brings two commuting letters to distance $1$ the two runs cease to agree and may end at different permutations. Equality of the terminal permutations is exactly the certificate that no such divergence has occurred.

...
% \begin{lemma}
% Suppose $w\in S_\infty$ and there exist $i<j<k$ for which $w(i)>w(j)>w(k)$. Then there exists an $i'$ such that $w(i')>w(i'+1)>w(i'+2)$.
% \end{lemma}
% \begin{proof}
% CITATION
% \end{proof}
...
\begin{lemma} \label{lemma:notsimple}
Let $R$ be an RC graph with $\hr(R) = m$ and suppose $\rtt_R(i,j) = \alpha_m$. Then $(i, j - 1)\notin R$.
%$(i,j),(i,j - 1)\in R$. Then $\rtt_R(i,j)\neq \alpha_m$.
\end{lemma}
\begin{proof}
  %We may assume without loss of generality that $i = 1$ and that there are no crossings in row $1$ to the right of $(1,j)$. 
  If $m$ is the only descent, then the characterization of Grassmannian RC graphs ensures that if $j>1$, then $(i,j-1)\notin R$. Otherwise, suppose $(a,b)\neq (i,j)$ satisfies $\rtt_R(a,b)=\alpha_k$ for some $k<m$. Then $R' = R\setminus\{(a,b)\}$ has $\maxd(R')=m$. As long as $(a,b)\neq (i, j-1)$, this gives us the result. This must be the case because two descents $k,m$ with $k\neq m$ cannot be adjacent in the same row, since if $m-k>1$ there must be a space between them, and if $k = m-1$ then the roots cannot be in the same row at all.
\end{proof}

\begin{lemma} \label{lemma:simplebound}
Suppose $R\in \RC_n(w)$ and $\rtt_R(i,j) = \alpha_p$. Then $p\geq i$.
\end{lemma}
\begin{proof}
If $p$ is a right descent of $w$, then $p - i + 1$ is also a right descent of $\wof{S}$, where 
  $$S = \{(i' - i + 1, j)\mid (i', j)\in R, i'\geq i\}.$$
  In particular, $p\geq i$. 
\end{proof}

\begin{lemma} \label{lemma:notsimpleatall}
Let $R\in \RC_m(w)$ be an RC graph. Suppose $(i,j)\in R$  and $(i,j')\in R$ for all $j'\leq j$. Then $\rtt_R(i,j)\neq \alpha_p$ for any $p\neq i$.
%, and $\rtt_R(i,j)=\alpha_i$ if any only if $(i+1,j)\notin R$, and $(1,j'),(2,j')\in R$ for all $j'<j$.
\end{lemma}
\begin{proof}
  
  By Lemma \ref{lemma:simplebound}, we may assume that $i=1$. In that case, we claim that $\rtt_R(1,j) = (1,a)$ for some $a\geq 2$. 
  
  In computing $\rtt_R(1,j)$, the initial root is $\alpha_j$ .The first reflections to apply are $s_1\cdots s_{j-2}s_{j-1}$, and 
  $$s_1\cdots s_{j-2}s_{j-1}(\alpha_j) = (1, j + 1)$$
  which is not simple. If $\hr(R) = 1$, then we are done. Otherwise, inducting on height, no reflection can increase the first index of the root since the only $(a,b)$ with $a+b-1=1$ are precisely $a=b=1$, which are not in rows below. This verifies the equation
  $$\rtt_R(1,j) = (1,a)$$
  Thus, if $p>1$ we have that $\rtt_R(1,j)\neq \alpha_p$, as desired.
\end{proof}


% \begin{lemma}
% Suppose $w$ is a permutation not equal to the identity and $R\in \RC_n(w)$. Then $\wtt_1(R)\geq \code_1(w)$.
% \end{lemma}
% \begin{proof}
% Suppose $\code_1(w) = k>0$. Taking the parabolic subgroup $J_1$, we have that $w^{J_1} = s_ks_{k-1}\cdots s_1$. This is a subword of every reduced word for $w$. In any reduced word that admits a compatible sequence, every letter before an occurence of $s_1$ must have label $1$. It follows that $\wtt_1(R)\geq k$, as desired. The case $k=0$ is trivially true for all nonempty RC graphs.
% \end{proof}

% \begin{lemma}\label{lemma:descentbound}
% Suppose $\maxd(w)=m$ and $k$ is the maximal nonnegative integer such that $w(m - a) > w(m - a + 1)$ for all $0\leq a\leq k$. For any $R\in \RC_m(w)$ we have that if $\rtt_R(i,j) = \alpha_m$, then $m - k\leq i+j-1\leq m$.
% \end{lemma}
% \begin{proof}
% Fix $(i,j)$. The case $m=1$ is trivial, so we induct on $m$, assuming that $m>1$. If $i+j-1=m$, there is nothing to prove, so we assume that $i+j-1<m$. Assume without loss of generality  that $i$ is the maximal row among all $R'\in \RC_m(w)$ and $(a,b)\in R'$ with $\rtt_{R'}(a,b) = \alpha_m$ and $a+b-1=i+j-1$. With this assumption, it is enough to address the case $i=1$, since we may consider the RC graph $R'' = \{(a-i+1,b)\mid (a,b)\in R', a\geq i\}$, which has $\rtt_{R''}(1,j) = \alpha_{m -i + 1}$ and $j=i+j-1 - (i - 1)$. There exists a chute move on $R$ since $R$ is not the bottom RC graph, and appealing to maximility the move must be at (1,j)$.


% %We claim that $i+j-1\geq m-k$.
%   % If the only descent of $w$ is $m$, this follows from the characterization of Grassmannian RC graphs. This also establishes a base case for induction on length. Otherwise, suppose $\code_1(w) = k>0$. Then $s_k$ is a left descent of $w$, and $\rtt_R(1,k)\neq \alpha_m$ by Lemma \ref{lemma:notsimple}.
% \end{proof}

\begin{definition}
Suppose $R$ is an RC graph and $(i,j)\in R$. Define
$$R^{\leq (i,j)} = \{(i',j')\in R :i'\leq i\text{ and }j'\geq j\}.$$
\end{definition}

% \begin{lemma} \label{lemma:descentchop}
% Let $R$ be an RC graph with $\hr(R)=m$ and suppose $(i,j)\in R$ with $\rtt_R(i,j)=\alpha_m$. Then $\maxd(R^{\leq (i,j)}) = i + j - 1$.
% \end{lemma}
% \begin{proof}
%   The only possibility contrary to this is if $\maxd(R^{\leq (i,j)})>i+j-1$. Assume without loss of generality that any chute move originating in $R^{\leq (i,j)}$ will exit the rectangle (that is, move past row $i$). We claim that the only such chute moves originate in row $i$. To see this, let  
% $$S = \{(i' + m - i,j' + i - m) : (i',j')\in R^{\leq (i,j)}\}.$$
  
%   % In that case, there is a crossing $(i',j')\in R^{\leq (i,j)}$ with $\rtt_R(i',j')=\alpha_{i+j-1}$, and $i'+j'-1>i+j-1$. By Lemma \ref{lemma:simplebound}, we have that $i'\leq i$, so that $j'<j$, contradicting the definition of $R^{\leq (i,j)}$.
%   % Let $w=\wof{R}$. We have that $i+j-1=m-a$ for some $a\geq 0$ by Lemma \ref{lemma:descentbound}, and $(i,j-1)\notin R$ by Lemma \ref{lemma:notsimple}. We argue by induction on $i$. If $i=1$, then $w$ is in fact dominant by Lemma \ref{lemma:descentbound}, since $w(i - 1)> w(i)$ for all $i\leq m$. Hence, so is $R^{\leq (i,j)}$, and we have the result. 
  
%   %$\wof{R^{\leq (-,j)}}=\wof{R}u$ decreasing the length by precisely $\ell(u)$ for some $u$, and all simple reflections in $u$ are strictly less than $m-1$, so no new descents are created and the $m$ descent remains, as desired. If $w(m-1) > w(m)$, the same argument applies with $i+j-1 = m-a$ for some $a\geq 0$.
 
% %has $\maxd(R') = \maxd(R) - j + 1$, since this is simply a uniform shift of the letter indices. In particular, relative order of descents is the same, and $\maxd(R')=i$, so that $\maxd(R^{\leq(i,j)}) = i + j - 1$. 
% %It remains to show that $i + j - 1$ is a right descent of $R^{\leq (i,j)}$.
% % We prove this by induction on $\hr(R)$. For $\hr(R)=1$, $i=j=1$ and we have the result. Otherwise, we argue by induction on $m - i$. If $i = m$, then $j=1$ and no reduction occurs. If $1<i<m$, we may consider $\trm(R)$, which has height $m-1$ and contains $(i-1,j)$ with $\rtt_{\trm(R)}(i-1,j)=\alpha_{m-1}$. By induction on $m-i$, we have $\maxd(\trm(R)^{\leq (i-1,j)}) = i + j - 2$, hence $i + j - 1$ is a right descent of $R^{\leq(i,j)}$. We wish to see that it is maximal. If not, since the inductive hypothesis holds, any larger descent must occur in the first row. Assuming that it is in fact the largest descent, Lemma \ref{lemma:notsimple}

% % as desired. If $i=1$, we must have that $j>1$ since $\hr(R)=j$ otherwise, so assume $j>1$. By Lemma \ref{lemma:notsimple} we have that $(1,j-1)\notin R$. Note that
% % $$R^{\leq (i,j)} = \{(1,j)\}\cup \{(1,j')\in R : j'\leq j\}.$$
% % Since $(1, j-1)\notin R$, $j$ is a right descent of $\wof{R^{\leq (i,j)}}$, hence the maximal descent, and the result follows.
% \end{proof}

\begin{lemma} \label{lemma:mroot}
Let $a=a_1\cdots a_n$ be a reduced word for a permutation $w$ with $\maxd(w) = m$. Let $i$ be the index with $\rtt_a(i) = \alpha_m$. Let
$$a' = a_1\cdots a_{i-1}(a_i - 1)a_{i+1}\cdots a_n$$
If $(a_{i} - 1)a_{i+1}\cdots a_n$ is reduced, then $\rtt_i(a') = (p,m)$ with $p\leq m - 1$.
\end{lemma}
\begin{proof}
Write
$$u = s_{a_n}\cdots s_{a_{i+1}}$$
Note that
$$s_{a_i - 1}s_{a_i}(\alpha_{a_i - 1}) = \alpha_{a_i}.$$
We have that
$$\rtt_a(i) = u(\alpha_{a_i}) = \alpha_m$$
which is also
$$\rtt_a(i) = us_{a_i - 1}s_{a_i}(\alpha_{a_i - 1})=us_{a_i - 1}(\alpha_{a_i - 1}+\alpha_{a_i}) = us_{a_i - 1}(\alpha_{a_i}) -u(\alpha_{a_i - 1})$$
By assumption $s_{a_i - 1}u^{-1}$ is reduced, so $u(\alpha_{a_i - 1})$ is a positive root. Hence
$$us_{a_i - 1}(\alpha_{a_i}) = u(\alpha_{a_i - 1}) + \alpha_m$$
Thus, either there is a $p < m$ such that
$$u(\alpha_{a_i - 1}) = \alpha_p+\cdots+\alpha_{m-1}$$
or there is a $q>m$ such that
$$u(\alpha_{a_i - 1}) = \alpha_{m + 1}+\cdots+\alpha_q$$
However, since $\maxd(u^{-1})<m$, the latter case is impossible, and we have that
$$\rtt_{a'}(i) = (p, m)$$
as desired.
\end{proof}

% \begin{lemma} \label{lemma:bumptwine}
...
% \end{lemma}
% \begin{proof}
%   We prove this by induction on $k$. For $k=2$, under the hypotheses of the lemma and defining
%   $$b = a_1\cdots a_{i_1-1}(a_{i_1} - 1)a_{i_1+1}\cdots a_n$$
% we have that $\rtt_b(i_1) = (p,m)$ with $p\leq m-1$ by Lemma \ref{lemma:mroot}. We also have that 
% $$\rtt_a(i_2) = (m, m + 2)$$
% Hence,
% $$\rtt_b(i_2) = (m+1,m+2)$$
% from which we can conclude that $i_2$ is not the next bumping index. Of course, if the bumping algorithm terminates at this point, we are done. Otherwise, call this bumping index $t_1$. If $t_1<i_2$, the base case immediately follows by the condition that the algorithm moves to the left. Suppose then that $i_2<t_1<i_1$. Assuming without loss of generality that $n=i_1$, we must have that $a_r < a_{i_1}$ for all $i_2<r<i_1$. In particular, $\rtt_b(t)=(p,q)$ with $q<m$, hence the algorithm again does not proceed to index $i_2$. Perhaps after multiple further iterations, the algorithm will either stop or pass $i_2$, never bumping it, and we have the base case.

% INDUCTIVE CASE BUMPY EVEN HIGHEREE
% \end{proof}

OK move along like little

\begin{proposition}[The bump map is a Little bump]\label{proposition:little_bump}
Let $R$ be as in Definition \ref{definition:lmap} with $m=\maxd(\wof R)$, and let $p$ be the index of the root $\alpha_m$ in $\mathrm{word}(R)$. Then
$$\mathrm{word}(\lmap(R))=\ltb{\mathrm{word}(R)}{p}.$$
\end{proposition}
\begin{proof}
  By Lemma \ref{lemma:notsimple}, since $\rtt_R(i,j) = \alpha_m$, we must have that $(i,j-1)\notin R$. We have that
  $$\rtt_R(i,j-1)=s_m(R\setminus \{(i,j)\})[i,j]^{-1}(\alpha_{i + j - 2}) = s_m(R\setminus \{(i,j)\})[i,j]^{-1}s_{i+j-1}s_{i+j-2}(\alpha_{i + j - 1})$$
  Further simplifying,
  $$\rtt_R(i,j-1)=(R\setminus \{(i,j)\})[i,j]^{-1}s_{i+j-2}(\alpha_{i + j - 1})=(R\setminus \{(i,j)\})[i,j]^{-1}(\alpha_{i+j-2}+\alpha_{i + j - 1})$$
  $$=(R\setminus \{(i,j)\})[i,j]^{-1}(\alpha_{i+j-2}) + \alpha_m$$
  If $(R\setminus \{(i,j)\})[i,j]^{-1}(\alpha_{i+j-2}) < 0$, then this root was already present in $R\setminus \{(i,j)\}$ at some $(i',j')>(i,j)$. This implies that
  $$R' = (R\setminus \{(i',j')\}))\cup \{(i,j - 1)\}$$
  is an RC graph for $\wof{R}$ by the strong exchange property, and $R'[i,j]^{-1}(\alpha_{i+j-1}) = \alpha_m$. However, by Lemma \ref{lemma:notsimple}, in that case $\rtt_R(i,j)\neq \alpha_m$, which is a contradiction. Thus, $(R\setminus \{(i,j)\})[i,j]^{-1}(\alpha_{i+j-2})$ is a positive root, hence so is $\rtt_R(i,j-1)$. It also follows that $\rtt_R(i,j-1)=(a,b)$ with $a\leq m<b$ since applying $s_m$ to this root adds $\alpha_m$ rather than subtracting it. Thus, the first step of $\lmap(R)$ is to replace $a_p$ with $a_p-1$, which is exactly what the Little bump on position $p$ does. If the resulting word is reduced, then we have the result. 

  Otherwise, define
  $$R'' = \{(i',j')\in R'\mid i' < i\text{ or }i'=i\text{ and }j'>j-1\}.$$
  If $\maxd(R'') = i + j - 2$, then define
  $$S = \{(i',j')\in R'\mid i'>i\text{ or }i'=i\text{ and }j'\leq j-1\}.$$
  Then
  $$\lmap(R) = \lmap(R'') \cup S$$
  Since
  $$\mathrm{word}(\lmap(R)) = \mathrm{word}(\lmap(R''))\mathrm{word}(S)$$
  the result then follows by induction.


  
  % Otherwise, define
  % $$R^{(1)} = ((R\setminus \{(i,j)\})\cup \{(i, j - 1)\})$$
  % There is a unique $(i_1,j_1)\in R^{(1)}$ with $\rtt_{R^{(1)}}(i_1,j_1)=-\rtt_{R^{(1)}}(i,j-1)$ by the strong exchange property, and by the above either $i_1<i$ or $i_1=i$ and $j_1>j-1$. We can rule out the case $i_1=i$ by the fact that no such root can cancel $\rtt_{R^{(1)}}(i,j-1)$ since it is in the same row. Hence, $i_1<i$.

  % Set $(i_0,j_0) = (i,j)$ and $R^{(0)}=R$. Inductively, assume that we have defined 
  % $$R^{(k)} = ((R^{(k-1)}\setminus \{(i_{k-1},j_{k-1})\})\cup \{(i_{k-1}, j_{k-1} - 1)\})$$
  % and $\mathrm{word}(R^{(k)})$ coincides with step $k$ of the Little bump algorithm at position $p$, with $i_{k-1} + j_{k-1} - 1 \leq m - k$.
  
  % If $R^{(k)}$ is reduced, then both algorithms stop, and we are done. Otherwise, suppose $(i_k,j_k)$ satisfies $\rtt_{R^{(k)}}(i_k,j_k)=-\rtt_{R^{(k)}}(i_{k-1},j_{k-1}-1)$. By the same argument as above, we have that $i_k<i_{k-1}$.
  
  
  
  % We then note that
  % $$\lmap(R) = ((R\setminus\{(i,j)\})\cup \{(i, j - 1)\})^{\geq i}\cup \{(a,b)\in R\setminus\{(i,j)\}\mid a<i\text{ and }b> j - 1\}\cup \lmap(R'')$$
  % By induction on the height of $R$, we have that 
  % $$\mathrm{word}(\lmap(R''))=\ltb{\mathrm{word}(R'')}{p'}$$ 
  % for some $p'$. We further argue that
  % $$\ltb{\mathrm{word}(R^{<i})}{q} = \mathrm{word}(\{(a,b)\in R\setminus\{(i,j)\}\mid a<i\text{ and }b> j - 1\}\cup \lmap(R''))$$
  % for some $q$. Adding on this right-hand border past column $j-1$, we see that the root at $p'$ in $R''$ corresponds to a root at the same grid position in $R^{<i}$. We can identify the same root at some position $q$ in $R^{<i}$, and the Little bump on position $q$ in $R^{<i}$ is exactly the same as the Little bump on position $p'$ in $R''$, which by induction can be embedded as an RC graph to the left of column $j$, and the claim follows by taking the resulting word.

  % Thus,
  % $$\mathrm{word}(\lmap(R)) = \ltb{\mathrm{word}(R^{<i})}{q}a_{\left|R^{<i}\right| + 1}\cdots a_{p-1}(a_p - 1)a_{p+1}\cdots a_{|R|} = \ltb{\mathrm{word}(R)}{p}$$
  % as desired.
\end{proof}
...
\begin{theorem}[Bijectivity of transition]\label{theorem:zero_bijection}
...
\end{theorem}
\begin{proof}
Weight preservation is Lemma \ref{lemma:zeroweight}. On words, $\zeromap$ realizes Algorithm~2 of \cite{little2003combinatorial}, which is bijective from the set of reduced words of $w$ onto the union of the sets of reduced words of the permutations $w'$ with $w\downvar{n}w'$ and $\ell(w')=\ell(w)$.  Restricting to the words of RC graphs, which may be a smaller set of words on both ends, the map at least remains injective.

It remains to upgrade the map to a bijection of sets of RC graphs. An RC graph of a fixed height is determined by its word together with its weight, since given a word we can recursively back out the pairs $(i,j)\in R$ from the weight; since $\zeromap$ is injective on words and preserves weights, it is injective on $\RC_n(w)^0$. For surjectivity, the identity of Bergeron--Sottile \cite{bsskew} recalled with the definition of $\downvar{n}$ gives
...
\end{proposition}
\begin{proof}
Under the Edelman--Greene correspondence \cite{edelman1987balanced}, an RC graph $R$ corresponds to a pair $(P(R),Q(R))$, where $P(R)$ is the insertion tableau of $\overleftarrow{\mathrm{word}(R)}$ and $Q(R)$ is the recording tableau, a standard tableau characterizing the (reversed) word. Morse and Schilling \cite{morse2016crystal} prove that the raising and lowering operators preserve $P(R)$ and act on $Q(R)$ as the ordinary crystal operators on semistandard tableaux; thus the recording tableau parameterizes each connected component of $\RC_n(w)$, the components being indexed by the insertion tableaux.

By Proposition \ref{proposition:little_bump}, $\zeromap$ acts on the underlying word by a composite of Little bumps. By the theorem of Hamaker and Young \cite[Theorem~4.4]{hamaker2014relating}, Little bumps preserve the recording tableau and commute with Coxeter--Knuth moves; the latter means the induced effect on the insertion tableau depends on $P(R)$ alone. In the coordinates $(P,Q)$, therefore, $\zeromap$ acts only on the first coordinate while $e_i$ and $f_i$ act only on the second, and the two commute.
\end{proof}
...
a bounded RC graph with $p$ rows. Since each application of $\zeromap$ preserves the weight vector (Lemma~\ref{lemma:zeroweight}), every intermediate graph again has all of its crossings in rows $1,\ldots,p$, so the iteration is well-defined. Moreover the result depends only on the crossing set of $R$ together with $\maxd(\wof{R})$: by Definition \ref{definition:zeromap}, an application of $\zeromap$ to a graph $S$ at a height exceeding $\maxd(\wof{S})$ deletes an empty row and does nothing else, so assigning $R$ a larger admissible height only inserts pure truncation steps into the iteration.
\end{definition}
...
\begin{definition}
For a word $\mathbf{r}=\mathbf{r}_1\mathbf{r}_2\cdots \mathbf{r}_k$, a sequence of positive integers $a_1a_2\cdots a_k$ is \emph{compatible} with $\mathbf{r}$ if $a_i\leq \mathbf{r}_i$ for all $i$, $a_i\leq a_{i+1}$ for all $i$, and $a_i<a_{i+1}$ if $\mathbf{r}_i<\mathbf{r}_{i+1}$. For a word $\mathbf{r}$, define $\mathrm{comp}(\mathbf{r})$ to be the set of all sequences compatible with $\mathbf{r}$.

Slide polynomials $\mathfrak{F}(\mathbf{r})$ for a word $\mathbf{r}$ are defined as 
$$\mathfrak{F}(\mathbf{r}) = \sum_{a\in \mathrm{comp}(\mathbf{r})} x^a$$
For an alternative indexing, for a weak composition $a$ we associate a word $\mathbf{r}(a)$ by writing $i$ exactly $a_i$ times for each $i$ in increasing order, and define 
$$\mathfrak{F}_a = \mathfrak{F}(\mathbf{r}(a)).$$

A \emph{quasi-Yamanouchi} RC graph with word $\mathbf{r}$ is the unique RC graph $R$ such that $\mathrm{word}(R)=\mathbf{r}$ and no simple chute move preserves the word of $R$. For any RC graph $A$, we define the \emph{quasi-Yamanouchi map} $A\mapsto\QY(A)$ to map $A$ to the unique quasi-Yamanouchi RC graph with the same word as $A$.
\end{definition}

\begin{proposition} \label{proposition:sliderc}
Let $\mathbf{r}$ be a reduced word for a permutation $w$. Then $\mathfrak{F}(\mathbf{r})\neq 0$ if and only if there exists an RC graph $R\in \RC(w)$ with $\mathrm{word}(R)=\mathbf{r}$. In that case, for any such $R$ we have the formula
$$\mathfrak{F}(\mathbf{r}) = \mathfrak{F}_{\wtt(\QY(R))} = \sum_{\substack{A\in\RC(w)\\\mathrm{word}(A)=\mathbf{r}}}x^{\wtt(A)}.$$
\end{proposition}
...
\begin{definition}[Injectification, $\equiv_\forest$] \label{definition:forestequivrc}
...
Let $\mathbf{r}_1$ and $\mathbf{r}_2$ be reduced words. Then we say that $\mathbf{r}_1\equiv_\forest\mathbf{r}_2$ if $P(\overline{\mathbf{r}_1}) = P(\overline{\mathbf{r}_2})$. Naturally, we may apply this to RC graphs as well: for RC graphs $R_1$ and $R_2$, we say that $R_1\equiv_\forest R_2$ if 
$$\overline{\mathrm{revword}(R_1)}\equiv_\forest \overline{\mathrm{revword}(R_2)},$$ 
where $\mathrm{revword}(R)$ is the reverse of $\mathrm{word}(R)$. For a permutation $w$, we denote by $\mathcal{C}_\forest(w)$ the set of equivalence classes of reduced words for $w$ under $\equiv_\forest$. For an RC graph $R$ we write $\code_\forest(R)$ (also written $\fcode(R)$) for the code of the indexed forest $F(\overline{\mathrm{revword}(R)})$.
\end{definition}

\begin{remark}\label{remark:forestperm}
The reversal of the word in Definition \ref{definition:forestequivrc} is necessary to maintain correctness, as several other conventions would need to be changed to match if we simply used $\overline{\mathrm{word}}$. Note that forest equivalence refines the partition of RC graphs by permutation: if $R_1\equiv_\forest R_2$ then $\wof{R_1}=\wof{R_2}$.
\end{remark}
...
\end{theorem}
\begin{proof}
For any equivalence class of words $W$ that have the same $P$-LBS, $\forest_{\code_\forest(W)}(x)$ is the sum of the compatible sequences on the words in $W$ (\cite[Proposition~5.10]{nadeau2024forest}), which are simply the RC graphs in the corresponding equivalence class of RC graphs by Proposition \ref{proposition:sliderc}.
\end{proof}
...
\begin{definition}[Forest commutation]
For an LBS $P$, assume the labels of $P$ are of the form $i^[j]$ with $i,j\geq 1$. Define $\mathrm{rl}(P)$ to be the set of labels of the roots of $P$ (that is, roots of trees in the disjoint union) in $\overline{\mathbb{Z}}$ together with $i^[0]$ if $i, i-1\notin \mathrm{Supp}(P)$.

Let $\mathbf{r}=r_1\cdots r_n$ be a reduced word of some permutation $w$. Let $1\leq p<n$ be an index. Let 
$$\overline{r}=\overline{r_1}\cdots \overline{r_n}$$
Define 
$$\mathrm{rl}(\mathbf{r},p) = \mathrm{rl}(P(\overline{r_{n}}\ \overline{r_{n-1}}\ \cdots \ \overline{r_{p+2}}))$$

Two reduced words $\mathbf{r}$, $\mathbf{s}$ are \emph{forest commutation equivalent} if one is obtained from the other by a sequence of interchanges of adjacent letters at an index $p$ such that 
$$\#\{\overline{a}\in\mathrm{rl}(\mathbf{r},p)\mid \overline{r_p}<\overline{a}<\overline{r_{p+1}}\}\geq 2.$$
In that case we write 
$$\mathbf{r}\sim_\forest\mathbf{s}$$
and extend this relation reflexively and transitively to an equivalence relation. While apparently asymmetric, symmetry is obtained by swapping $\mathbf{r}$ and $\mathbf{s}$ if necessary.
\end{definition}

\begin{proposition} \label{proposition:forestcommute}
Let $R,S\in \RC_n(w)$ be bounded RC graphs of the same height. Then $R\equiv_\forest S$ if and only if $\mathrm{word}(R)\sim_\forest \mathrm{word}(S)$.
\end{proposition}
\begin{proof}
At the word level, this is \cite[Proposition~5.8]{nadeau2023permutahedral}, which transfers directly to $\equiv_\forest$ since the equivalence relation is defined entirely at the word level without regard to the weight.
\end{proof}


\begin{lemma} \label{lemma:littlebumprecurse}
  Let $a_1\cdots a_n$ be an admissible reduced word for a permutation $w$ with $\maxd(w)=m$ and suppose $r$ is an index such that $\rtt_r(a_1\cdots a_n)=(m, m+1)$ (that is, the right root of the word $a_1\cdots a_n$ at position $r$ is the simple root $\alpha_m$). If the initial step of a Little bump at position $r$ does not result in a reduced word, then there is a unique index $p<r$ such that
  $$\ltb{(a_1\cdots a_n)}{r} = \ltb{(a_1\cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_n$$
\end{lemma}
\begin{proof}
  By Proposition \ref{proposition:little_bump}, the Little bump at position $r$ decrements $a_r$ to $a_r - 1$ and any subsequent bumps (rectification in the insertion algorithm) occur strictly to the left.
\end{proof}


\begin{lemma}[Lifting a single swap]\label{lemma:forestsimple}
% Suppose $R,S\in \RC_n(w)^0$ for some permutation $w$ with $\maxd(w)=n$. Suppose also that $\mathrm{word}(R)\sim_\forest \mathrm{swap}_q(\mathrm{word}(R))$ for some $1\leq q<n$. If 
% $$\mathrm{word}(\lmap(R)) = \ltb{\mathrm{word}(R)}{r}$$
% and $q\notin \{r,r-1\}$, then
% $$\ltb{\mathrm{word}(R)}{r}\sim_\forest \ltb{\mathrm{swap}_q(\mathrm{word}(R))}{r}$$
Suppose $R,S\in \RC_n(w)^0$ are two distinct RC graphs for some permutation $w$ with $\maxd(w)=n$. Suppose also that $R\equiv_\forest S$. If there is a permutation $w'$ such that
$$\lmap(R),\lmap(S)\in \RC_{n+1}(w')$$
% $$\mathrm{word}(\lmap(R)) = \ltb{\mathrm{word}(R)}{r}$$
% and $q\notin \{r,r-1\}$, then
% $$\ltb{\mathrm{word}(R)}{r}\sim_\forest \ltb{\mathrm{swap}_q(\mathrm{word}(R))}{r}$$
then $\lmap(R)\equiv_\forest \lmap(S)$.
\end{lemma}
\begin{proof}
  % By Proposition \ref{proposition:forestcommute}, we have that $\mathrm{word}(R)\sim_\forest \mathrm{word}(S)$. We prove that
  % $$\mathrm{word}(\lmap(R))\sim_\forest \mathrm{word}(\lmap(S))$$
We first address the special case where $\mathrm{word}(R)$ and $\mathrm{word}(S)$ differ by a single swap. Suppose $\mathrm{word}(R) = a_1\cdots a_N$ and $\rtt_r(a_1\cdots a_N) = \alpha_n$ for some $r$. Let $q$ be an index such that there are distinct $\overline{a},\overline{b}\in\mathrm{rl}(\mathrm{word}(R),q)$ satisfying $\overline{a_q}<a^{[b]}<c^{[d]}<\overline{a_{q+1}}$. Without loss of generality, we may assume that $q=1$ by \cite[Lemma~5.9]{nadeau2023permutahedral}. 

Using Lemma \ref{lemma:littlebumprecurse}, write
$$\ltb{(a_1\cdots a_N)}{r} = \ltb{(a_1\cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_N$$
for some $p<r$. Inducting on $r$, consider the case where $r=2$. Necessarily, $|a_1 - a_2|\geq 2$, since otherwise no such commutation move would exist. If $a_2 < a_1$, the same elements of $\mathrm{rl}(\mathrm{word}(R),q)$ (and possibly more) remain between $\overline{a_1}$ and $\overline{a_2-1}$, so the swap is still valid. If $a_2 > a_1$, the difficulty would be when $a_2 = a_1 + 2$. However, this does not satisfy the hypotheses of the lemma, since there is no $w'$ satisfying $\lmap(R),\lmap(S)\in \RC_{n+1}(w')$, as the two arrangements of the bumped letters yield different permutations.


Now we address the case where $q<r-1$. Observe by Lemma \ref{lemma:littlebumprecurse} that
$$\mathrm{word}(\lmap(R)) = \ltb{(a_1a_2\cdots \cdots a_{r-1})}{p}(a_r - 1)a_{r+1}\cdots a_n$$
By \cite[Lemma~5.9]{nadeau2024forest}, stripping off a suffix does not make a valid swap invalid. Thus,
$$a_1a_2\cdots a_{r-1}\sim_\forest a_2a_1\cdots \cdots a_{r-1}$$
There are RC graphs $R',S'$ that have these respective words, and $R'\equiv_\forest S'$ by Proposition \ref{proposition:forestcommute}. It is possible, however, that the Little bump at position $p$ is not on the correct letter to achieve $\lmap(R')$ and $\lmap(S')$, thwarting a direct induction. If that is the case, we use the reduction technique of Lemma \ref{lemma:descentchop}; that is, let $(i,j)$ be the coordinates corresponding to the index $p$ in the word. Then, setting
$$U=\mathrm{word}(\{(a,b)\in R\mid a\leq i\text{ and }b> j\}$$
we have that
  $$\ltb{\mathrm{word}(R^{\leq i})}{p'} = \lmap(R^{\leq(i, j)}))\cup  U$$
for some corresponding $p'$ at the same coordinates, but potentially different position in $\mathrm{word}(R^{\leq i})$. If $U$ is empty, direct induction applies. We may also apply induction if both letters of the swap are in $U$ or both letters of the swap are in $R^{\leq(i, j)})$. The remaining case is when one letter of the swap is in $U$ and the other is in $\lmap(R^{\leq(i, j)})$. 

Getting into the details, assume $(1,a_1)\in R^{\leq(i, j)})$ and $(2,a_2-1)\in U$ are the two letters of the swap. We note that either $a_2 - 1 < a_1 - 2$ or $a_2 - 1 > a_1$.


% Then, by the definition of $\mathrm{rl}$, there are at least two elements of $\mathrm{rl}(\mathrm{word}(R),1)$ between $a_1$ and $a_2$. If both of these elements are in $U$, then the swap is still valid after bumping, since the same elements remain between $a_1$ and $a_2-1$. If both of these elements are in $\lmap(R^{\leq(i, j)})$, then the swap is still valid after bumping, since the same elements remain between $a_1$ and $a_2-1$. The remaining case is when one element is in $U$ and one element is in $\lmap(R^{\leq(i, j)})$. In this case, we have that $|a_1 - a_2| = 2$, which does not satisfy the hypotheses of the lemma, as there is no $w'$ satisfying $\lmap(R),\lmap(S)\in \RC_{n+1}(w')$, as the two arrangements of the bumped letters yield different permutations.
\end{proof}

% \todo{GAP: the monotonicity \eqref{eq:sepmono} is asserted, not proved. It does \emph{not} follow from the prefix-invariance of \cite[Lemma~5.9]{nadeau2024forest}, which passes from $U$ to a subword of $U$ and so runs in the direction that weakens the hypothesis; here $U'$ is not a subword of $U$ but its pre-image under a bump, of the same length with letters raised by $0$ or $1$.

% In proving \eqref{eq:sepmono} one may assume $p=1$, that is, that the interchanged letters are the first two letters of the word. Indeed the criterion of \cite[Proposition~5.8]{nadeau2024forest} involves only the segment inserted before the pair, the segment inserted after it playing no role; under the reversal that segment is the one lying to the \emph{left} of the pair in the convention of Definition \ref{definition:rcgraph}, namely $V$, which may therefore be deleted.

% With that reduction the statement to be proved is the following, and nothing more. Let $a'b'U'$ be a reduced word carried by a single Little bump at the last descent to $abU$, with $|a-b|\geq 2$ and $|a'-b'|\geq 2$. If $a,b$ are separated by at least two elements of $\mathrm{rl}(\overleftarrow{U})$, then $a',b'$ are separated by at least two elements of $\mathrm{rl}(\overleftarrow{U'})$. Three further restrictions may be carried along. First, $a'-a$ and $b'-b$ lie in $\{0,1\}$, since a bump decrements each letter at most once. Second, they are never both $1$, by Lemma \ref{lemma:bumpadjacent}; at $N\leq 6$ the pair $(a'-a,b'-b)$ is $(0,0)$ in $254$ of the $542$ instances and each of $(0,1)$ and $(1,0)$ in $144$. Third, when $U'=U$ and $(a',b')=(a,b)$ there is nothing to prove, which disposes of a further $88$ instances; so the residue consists of $166$ instances in which only the segment changes and $288$ in which exactly one endpoint moves.

% Nor does it suffice that $a'$ and $b'$ commute. Commutation is exactly what \cite[Proposition~5.8]{nadeau2024forest} strengthens: at $N\leq 6$, of the $654$ adjacent commutations arising in the nontrivial case $\maxd(w)=m+1$, only $542$ are forest equivalences, the remaining $112$ commuting without being $\Omega$-moves.

% Two natural strengthenings of \eqref{eq:sepmono} are false, and the monotone form is stated because it survives both. First, the rootlist itself is not preserved even when the segment $U$ is unchanged as a sequence of values, because injectification is global: for $W_1=(4,2,5,4)$ and $\mathrm{word}(R_1)=(3,2,5,4)$ with $p=1$, the segment $(5,4)$ is untouched, yet the letter $4$ in it is tagged $4^{[2]}$ above and $4^{[1]}$ below, since the leading letter changes value. Second, the perturbation of $\mathrm{rl}$ caused by a bump is not confined to a bounded window around the decremented values: at $N\leq 6$ there are $388$ single bumps in which the change reaches beyond the intervals $[v-1,v+1]$ of the decremented values $v$, the smallest being $(5,4,5)\mapsto(5,3,4)$, where the untagged letter $7$ changes status. Individual separators are correspondingly not preserved: among the instances in which $U$ is untouched, $72$ lose one of the two original separators and acquire a different one in its place.

% The monotone statement \eqref{eq:sepmono} is verified at $N\leq 6$ over all $542$ instances in which the images are forest equivalent, the separator count going from $2$ to $2$ in $278$ cases, from $2$ to $3$ in $258$, and from $3$ to $4$ in $6$, and never decreasing. Consistently, in all $542$ the two words upstairs are again forest equivalent. The criterion of \cite[Proposition~5.8]{nadeau2024forest}, applied with the reversal described above, reproduces the forest invariant as implemented in \texttt{schubmult} exactly, in all $4412$ adjacent transpositions tested; the reversal is not optional, the criterion applied to the unreversed word disagreeing in $348$ of $604$ cases already at $N=5$.}

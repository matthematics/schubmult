...
Two reduced words are \emph{commutation equivalent} if one is obtained from the other by a sequence of interchanges of adjacent letters differing by at least $2$. A single such interchange is \emph{bare} if it is not part of a Coxeter--Knuth move, that is, if neither of the two letters adjacent to the interchanged pair lies strictly between them in value. Commutation classes are \emph{not} preserved by bumping: a bump decrements a letter, and if the decrement brings two commuting letters to distance $1$ the two runs cease to agree and may end at different permutations. Equality of the terminal permutations is exactly the certificate that no such divergence has occurred.
...
$$\mathbf{w}_j=\mathbf{w}_j^{(0)},\ \mathbf{w}_j^{(1)},\ \ldots,\ \mathbf{w}_j^{(a_j)}$$
be the successive words produced by the iteration of Definition \ref{definition:zeromap}, each obtained from the previous one by the Little bump at the last descent. Suppose either
\begin{enumerate}
...
\end{lemma}
\begin{proof}[Proof of {\rm(1)}]
It suffices to treat a single Coxeter--Knuth move $\alpha$, the general case following by composing along a chain, and then to treat a single bump, the general case following by induction. Since $\mathbf{w}$ and $\mathbf{w}\alpha$ are reduced words for the same permutation, the last inversion of that permutation, and hence the letter at which the bump is begun, is the same for both. By \cite[Lemma~3.3]{hamaker2014relating} that bump commutes with $\alpha$, so the two bumped words again differ by a Coxeter--Knuth move and in particular are reduced words for a common permutation. The stopping criterion of Definition \ref{definition:zeromap} depends only on the permutation, so the two iterations terminate together.
\end{proof}

\todo{GAP: part (2) of Lemma \ref{lemma:lockstep} is stated but not proved; part (1) is proved above. By \cite[Lemma~3.3]{hamaker2014relating} the only commutations not covered by part (1) are the bare ones, and these are exactly the ones for which the terminal permutations can differ.

Verified computationally for $N\leq 6$ over pairs of bounded RC graphs of a common height. For a single braid move the conclusion holds in all $260$ instances in the nontrivial case $\maxd(\wof{B})=\hr(B)$ and all $385$ in the trivial case, and the terminal permutations agree in every one of them, as part (1) predicts. Of the $654$ single commutation moves in the nontrivial case, $442$ carry a Coxeter--Knuth witness, and for these the terminal permutations again agree without exception; the remaining $212$ are bare, and there the terminal permutations agree in $49$ cases and differ in $163$, so the hypothesis in part (2) is necessary. For commutation equivalence in the general, multi-step form, the conclusion holds in all $5435$ instances in the nontrivial case and all $45162$ in the trivial case.}
...
Write $\mathrm{word}(R_2)$ as $\mathrm{word}(R_1)$ with the entries $a,b$ in positions $p$ and $p+1$ interchanged, and let $a',b'$ be the entries of $W_1:=\mathrm{word}(B_1)$ in those same positions. Since $\mathrm{word}(R_1)$ and $\mathrm{word}(R_2)$ are reduced words for the common permutation $\wof{R_1}$ differing only in the order of the adjacent letters $a,b$, we have $s_as_b=s_bs_a$, and $a\neq b$ because interchanging equal letters would give $R_1=R_2$; hence
...
By \cite[Lemma~3.3]{hamaker2014relating}, a bump applied to these three letters commutes with a Coxeter-Knuth move (perhaps resulting in a Coxeter-Knuth move of a different type), so that these three letters should differ by a Coxeter-Knuth move in the end result. In the case where $\mathrm{word}(B_1)$ contains the $b'ab'$, by assumption the right $b'$ becomes $b$, so that the result is $b'ab$, which is indeed Coxeter-Knuth equivalent to $b'ba$. However, in that case we have that the $ab'a$ in $\mathrm{word}(B_2)$ is bumped to $aba$, which is not Coxeter-Knuth equivalent to $b'ba$. Hence, the configuration \eqref{eq:forestsimple_bad} is impossible, since it is implied by Lemma \ref{lemma:lockstep}(1) that the ending permutations must not be the same, which contradicts the hypothesis that $R_1\equiv_\forest R_2$.


Since we have established \eqref{eq:forestsimple_positions}, the proof runs as follows. Let $W_2$ be $W_1$ with the entries in positions $p,p+1$ interchanged. Since $s_{a'}$ and $s_{b'}$ commute, $W_2$ is again a reduced word for $w$, commutation equivalent to $W_1$.

A Little bump is equivariant under the interchange of two adjacent commuting letters. Indeed, by Definition \ref{definition:littlebump} a bump is begun at the letter effecting the last inversion of the permutation, and each subsequent step is applied to the unique letter whose deletion leaves the current word reduced; both are conditions on the letter rather than on its position, and neither is affected by the order in which two adjacent commuting letters are listed. Decrementing a letter likewise does not depend on its position. Consequently the runs on $W_1$ and on $W_2$ decrement corresponding letters at every step, and by Lemma \ref{lemma:lockstep}(2), applicable because $W_1$ and $W_2$ have the same terminal permutation, they proceed with a common permutation at every stage and terminate together. Hence
$$W_2\!\uparrow\cdots\uparrow\;=\;\bigl(W_1\!\uparrow\cdots\uparrow\bigr)\ \text{with positions } p,p+1 \text{ interchanged}\;=\;\mathrm{word}(R_2).$$
On the other hand $\mathrm{word}(B_2)\!\uparrow\cdots\uparrow=\mathrm{word}(R_2)$ as well, and the word-level map is injective (Theorem \ref{theorem:zero_bijection}), so $\mathrm{word}(B_2)=W_2$. Thus $\mathrm{word}(B_2)$ is $\mathrm{word}(B_1)$ with the entries in positions $p,p+1$ interchanged.

It remains to see that these two words are forest equivalent. By hypothesis $\mathrm{word}(R_1)=UabV$ and $\mathrm{word}(R_2)=UbaV$ with $a,b$ separated by at least two elements of the rootlist $\mathrm{rl}(U)$ \cite[Proposition~5.8]{nadeau2024forest}. Since a Little bump alters each letter of a word by at most one, and by \eqref{eq:forestsimple_positions} the letters $a',b'$ of $W_1$ at positions $p,p+1$ again commute, the same separation criterion applies upstairs, whence $B_1\equiv_\forest B_2$.
\end{proof}

\todo{GAP: the proof is complete except for the last paragraph, where the transport of the rootlist separation criterion upstairs is asserted rather than proved.

Computations at $N\leq 6$ confirm the reduction. Over all pairs $B_1,B_2\in\RC_{m+1}(w)^0$ whose images under $\zeromap$ differ by an adjacent transposition at positions $p,p+1$, in the nontrivial case $\maxd(w)=m+1$, the conclusion holds in all $542$ instances in which the images are forest equivalent. Of these, $216$ have $|a-b|\geq 3$ and so are settled by the crude bound alone, while $326$ have $|a-b|=2$ and require the analysis of \eqref{eq:forestsimple_bad}. More sharply, among all $4412$ instances the conclusion holds in exactly those for which $|a'-b'|\geq 2$ and fails in exactly those for which $|a'-b'|=1$ ($2044$ instances, every one of them in the configuration \eqref{eq:forestsimple_bad} with $|a-b|=2$); so \eqref{eq:forestsimple_positions} is not merely sufficient but equivalent to the conclusion, and the equivariance argument is the whole content of the lemma.

One natural shortcut is ruled out. Coxeter--Knuth equivalence of the images is neither necessary nor sufficient for the conclusion: among the $542$ forest-equivalent instances, $246$ have distinct Edelman--Greene $P$-tableaux, while $1944$ of the failures have equal ones.}

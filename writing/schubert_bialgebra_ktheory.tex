...
\begin{definition}
A \emph{WC graph} is simply a finite subset $W\subseteq \mathbb{N}\times \mathbb{N}$ with no additional restrictions. We define the \emph{weight} of a WC graph $W$ to be the sequence $\wtt(W) = (i_1,\ldots,i_n)$, where $i_j$ is the number of elements of $W$ in row $j$. We define the \emph{permutation} of a WC graph $W$ to be the permutation

...
\subsection{Equivariant Grassmannian RC graph ring}
We adjoin variables to $\mathcal{M}_n$ to obtain $\mathcal{M}_n[\mathbf{t}]$. 

\begin{proposition}
There exists a unique $\mathbb{Z}[\mathbf{t}]$-basis of $\grass_n[\mathbf{t}]$
$$\{D_T(\mathbf{t})\mid T\in\mathrm{SSYT}_n\}$$
such that for each single-box $m\in \mathrm{SSYT}_n$ we have the formula
$$D_T(\mathbf{t})\cdot (m -t_{n + 1 - m + \lambda_m})= D_{T\boxtimes m}(\mathbf{t})$$
where $\lambda=\mathrm{shape}(T)$.
\end{proposition}
% \begin{proof}
%   Suppose $T\in\grass_n$ and $r_1<r_2<r_3$ are single-box Grassmannian RCs. We claim that
%   $$T(r_2-t_{\wof{T}(r_2)})(r_1-t_{\wof{Tr_2}(r_1)})(r_3-t_{\wof{Tr_2r_1}(r_3)})=T(r_2-t_{\wof{T}(r_2)})(r_3-t_{\wof{Tr_2}(r_3)})(r_1-t_{\wof{Tr_2r_3}(r_1)})$$

% \end{proof}

% Let $M\in\BRC_k$ be an RC graph with a single crossing for the permutation $s_k$. Let $R\in \BRC_k[t]$. Then we define
% $$R\cdot M = t_{\wof{R}(\wtt(M))}R + R\boxtimes M$$

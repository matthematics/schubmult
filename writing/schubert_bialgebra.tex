...
\begin{abstract}
The forest polynomials $\forest_a$ of Nadeau--Tewari form a $\mathbb{Z}$-basis of $\mathbb{Z}[x_1,x_2,\ldots]$ that plays the role for the cohomology of the quasisymmetric flag variety that Schubert polynomials play for the classical flag variety. Nonnegativity of the structure constants $\beta_{a,b}^c$ in $\forest_a\,\forest_b = \sum_c \beta_{a,b}^c\,\forest_c$ was established by Nadeau--Tewari via the forest equivalence class structure on monomials, with a later constructive straightening proof by Bergeron--Gagnon--Nadeau--Spink--Tewari, but no Littlewood--Richardson-style enumerative rule has been available. We give such a rule: $\beta_{a,b}^c$ counts pairs of forest RC graphs of forest-codes $a$ and $b$ whose lift product lands on a forest RC graph of forest-code and weight both equal to $c$. The same rule descends to the cup product on $H^\bullet(\mathrm{QFl}_n)$.

The proof develops a unifying ring-theoretic framework that we believe is of independent interest. We introduce the \emph{Schubert bialgebra} $\coma$, a graded direct sum of polynomial rings with the Schubert basis as structure-constant carrier, and lift the multiplication on its graded dual $\dcoma$ to a product on a free abelian group $\BRC$ of bounded RC graphs. As consequences of the same machinery we obtain enumerative Littlewood--Richardson rules for the dual Schubert, dual key, dual forest, and dual slide bases of $\dcoma$, each realizing its structure constants as cardinalities of explicit sets of RC graphs.
\end{abstract}
...
Here $A*B$ is the \emph{lift product} (\S\ref{section:liftproduct}), a binary operation on RC graphs introduced in this paper for the purpose.
...
The proof requires substantial machinery, which we develop in its own right because it is of independent interest: a Schubert bialgebra $\coma$ with a graded dual $\dcoma$ whose product captures branching of Schubert polynomials; an associative product $\sqcupplus$ on the free abelian group $\BRC$ of bounded RC graphs that realizes $\dcoma$ as a homomorphic image; the squash product $\boxtimes$ and the lift product $*$ on $\BRC_n$; a multiplactic algebra $\mathcal{M}_n$ of elementary RC factors; a Cauchy-type factorization through a slide quotient; and an application of the quasi-crystal framework of Cain--Guilherme--Malheiro \cite{cain2023quasicrystals} to RC graphs. 

% Along the way, the same apparatus produces four further Littlewood--Richardson rules of independent interest --- for the dual Schubert, dual key, dual forest, and dual slide bases of $\dcoma$ --- which we record as Theorems~\ref{theorem:LR}, \ref{theorem:LRkey}, \ref{theorem:LRforest}, and \ref{theorem:lrfslide} below. The dual forest rule is itself the keystone bridge to the forest polynomial rule above. These dual rules form a coherent family of branching/splitting formulas for Schubert, key, forest, and slide polynomials under the partition of variables into two blocks $\{x_1,\ldots,x_p\}$ and $\{x_{p+1},\ldots,x_{p+q}\}$.
...
where $a$ and $b$ are weak compositions of lengths $p$ and $q$ respectively, and the equation $a+b=c$ indicates that the pointwise sum of $a$ and $b$ is equal to $c$. The counit is given by $\varepsilon(c)=0$ unless $c$ is the empty composition, in which case $\varepsilon(c)=1$. With these definitions, $\coma$ and $\dcoma$ are dual bialgebras over $\mathbb{Z}$, a fact that is straightforward to verify. They fail to be connected and are not Hopf algebras. A ``Littlewood-Richardson rule'' for a basis of the dual algebra is equivalent to a branching/splitting formula for the corresponding polynomial basis under the partition of variables into two blocks $\{x_1,\ldots,x_p\}$ and $\{x_{p+1},\ldots,x_{p+q}\}$.

Combinatorially, $\dcoma$ is interesting, but not fundamentally new at this point. What is new is the interactions of the algebraic structure with the dual Schubert basis $\dsch_u^n$ of $\dcoma$ indexed by $u\in S_\infty$ and $n$ at least as large as the last descent of $u$, defined by
...
For the dual Schubert basis by $\dsch_u^n$ for permutations $u$, we have the following result.
...
The dual forest basis has nonnegative structure constants for the same structural reason. The coefficient $f_{a,b}^c$ in $\forest_a^*\sqcupplus\forest_b^*=\sum_c f_{a,b}^c\,\forest_c^*$ counts, within the forest equivalence class of composition $c$, the RC graphs whose row-splitting lands in forest-classes of compositions $a$ and $b$; the count is well-defined on equivalence classes and is nonneg\-ative (Theorem~\ref{theorem:LRforest}).
...
\begin{lemma}[Functoriality of $\clp$] \label{lemma:clpfunc}
...
The only subtlety is that each invocation of $\zeromap$ requires the bottom row to be empty (Definition \ref{definition:zeromap}); when this fails, $\zeromap$ is preceded internally by Algorithm \ref{algorithm:zero}, which strips the last row using a reduced-word manipulation that depends only on the last row and rows immediately above it. The output of this stripping is independent of the rows lying strictly above the rows being processed, so the stripping performed at stage $i$ in computing $\clp^p(R)$ directly is identical to the stripping performed at stage $i$ in computing $\clp^p(\clp^{p+q}(R))$ for $i\leq r$, and the same iterative computation continues identically thereafter.
\end{proof}

...
\begin{abstract}
We introduce the ``Schubert bialgebra,'' a ring-theoretic direct sum of polynomial rings, whose Schubert basis recovers the classical structure constants $c_{u,v}^w$, and lift the product structure on the graded dual to a product structure on RC graphs. We present Littlewood-Richardson-style rules for the dual bases of each of the Schubert polynomials, key polynomials, slide polynomials, and forest polynomials that count certain RC graphs. We also give a positive, manifestly combinatorial Littlewood-Richardson rule for the structure constants of forest polynomials (also in terms of RC graphs), and in turn the cohomology ring of the quasisymmetric flag variety.
\end{abstract}
...
where $a$ is any composition. We present an explicit formula for the dual Schubert basis in $\dcoma$ with integer coefficients. These are extremely similar to the dual Schubert polynomials of Postnikov and Stanley defined in \cite{postnikov2009chains}, and in fact seem to be related by a scalar factor and a basis transition according to our empirical tests, which were suggested by Hamaker in connection to \cite{hamaker2023dual}. The dual Schubert polynomials of Stanley and Postnikov however do not naturally possess the multiplicative structure of $\dcoma$, but do come with an integral formula that mimics the action of divided difference operators on Schubert polynomials. In principle these structures should be transferrable to our dual Schubert basis, but we have not yet explored this.
...
The centerpiece of the paper is, in our view, Theorem \ref{theorem:forestpolyLR}: a positive, combinatorial Littlewood--Richardson rule for the product of two forest polynomials \emph{themselves}, not merely for the dual basis. Concretely, the structure constants $\beta_{ab}^c$ in
$$\forest_a\,\forest_b = \sum_c \beta_{ab}^c\,\forest_c$$
are cardinalities of explicit sets of pairs of RC graphs: $\beta_{ab}^c$ counts pairs $(A,B)$ of \emph{forest} RC graphs of forest-code $a$ and $b$ whose lift product $A*B$ lands on some forest RC graph $R$ with $\code_\forest(R)=\wtt(R)=c$. The proof requires substantially more machinery than the dual rules: we introduce the \emph{squash product} $\boxtimes$ and an auxiliary \emph{lift product} $*$ on $\BRC_n$ that lives in a multiplactic algebra $\mathcal{T}_n$ of elementary RC factors, exploit a Cauchy-type factorization through the slide quotient of \S\ref{section:slide}, and combine all of this with the forest equivalence of \cite{nadeau2024forest}.

We wish to emphasize how unusual this is. Even for Schubert polynomials themselves, no positive combinatorial formula for the structure constants is known in full generality. The forest polynomials are a basis of $\coma$ that interpolates between key polynomials and Schubert polynomials, and for this basis Theorem \ref{theorem:forestpolyLR} provides a complete, manifestly positive answer. We are unaware of any prior result giving a positive combinatorial Littlewood--Richardson rule for any natural Schubert-like polynomial basis of $\coma$ (other than Schur polynomials themselves) at this level of generality.
...
$\mathcal{T}_n$ & The multiplactic algebra of full Grassmannian RC graphs.\\
...
\[
  (R_1', R_2', \wof{R}),
\]
since $R_2'=\clp^q(P_2)$. These represent the same RC graph $\clp^{p+q}(R)$. It follows from Lemma \ref{lemma:injap} that $R_1=R_1'$ and $R_2=R_2'$. We may conclude that if we know the RC graphs $R_1, R_2, R_3$, then the RC graphs that appear as terms in the product $(R_1\diamond R_2)\diamond R_3 = R_1\diamond(R_2\diamond R_3)$ do not depend on the bracketing.
...
To read off $b_{\mu,\nu}^\lambda$, fix any $C$ with $\lambda([C])=\lambda$ and let $S_1=\mathrm{rep}(R_1)$, $S_2=\mathrm{rep}(R_2)$ be the chosen representatives of the $\mu$- and $\nu$-classes (for any $R_1,R_2$ in those classes). By Theorem~\ref{theorem:modactrc} the product $\diamond$ on $\BRC$ admits a $(\clp^p,\trm^p)$-decomposition: for any $S_1,S_2$ with $\hr(S_1)=p$, $\hr(S_2)=q$, the set $\{R\in\RC_{p+q}:\clp^p(R)=S_1,\,\trm^p(R)=S_2\}$ enumerates the summands of $S_1\diamond S_2$ with multiplicity one. Projecting to $\BRC/I_\sim$, (H3) ensures that the class of $R$ depends only on the classes of $\clp^p(R)$ and $\trm^p(R)$, so the contribution of $S_1\cdot S_2$ to the coefficient of $[C]$ in $E_\lambda$ is exactly the count of $R\sim C$ for which $\clp^p(R)=S_1$ and $\trm^p(R)=S_2$, i.e.\ for which $\clp^p(R)$ and $\trm^p(R)$ are themselves fixed points of $\mathrm{rep}$ and represent the $\mu$- and $\nu$-classes. This is the stated formula. Being a cardinality, it is nonnegative.
...
where the product is the lift product $*$ on $\BRC_n$ transferred to $\mathcal{T}_n$ via $\mathrm{lift}_n$. Equivalently, given any RC graph $R$ admitting a lift-factorization $R=R^{\mathrm{left}}*R^{\mathrm{right}}$ with $\hr(R^{\mathrm{left}})\leq k<\hr(R^{\mathrm{right}})$, the factors $R^{\mathrm{left}}$ and $R^{\mathrm{right}}$ commute under $*$ with any analogous factors arising from other RC graphs.
\end{lemma}
\begin{proof}
...
\begin{remark}
To our knowledge, Corollary~\ref{corollary:cupforest} provides the first positive combinatorial formula for the cup product structure constants of $H^\bullet(\mathrm{QFl}_n)$ in any natural geometric basis. The corresponding question for $H^\bullet(\mathrm{Fl}_n)$ in the Schubert basis is the classical (and open) problem of finding a manifestly positive combinatorial rule for Schubert structure constants. The presence of such a rule for the quasisymmetric flag variety $\mathrm{QFl}_n$, despite the absence of one for $\mathrm{Fl}_n$, reflects the special combinatorial rigidity of the forest equivalence relation $\equiv_\forest$ on RC graphs.
\end{remark}
...
We hope that Theorem \ref{theorem:forestpolyLR} will be of independent interest: it gives a positive combinatorial formula for the structure constants $\beta_{ab}^c$ of the forest polynomials themselves, not their duals. We should be careful here about exactly what is and is not quotiented: the lift product $*$ on the multiplactic algebra $\mathcal{T}$ descends to an associative product on a composition-indexed compatible-sequence slide polynomial ring, but it does \emph{not} descend associatively along the map sending an RC graph to its $Q$-LBS in the forest case, nor to the RC graph itself. In other words, the forest invariant or the reduced word (retaining the compatible sequence) not a congruence for $*$, and Theorem \ref{theorem:forestpolyLR} cannot be obtained by passing to a quotient on which the forest LR coefficients $\beta_{ab}^c$ are themselves structure constants of an induced product; this is precisely the obstruction that the squash product $\boxtimes$ and the apparatus of \S\ref{section:forestrule} are designed to circumvent. 

...
\end{lemma}
\begin{proof}
We use the same description of $\equiv_\forest$ that underlies the proof of Lemma~\ref{lemma:forestideal}. By \cite[Proposition~5.8 and Lemma~5.9]{nadeau2024forest}, forest equivalence on injectified words is generated by \emph{pairwise commutations} of two adjacent letters $a, b\in\overline{\mathbb{Z}}$ — there are no triple-letter (Knuth-style) generators — and the validity of such a commutation depends only on $\mathrm{val}(a)$, $\mathrm{val}(b)$, and the (forest-related) letters strictly surrounding the pair. These generators enjoy two key invariances:
\begin{itemize}
\item \emph{Prefix-invariance} (\cite[Lemma~5.9]{nadeau2024forest}): a valid commutation of two adjacent letters in a suffix remains valid after prepending any common prefix; equivalently, $u\equiv_\forest u'\Rightarrow w\cdot u\equiv_\forest w\cdot u'$.
\item \emph{Shift-invariance} (\cite[Proposition~5.8]{nadeau2024forest}): commutations are preserved under simultaneous translation of all letters by a fixed integer in the underlying $\mathbb{Z}$-coordinate.
\end{itemize}

We first establish
$$(\ast)\qquad A\equiv_\forest A'\Rightarrow A*B\equiv_\forest A'*B,\qquad B\equiv_\forest B'\Rightarrow A*B\equiv_\forest A*B'.$$
By definition of the lift product, $\overline{\mathrm{word}(A*B)}$ is the concatenation $u_A\cdot u_B$ obtained by reading the multiplactic factorizations of $A$ and $B$ in order, where $u_A = \overline{\mathrm{word}(A)}$ and $u_B$ is a column-shift (in the sense of shift-invariance) of $\overline{\mathrm{word}(B)}$ placing its letters strictly past those of $u_A$.

For the second half of $(\ast)$, prefix-invariance applied with prefix $u_A$ gives $A*B\equiv_\forest A*B'$ directly from $\overline{\mathrm{word}(B)}\equiv_\forest \overline{\mathrm{word}(B')}$ (the column-shift commutes with $\equiv_\forest$ by shift-invariance).

For the first half, we mirror the argument used to establish right-ideal closure in Lemma~\ref{lemma:forestideal}. It suffices to consider a single generating commutation that swaps adjacent letters $a, b$ in $u_A$, exhibiting $\overline{\mathrm{word}(A')}$. We may further reduce, as in that lemma, to the case where $a, b$ are the last two letters of $u_A$. Forming $A*B$ replaces the suffix $\cdots ab$ of $u_A$ with $\cdots ab\cdot u_B$. The strict column-separation of $u_B$ from $u_A$ ensures that no letter of $u_B$ lies in the forest-relatedness window between $a$ and $b$ (which depends only on values close to $\max(\mathrm{val}(a),\mathrm{val}(b))$, while $u_B$'s letters live at strictly larger column-shifted values). Hence the commutation $\cdots ab\cdots\leftrightarrow\cdots ba\cdots$ is still a valid generator of $\equiv_\forest$ acting on the longer word $u_A\cdot u_B$, and produces $\overline{\mathrm{word}(A'*B)}$. This establishes $A*B\equiv_\forest A'*B$.

Iterating $(\ast)$ yields $A'*B'\equiv_\forest A*B$ for every $A'\equiv_\forest A$, $B'\equiv_\forest B$. Now let $R = A'*B'\in S$ and $R\equiv_\forest R'$ by a single generating commutation of letters $a, b$ adjacent in $\overline{\mathrm{word}(R)} = u_{A'}\cdot u_{B'}$. Three cases occur:
\begin{enumerate}
\item Both $a,b$ lie in $u_{A'}$. The commutation produces $u_{A''}\cdot u_{B'}$ with $A''\equiv_\forest A'$, hence $R' = A''*B'\in S$.
\item Both $a,b$ lie in $u_{B'}$. Symmetric: $R' = A'*B''\in S$ with $B''\equiv_\forest B'$.
\item The pair $(a,b)$ straddles the boundary: $a$ is the last letter of $u_{A'}$ and $b$ is the first letter of $u_{B'}$. By the same strict column-separation just noted, the only generating commutations available across the boundary are those whose forest-relatedness condition depends solely on $\mathrm{val}(a),\mathrm{val}(b)$ with no admissible separating letters; such a commutation reorganizes the multiplactic factorization by moving the boundary letter $a$ into the $B$-factor (or equivalently, $b$ into the $A$-factor), which by Lemma~\ref{lemma:liftcommute} yields multiplactic factorizations $A''*B''$ with $A''\equiv_\forest A'$ and $B''\equiv_\forest B'$. Hence $R'\in S$.
\end{enumerate}
A general forest move from $R$ to $R'$ is a finite composition of single commutations, so $R'\in S$ by induction.
\end{proof}
...
The final step is to identify which forest classes $[R]$ contribute and to count $m_{[R]}$ explicitly. Fix a target weak composition $c$ and a representative forest RC graph $R$ with $\code_\forest(R)=\wtt(R)=c$ (i.e.\ a \emph{forest} RC graph with forest code $c$). For any pair $(A',B')$ with $A'\equiv_\forest A$ and $B'\equiv_\forest B$, the lift product $A'*B'$ lies in the forest class of $R$ if and only if $A'*B'\equiv_\forest R$; by Lemma~\ref{lemma:foreststable}, this is equivalent to the existence of some pair $(A'',B'')$ with $A''*B''=R$, $A''\equiv_\forest A$, $B''\equiv_\forest B$. Hence the coefficient $\beta_{ab}^c$ of $\forest_c$ in $\forest_a\forest_b$ equals the number of such pairs $(A'',B'')$. Specializing the representative pair to one in which $A''$ and $B''$ are themselves forest RC graphs (which exists, since the class of $A$ contains the unique forest RC graph with $\code_\forest=\code=a$, and similarly for $B$), the conditions $A''\equiv_\forest A$ and $\code_\forest(A'')=a$ become equivalent to $\code(\wof{A''}) = \code_\forest(A'') = a$, and similarly for $B''$. This is the count claimed in the statement of the theorem.
\end{proof}

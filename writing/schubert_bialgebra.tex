...
\subsection*{Key polynomials and dual keys}

The key polynomials $\kappa_a$ (Demazure characters in type $A$) form a $\mathbb{Z}$-basis of $\mathbb{Z}[x_1,x_2,\ldots]$ indexed by weak compositions; we defer definitions and a survey of the literature to the start of \S\ref{section:crystals}. The dual key basis $\{\kappa_a^*\}$ of $\dcoma$ is defined by $\langle\kappa_a^*,\kappa_b\rangle=\delta_{a,b}$. Linear functionals on $\coma_n$ that pair to $\delta_{a,b}$ against the $\kappa_b$ are implicit in the literature on Demazure characters (see, e.g., Joseph \cite{joseph1985demazure} and Mason \cite{mason2009explicit}); what appears to be new here is the structure of these duals as a basis of a bialgebra, and in particular the product $\sqcupplus$ on $\dcoma$ in the dual key basis. Using the Little-bump description \cite{little2003combinatorial,hamaker2014relating} of $\zeromap$ and the Demazure crystal structure on RC graphs of \cite{assaf2018demazure}, we obtain:
...
Equivalently, Theorem~\ref{theorem:LRkey} is a positive combinatorial splitting rule
$$\kappa_c(x_1,\ldots,x_{p+q}) = \sum_{a,b} k_{a,b}^c\,\kappa_a(x_1,\ldots,x_p)\,\kappa_b(x_{p+1},\ldots,x_{p+q}).$$
Nonnegativity of the $k_{a,b}^c$ is in retrospect a consequence of Mathieu's existence theorem for excellent filtrations on tensor products of Demazure modules \cite{mathieu1990filtrations}; an explicit positive combinatorial rule has not previously appeared.

\subsection*{Forest polynomials and dual forests}

The \emph{forest polynomials} $\forest_a$ of Nadeau and Tewari \cite{nadeau2024forest}, indexed by weak compositions $a$, form a $\mathbb{Z}$-basis of the polynomial ring whose role for the cohomology of the quasisymmetric flag variety \cite{bergeron2025qsymflag} parallels that of Schubert polynomials for $\mathrm{Fl}_n$. Forest polynomials are governed by an equivalence relation $\equiv_\forest$ on words (Nadeau--Tewari forest classes); we collect background and definitions at the start of \S\ref{section:forest}. The dual forest basis $\{\forest_a^*\}$ of $\dcoma$ is defined by $\langle\forest_a^*,\forest_b\rangle=\delta_{a,b}$. As we record in Proposition~\ref{proposition:forestdualcoefficients}, this dual basis coincides --- up to a divided-power normalization --- with the volume polynomials of Nadeau--Spink--Tewari \cite{nadeau2024quasisymmetric} and, geometrically, with the Kronecker dual of the homology basis $\{[X(F)]\}\subset H_\bullet(\mathrm{QFl}_n)$ of Bergeron--Gagnon--Nadeau--Spink--Tewari \cite{bergeron2025qsymflag}. What appears to be new here is the product structure: the concatenation product $\sqcupplus$ on $\dcoma$, restricted to $\{\forest_a^*\}$, has not previously been studied, and our main result for this basis is an explicit positive combinatorial rule for its structure constants.
...
As with the dual key basis, Theorem~\ref{theorem:LRforest} is equivalent to a positive combinatorial splitting rule
$$\forest_c(x_1,\ldots,x_{p+q}) = \sum_{a,b} f_{a,b}^c\,\forest_a(x_1,\ldots,x_p)\,\forest_b(x_{p+1},\ldots,x_{p+q}).$$
This splitting lifts, on cohomology, to a ring homomorphism $\iota_{p,q}^*:H^\bullet(\mathrm{QFl}_{p+q})\to H^\bullet(\mathrm{QFl}_p)\otimes H^\bullet(\mathrm{QFl}_q)$ that is the natural quasisymmetric-flag analogue of the Levi comorphism of \cite{ressayre2009branching}; we develop this in \S\ref{subsection:qflbranch}.

\subsection*{An enumerative Littlewood--Richardson rule for forest polynomials}

The combinatorial centerpiece of the paper is a Schubert-calculus-style enumerative LR rule for the product of two forest polynomials themselves (Theorem~\ref{theorem:forestpolyLR} in \S\ref{section:forestrule}). The structure constants $\beta_{ab}^c$ in
$$\forest_a\,\forest_b = \sum_c \beta_{ab}^c\,\forest_c$$
are realized as cardinalities of explicit sets of pairs of RC graphs: $\beta_{ab}^c$ counts pairs $(A,B)$ of \emph{forest} RC graphs of forest-code $a$ and $b$ whose lift product $A*B$ lands on some forest RC graph $R$ with $\code_\forest(R)=\wtt(R)=c$. Nonnegativity of the $\beta_{ab}^c$ is already known constructively, by the straightening algorithm of Bergeron--Gagnon--Nadeau--Spink--Tewari \cite{bergeron2025equivariant}; what is new in Theorem~\ref{theorem:forestpolyLR} is the realization of each coefficient as a single cardinality of an explicit set, in the spirit of the classical Littlewood--Richardson rule for Schur functions. The proof requires substantial extra machinery (the squash product $\boxtimes$, the lift product $*$, a multiplactic algebra $\mathcal{M}_n$, a Cauchy-type factorization through the slide quotient of \S\ref{section:slide}, and the Nadeau--Tewari forest equivalence). Rather than develop the full theory of these constructs, we treat them as means to an end, leaving further development for the future.
...
To tie up the claim that $\BRC$ is a ring, distributivity is directly imposed by the definition of the product as extending from the definition blinearly, and the identity element is the empty RC graph of height $0$.
\end{proof}
...
$$K_a =\sum_{R\in\mathrm{Dem}_a\atop R=\mathbb{Y}(R)} R$$
\end{definition}
...
% Composing the signed expansion with $\RC_n$ defines an auxiliary product $A*B$ on $\BRC_n$, the \emph{lift product} (\S\ref{section:liftproduct}).
...
\begin{definition}
  ...
  We observe by Proposition \ref{proposition:eproductworks} that $\RC_n(\mathbf{S}_w(n)) = \mathcal{S}_w(n)$, so that $\mathbf{S}_w(n)$ is a lift of $\mathcal{S}_w(n)$ to $\mathcal{M}_n$, which usually has signs.
\end{definition}
...
$$\wtt(R_1*R_2) = \wtt(R_1) + \wtt(R_2)$$
componentwise. The ring-homomorphism statement is immediate: $\mathrm{ev}(R_1*R_2) = x^{\wtt(R_1)+\wtt(R_2)} = x^{\wtt(R_1)}\,x^{\wtt(R_2)} = \mathrm{ev}(R_1)\mathrm{ev}(R_2)$.
\end{proof}
...
The purpose of this subsection is to be helpful to those who do computations in this algebra: an algorithm for computing $\mathrm{lift}_n(R)$ for an RC graph $R$ does not present itself as obvious, and we flesh one out here for convenience.


\begin{definition}
Let $p\leq k$ be positive integers and let $w\in S_N$ be a permutation with $\maxd(w)\leq k$. Suppose $R\in \RC_k(w)$ and let $A\in\binom{[k]}{p}$. Define an RC graph $\sigma_k(R,A)$ as follows. Suppose $A=\{a_1<\cdots < a_p\}$ and let the reduced/compatible biword for $R$ be
...
Then apply $\lzeromap_k$ to this biword to obtain $\sigma_k(R,A)$.
\end{definition}

\begin{lemma} \label{lemma:liftpieri}
We have that 
$$\sigma_k:\RC_k(w)\times \binom{[k]}{p}\to \bigcup_{\substack{w\tom{k} w'\\\ell(w,w')=p}}\RC_k(w')$$ 
is an isomorphism of Demazure crystals.
\end{lemma}
\begin{proof}
  This is precisely Proposition \ref{proposition:boxproduct} in the case where the right factor is a full $p$-Grassmannian elementary symmetric polynomial RC graph of height $k$.
\end{proof}

\begin{definition}[Pieri insertion]
...
We state the following without proof, as the proof is straightforward, tedious, and unimportant for the rest of the exposition.
...
\begin{method}[Computing $\mathrm{lift}_n(R)$ for an RC graph $R$]
To find the lift of an RC graph $R\in \RC_n(w)$, suppose $\code(ww_0(n + 1)) = a$. We need to peel off $n+1-a_1$ reflections in an $n$-Pieri manner, yielding an RC graph $R'\in\RC_n(w')$ where $w'\in S_n$ is uniquely determined by $\code(w'w_0(n))=(a_2,\ldots,a_n)$. By Lemma \ref{lemma:liftpieri}, there is precisely one RC graph $R'\in\RC_{n-1}(w')$ and one subset $A\subseteq [n]$ of size $n+1-a_1$ such that $\sigma_{n}(R',A)=R$: Specifically, $A$ can be characterized by the fact that if
$$A=\{A_1<\cdots < A_{n+1-a_1}\}$$
then the rightmost crossings (leftmost letters in the reduced word) in row $A_i$ of $R$ for each $i$ correspond to a sequence of reflections $t_{c_1d_1},\ldots,t_{c_{n+1-a_1}d_{n+1-a_1}}$ such that $w' = w t_{c_1d_1}\cdots t_{c_{n+1-a_1}d_{n+1-a_1}}$, and
$$c_i\leq n<d_i$$
for all $i$.


This subset $A$ tells us precisely which reflections to remove from $R$ to obtain the $n$th tensor component of $\mathrm{lift}_n(R)$. We then repeat this process with $R'$ to obtain the next tensor component, recursively, until we have peeled off all $n$ steps. The result is the unique lift of $R$.
\end{method}
...
% In particular, for every $i\geq 1$, exactly one of the following holds:
% \begin{enumerate}
% \item $\ddot{e}_i$ acts on the $A$-factor: $\ddot{e}_i(A)\neq\perp$ and $\ddot{e}_i(A*B) = \ddot{e}_i(A)*B$;
% \item $\ddot{e}_i$ acts on the $B$-factor: $\ddot{e}_i(B)\neq\perp$ and $\ddot{e}_i(A*B) = A*\ddot{e}_i(B)$;
% \item $\ddot{e}_i(A*B) = \perp$;
% \end{enumerate}
% and likewise for $\ddot{f}_i$.
\end{proposition}
\begin{proof}
...
The map $A\otimes E\mapsto A\sqcup E$ is a Kashiwara crystal isomorphism onto the connected component of $A\sqcup E$ in $\RC_n(\wof{A\sqcup E})$. This is precisely the pairing argument given in the proof of Proposition~\ref{proposition:boxproduct}: the unpaired row-$i$ crossings of $A$ pair against the unpaired row-$(i+1)$ crossings of $E$ at the seam in exactly the order dictated by the standard Kashiwara tensor rule, so $e_i, f_i$ on $A\sqcup E$ act as on $A\otimes E$. (Note that the underlying words \emph{interleave} under $\sqcup$ -- $\wof{A\sqcup E} = \wof{A}\shup^N\wof{E}$ where $N$ is the position of the last unfixed point of $\wof{A}$ -- but this affects only how the seam is identified at the level of the word, not the crystal action.) Postcomposing with the Kashiwara crystal morphism $\clp^{n}$ (Theorem~\ref{theorem:zerocrystal}) gives the Kashiwara crystal isomorphism
...
We now state and prove the LR rule for forest polynomials, which is the main motivation for introducing the lift product.
...
% where the second sum is over all $R$ such that $A'*B'=R$ and $R$ is quasi-Yamanouchi.
% By the previous lemma, the set of such $R$ is stable under forest equivalence, so we may group the sum by forest equivalence classes to obtain
% $$\forest(A)*\forest(B) = \sum_{R} \forest(R)$$
...
\item[$A_p(R)$] The triple $(\clp^p(R),\trm^p(R),\wof{R})$; injective by Lemma~\ref{lemma:injap}.
\end{description}
...
\item[$\rho_{p,q}$, $\iota_{p,q}^*$] The variable-splitting substitution $\coma_{p+q}\to\coma_p\otimes\coma_q$, and the induced ring homomorphism $H^\bullet(\mathrm{QFl}_{p+q})\to H^\bullet(\mathrm{QFl}_p)\otimes H^\bullet(\mathrm{QFl}_q)$ (\S\ref{subsection:qflbranch}).

...
\begin{definition}
We adjoin variables to $\grass_n$ to obtain $\grass_n[\mathbf{t}]$. Define
$$\mathfrak{E}_n^n(\mathbf{t}) = \sum_{k=0}^n t_1^{n-k}\mathbf{E}_k^n$$
Then define
$$\mathfrak{E}_p^n(\mathbf{t}) = \partial^{s_{n - p}\cdots s_2s_1}(\mathbf{t})\mathfrak{E}_n^n(\mathbf{t})$$

Let $m_i$ denote the RC graph with a single crossing at row $i$ and column $n+1-i$. Define a \emph{straight double elementary RC graph} $E$ to be of the form
$$E(\mathbf{t}) = (m_{i_1} - t_{n+1-i_1})(m_{i_2} - t_{n+1-i_2})\cdots (m_{i_p} - t_{n+1-i_p})$$
where
$$1\leq i_1 < i_2 < \cdots < i_p \leq n$$
The virtue of this definition is that $E(\mathbf{t})$ is uniquely determined by its row weight.
\end{definition}

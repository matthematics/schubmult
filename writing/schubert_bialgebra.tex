...
The combinatorial centerpiece of this paper is such a rule. The forest-code map $\code_\forest$ partitions $\RC_n$ into Nadeau--Tewari forest equivalence classes whose generating polynomials are the $\forest_a$. Within each such class we single out a distinguished choice of representative: call an RC graph $R$ a \emph{forest RC graph} if its forest-code equals its weight, $\code_\forest(R) = \wtt(R)$. Forest RC graphs are not the only sensible choice of class representatives in general, but they are a particularly clean one and the choice that drives our proof.

\begin{theorem}[Forest polynomial LR rule]\label{theorem:forestpolyLR-intro}
...
Here $A*B$ is the \emph{lift product} (\S\ref{section:liftproduct}), a binary operation on RC graphs introduced in this paper for the purpose. Under the Borel-type surjection $\coma_n\twoheadrightarrow \coma_n/\mathrm{QSym}_n^+\cdot\coma_n\cong H^\bullet(\mathrm{QFl}_n)$ of \cite{bergeron2025qsymflag}, the forest polynomials descend to a basis of $H^\bullet(\mathrm{QFl}_n)$, so the same rule computes the cup product there.

The proof requires substantial machinery, which we develop in its own right because it appears to be of independent interest: a Schubert bialgebra $\coma$ with a graded dual $\dcoma$ whose product captures branching of Schubert polynomials; an associative product $\sqcupplus$ on the free abelian group $\BRC$ of bounded RC graphs that realizes $\dcoma$ as a homomorphic image; the squash product $\boxtimes$ and the lift product $*$ on $\BRC_n$; a multiplactic algebra $\mathcal{M}_n$ of elementary RC factors; a Cauchy-type factorization through a slide quotient; and an application of the quasi-crystal framework of Cain--Guilherme--Malheiro \cite{cain2023quasicrystals} to RC graphs. Along the way, the same apparatus produces four further Littlewood--Richardson rules of independent interest --- for the dual Schubert, dual key, dual forest, and dual slide bases of $\dcoma$ --- which we record as Theorems~\ref{theorem:LR}, \ref{theorem:LRkey}, \ref{theorem:LRforest}, and \ref{theorem:lrfslide} below. The dual forest rule is itself the keystone bridge to the forest polynomial rule above. These dual rules form a coherent family of branching/splitting formulas for Schubert, key, forest, and slide polynomials under the partition of variables into two blocks $\{x_1,\ldots,x_p\}$ and $\{x_{p+1},\ldots,x_{p+q}\}$.
...
This splitting lifts, on cohomology, to a ring homomorphism $\iota_{p,q}^*:H^\bullet(\mathrm{QFl}_{p+q})\to H^\bullet(\mathrm{QFl}_p)\otimes H^\bullet(\mathrm{QFl}_q)$ that is the natural quasisymmetric-flag analogue of the Levi comorphism of \cite{ressayre2009branching}; we develop this in \S\ref{subsection:qflbranch}. The dual forest LR rule is also the keystone bridge to the forest polynomial LR rule announced in the lead subsection: Theorem~\ref{theorem:forestpolyLR-intro} is restated as Theorem~\ref{theorem:forestpolyLR} in \S\ref{section:forestrule} once the lift product $*$ has been formally defined, and its proof passes through the dual forest rule together with the squash product $\boxtimes$, the multiplactic algebra $\mathcal{M}_n$, a Cauchy-type factorization through the slide quotient of \S\ref{section:slide}, and the Nadeau--Tewari forest equivalence.

...
\begin{definition}
%Define $\mathcal{E}_
Let $a = (a_1,\ldots,a_n)$ be a weak composition of length $n$ satisfying $a_i\leq i$ for all $i$. Define $\mathcal{E}_a$ to be the Demazure crystal
$$\mathbf{E}_{a_1}^1\otimes \mathbf{E}_{a_2}^2\otimes \cdots \otimes \mathbf{E}_{a_n}^n$$

Given a biword
$$\binom{u_1\cdots u_m}{v_1\cdots v_m},$$
a positive integer $k$, and a positive integer $p\leq k$, we define an insertion algorithm as follows. Find the leftmost $i$ such that $u_i\leq p$. Insert a $p$ between position $i - 1$ and position $i$ in the top row, and insert $N$ between position $i - 1$ and position $i$ in the bottom row, where $N>v_j$ for all $j$. Apply Little's algorithm until the maximum descent of the bottom row is at most the maximum of the maximum descent of $v_1\cdots v_m$ and $k$.

A Pieri $(p,k)$-deletion from a biword is a set of positive integers $c_1<\cdots < c_p\leq k$ such that the deletion of the indices $i_1, \ldots, i_p$ of the leftmost occurrences in $u$ such that $u_{i_j} = c_j$ yields a reduced word for a permutation with exactly $p$ fewer inversions, and if $t_{a_1,b_1},\cdots,t_{a_p,b_p}$ are the right reflections deleted at successive steps, then $a_i\leq k < b_i$ for all $i$ and $a_i\neq a_j$ if $i\neq j$.

For a partition $\lambda$ and a weak composition $\alpha$ with $\alpha_i\leq \lambda_i$ for all $i$, an $(\alpha,\lambda)$-Pieri factorization of a biword is $\ell(\lambda)$ successive Pieri $(\alpha_i,\lambda_i)$-deletions, where $|\alpha|$ is the total length of the word.

Suppose $A_1\otimes A_2\otimes \cdots \otimes A_N$ is an elementary tensor of elementary symmetric RC graphs in weakly increasing order of height $(\lambda_N,\ldots,\lambda_1)$, and define
$$\mathrm{Pieri}(A_1\otimes A_2\otimes \cdots \otimes A_N) = (\wtt(A_1),\ldots,\wtt(A_N))$$

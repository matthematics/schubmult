...
The key polynomials $\kappa_a$ (Demazure characters in type $A$) form a $\mathbb{Z}$-basis of $\mathbb{Z}[x_1,x_2,\ldots]$ indexed by weak compositions; we defer definitions and a survey of the literature to the start of \S\ref{section:crystals}. The dual key basis $\{\kappa_a^*\}$ of $\dcoma$, defined by $\langle\kappa_a^*,\kappa_b\rangle=\delta_{a,b}$, has not, to our knowledge, been studied previously. Using the Little-bump description \cite{little2003combinatorial,hamaker2014relating} of $\zeromap$ and the Demazure crystal structure on RC graphs of \cite{assaf2018demazure}, we obtain:
...
Fix an RC graph $C$ with $\mathrm{extwt}(C)=c$, and assume $C$ is a highest weight. Then $k_{a,b}^c$ is the number of RC graphs $R$ such that $\mathbb{Y}(R)=C$ satisfying $\mathrm{extwt}(\clp^p(R)) = a$ and $\mathrm{extwt}(\trm^p(R)) = b$ and $\clp^p(R),\trm^p(R)$ are highest weights. The count is independent of the choice of $C$.
\end{theorem}
...
Fix an RC graph $C\in\mathcal{C}_\forest(c)$. Then $f_{a, b}^c$ is the number of $R\in[C]$ such that $\clp^p(R)\in \mathcal{C}_\forest(a)$ and $\trm^p(R)\in \mathcal{C}_\forest(b)$, satisfying the additional condition that $\wtt(\clp^p(R))=a$ and $\wtt(\trm^p(R))=b$, and is in particular nonnegative. The count is independent of the choice of $C$.
\end{theorem}
...
has the same cardinality as $\mathcal{RC}_n(w)^0$ by injectivity, and is a subset of $\bigcup_{w\downvar{n} w'}\mathcal{RC}_{n-1}(w')$. Since the right hand side has the same cardinality as $\mathcal{RC}_n(w)^0$, we conclude that $\zeromap$ is surjective, as desired.
\end{proof}
...
The \emph{forest polynomials} $\forest_a$ were introduced by Nadeau and Tewari in \cite{nadeau2024forest} and are closely related to Schubert polynomials. They can be defined by reduced words and compatible sequences, but the invariant preserved by these words is distinct from the permutation. Given a reduced word/compatible sequence pair $(\mathbf{r},\mathbf{s})$, there is an insertion algorithm that produces a corresponding invariant $\mathcal{F}(\mathbf{r},\mathbf{s})$ that represents an equivalence relation on the words, where the compatible sequence can vary arbitrarily. We therefore shorten the notation to $\mathcal{F}(\mathbf{r})$. The equivalence classes consist of words that are all for the same permutation, so that if we define
$$R\equiv_{\forest} R'$$
to mean that $\mathcal{F}(\mathrm{word}(R)) = \mathcal{F}(\mathrm{word}(R'))$, then the forest polynomials partition the set of RC graphs for the given permutation. The forest polynomials are indexed in the original work by ``indexed forests,'' hence the name. These correspond bijectively to weak compositions, so we can define a weak composition $\fcode(R)$ by associating an indexed forest to this equivalence class as in the article. Define $\mathcal{C}_\forest(a)$ for a composition $a$ to be the set of equivalence classes of RC graphs under this relation such that $\fcode(R)=a$.
...
The statement of Theorem~\ref{theorem:forestpolyLR} concerns forest polynomials directly rather than their duals, and although its proof draws on machinery developed alongside the dual rules (the slide quotient of \S\ref{section:slide}, in particular), a reader interested only in this result need not work through the earlier LR rules first.
...
$$\phi:\BRC_n\to\mathbb{Z}[x_1,\ldots,x_n],\qquad \phi(R) = x^{\wtt(R)},$$
...
$$\wtt(R_1*R_2) = \wtt(R_1) + \wtt(R_2)$$
componentwise. The ring-homomorphism statement is immediate: $\phi(R_1*R_2) = x^{\wtt(R_1)+\wtt(R_2)} = x^{\wtt(R_1)}\,x^{\wtt(R_2)} = \phi(R_1)\phi(R_2)$.
\end{proof}
...
\begin{proposition} \label{proposition:quasiprod}
...
% Earlier draft (commented out 2026-06-05) of this lemma tried to establish
% the much stronger statement
%   (*)  A ~_forest A'  =>  A*B ~_forest A'*B   (and symmetrically),
% which is FALSE: it would force the product of two forest polynomials to be
% a single forest polynomial, contradicting the structure of the beta_{ab}^c.
% That argument silently assumed word(A*B) = word(A) . word(B) (concatenation);
% but * row-interleaves and Little-bumps, so letters of A and B are mixed and
% transformed. The corrected proof below routes through Theorem 6.3 of
% Nadeau--Tewari ("th:mult_forest_poly") and the quasi-crystal isomorphism of
% Theorem~\ref{theorem:slideproduct}, which together let us transport NT's
% shuffle-stability statement to the lift product.
...
\end{proposition}
\begin{proof}
% The lift product is \emph{not} given by word concatenation: as discussed, it row-interleaves $A'$ and $B'$ and applies Little bumps to restore reducedness. The proposition is therefore not a tautology. What it does follow from is the quasi-crystal language of \cite{cain2023quasicrystals}, applied as follows.
...
By Theorem~\ref{theorem:slideproduct}, the map $A'\,\ddot\otimes\,B'\mapsto A'*B'$ is an isomorphism of quasi-crystals from the connected component of $A\,\ddot\otimes\,B$ in $\BRC_n\,\ddot\otimes\,\BRC_n$ to its image in $\BRC_n$, and at the level of slide polynomials this gives
\[
\mathfrak{F}(A)*\mathfrak{F}(B)\;=\;\sum_{R}\mathfrak{F}(R)
\]
with $R$ ranging over a multiset of QY representatives in bijection with the slide classes of the lift-product image. Composing with $R\mapsto\overline{\mathrm{word}(R)}$ identifies that multiset with the slide classes appearing in $\mathfrak{F}(W_A)\,\mathfrak{F}(W_B)$, hence with the slide classes in $\mathsf{Sh}(W_A,W_B)$. This is precisely the bijection in (2), and (1) is its forward direction.
\end{proof}
...
\end{lemma}
\begin{proof}
We reduce to NT's shuffle-stability statement~\cite[proof of Theorem~6.3]{nadeau2024forest}, which establishes that for any forest classes $\mathcal{C}_1,\mathcal{C}_2$ of injective words on $\overline{\mathbb{Z}}$, the multiset
\[
\mathsf{Sh}(\mathcal{C}_1,\mathcal{C}_2)\;=\;\bigsqcup_{W_1\in\mathcal{C}_1,\,W_2\in\mathcal{C}_2}\mathsf{Sh}(W_1,W_2)
\]
is closed under $\equiv_\forest$. Their argument uses two ingredients: (a) shuffles are \emph{by definition} closed under transpositions of two adjacent letters that come from different factors, hence under any cross-boundary single-step forest move; and (b) NT Lemma~5.10 (anti-monoid congruence: if $UabV\equiv_\forest UbaV$ via a single generating commutation, then $U_1abV_1\equiv_\forest U_1baV_1$ for any subwords $U_1\subseteq U$, $V_1\subseteq V$), which handles single-step moves whose pair of letters lies entirely within one factor.
...
\item[$\RC_n$] The set of bounded RC graphs with $n$ rows.
\item[$\RC_n(w)$] The set of $R\in\RC_n$ with $\wof{R}=w$.
\item[$\RC_n^0$] The set of $R\in\RC_n$ whose last row is empty.
...
\item[$\trm^p(R)$] ``Truncate'': the RC graph obtained from the last $\hr(R)-p$ rows of $R$.
\item[$\clp^p(R)$] ``Clip'': the RC graph in $\RC_p$ obtained from $R$ by an insertion algorithm based on $\zeromap$; the dual partner of $\trm^p$.
...
\begin{description}
\item[$e_i,f_i$] Crystal raising and lowering operators on RC graphs.
\item[$\mathrm{Dem}(R)$] The Demazure crystal of $R$ (Assaf--Schilling).
\item[$\mathrm{extwt}(R)$] The extremal weight of $\mathrm{Dem}(R)$, i.e.\ the weight of its unique lowest weight element.
\item[$\mathbb{Y}(R)$] The highest-weight (Coxeter--Knuth) representative of $R$ in its Demazure crystal.
...
\item[$\mathcal{F}(R)$] The forest invariant of $R$, an indexed forest (Nadeau--Tewari).
\item[$\fcode(R)\,(=\code_\forest(R))$] The composition associated to $\mathcal{F}(R)$. The forms $\fcode(R)$ and $\code_\forest(R)$ are used interchangeably.
\item[$\mathcal{C}_\forest(a)$] The set of forest classes of RC graphs with $\fcode=a$.
\item[$\equiv_\forest$] Forest equivalence: $R\equiv_\forest R'$ iff $\mathcal{F}(R)=\mathcal{F}(R')$.
\item[$\RC_\forest(c)$] The set of $R\in\RC_n$ with $\wtt(R)=c$ such that $\mathrm{word}(\mathbb{Y}_\forest(R))$ is lex-minimal among $R'$ with $\code_\forest(R')=\code_\forest(R)$ (\S\ref{section:forest}).
\end{description}

\subsubsection*{The Schubert bialgebra and its dual}
\begin{description}
\item[$\coma_n$] The polynomial ring $\mathbb{Z}[x_1,\ldots,x_n]$.
\item[$\coma$] The Schubert bialgebra $\bigoplus_{n\geq 0}\coma_n$, with $\coma_m\cdot\coma_n=0$ when $m\neq n>0$.
\item[$\dcoma$] The graded dual bialgebra of $\coma$.
\item[$\sch_u^n$] Schubert polynomial in $\coma_n$, $\maxd(u)\leq n$.
\item[$\dsch_u^n$] Element of $\dcoma$ dual to $\sch_u^n$.
\item[$\kappa_a$, $\kappa_a^*$] Key polynomial / its dual; $a\in\mathrm{Comp}_n$.
\item[$\forest_a$, $\forest_a^*$] Forest polynomial / its dual; $a\in\mathrm{Comp}_n$.
...
\item[$\Delta^*$] The dual coproduct on $\dcoma$ (the dual of the polynomial product on $\coma$).
\end{description}
...
\begin{description}
\item[$\BRC$] $\bigoplus_{n\geq 0}\mathbb{Z}^{\oplus\RC_n}$, with the product $\sqcupplus$.
\item[$\BRC_n$] The graded component $\mathbb{Z}^{\oplus \RC_n}$ of $\BRC$ (RC graphs of height exactly $n$).
\item[$\sqcupplus$] The associative concatenation product, used on $\BRC$, on $\dcoma$, and on the quotients $\BRC/I_\sim$; built from $\clp^p$ and $\trm^p$ on $\BRC$ (Definition in \S\ref{section:ringproduct}).
\item[$*$] The auxiliary ``lift product'' on $\BRC_n$ used in the forest LR rule (\S\ref{section:liftproduct}).
\item[$\boxtimes$] The ``squash product'' of RC graphs (\S\ref{section:liftproduct}); the row-wise concatenation operation that lifts to $\BRC_n\otimes\BRC_n\to\BRC_n$ before passing to $*$.
\item[$\alpha:\BRC\to\dcoma$] Ring homomorphism with $\alpha(R)=\dsch_{\wof{R}}^{\hr(R)}$.
\item[$\Omega$] $\ker\alpha$, the two-sided ideal of $\BRC$ on which $\alpha$ vanishes.
\item[$\Theta$, $\Gamma$] Ideals of $\BRC$ whose quotients are indexed by Demazure crystals, resp.\ forest classes (yielding Theorems~\ref{theorem:LRkey}, \ref{theorem:LRforest}).
\item[$\mathcal{M}_n$] The multiplactic algebra of full Grassmannian RC graphs.
\item[$\mathcal{E}_n$] The elementary basis of $\coma_n$.
...
\item[$P_n:\mathrm{SSYT}(n)\to\grass_n$] The bijection of \S\ref{section:multiplactic} sending a semistandard Young tableau to a full Grassmannian RC graph.
\end{description}

\subsubsection*{Slide polynomials and the general LR template}
\begin{description}
\item[$\mathfrak{F}_a(x)$, $\mathfrak{F}_a^*$] Slide polynomial in $\coma$ (Assaf--Searles) indexed by a weak composition $a$; its dual in $\dcoma$.
\item[$\QY(R)$] Quasi-Yamanouchi representative of $R$ in its $(\mathrm{word},\hr)$-class; smallest element under the dominance/refinement order on weights.
\item[$\RC_\QY(c)$] Set of RC graphs $R$ with $\wtt(R)=c$ such that $\mathrm{word}(\QY(R))$ is lex-minimal among $R'$ with $\wtt(\QY(R'))=\wtt(\QY(R))$.
\item[$\Sigma$] Two-sided ideal of $\BRC$ generated by $R_1-R_2$ with $\QY(R_1)=\QY(R_2)$ (Lemma~\ref{lemma:slideideal}).
\item[$\mathcal{F}_a$] The element $\sum_{R=\QY(R),\,\wtt(R)=a} R\in\BRC/\Sigma$, a representative of the slide basis under $\alpha$.
\item[$\sim$] A generic equivalence relation on $\bigsqcup_n\RC_n$ satisfying (H1)--(H4) of Theorem~\ref{theorem:general_lr_template}.
\item[$I_\sim$] The two-sided ideal $\mathrm{span}_{\mathbb{Z}}\{R_1-R_2:R_1\sim R_2\}\subseteq\BRC$.
\item[$J$, $\lambda$] Index set, resp.\ labeling map $(\RC/\!\sim)\to J$ in the general LR template.
\item[$E_\lambda$] The class sum $\sum_{[R]:\,\lambda([R])=\lambda}[R]\in\BRC/I_\sim$.

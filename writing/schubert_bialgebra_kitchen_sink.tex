...
$$R - \hw(R)$$
Then $\hw:\BRC\to\mathcal{N}$ is a homomorphism of rings.

Is squash inserting?

Yes squash product is left insertion into the highest weight-tableau representation.

Define a module $\ww(\BRC)$ and function
$$\ww:\BRC\to \mathcal{P}$$
such that if
$$R = F\hw(R)$$
then
...
Define
$$\hw(R_1)\diamond \hw(R_2)=\hw(R_1\diamond R_2)$$
$$\ww(R_1)\ww(R_2) = \ww(R_1\diamond R_2)$$

The function 
$$\BRC\to \mathcal{N}\otimes_{\BRC} \mathcal{P}$$
is an isomorphism of rings. 
%We afford $\hw(\BRC)\otimes \ww(\BRC)$ the structure of a ring as follows. Suppose

% For each such $R$, suppose
% $$R = F_R\hw(R)$$
% Then define
% $$(\hw(R_1)\otimes \ww(R_1))\cdot (\hw(R_2)\otimes \ww(R_2)) = \sum_R \hw(R)\otimes F_{\shup^{\hr(R_1)}R_2}F_{R_1}F_R\ww(\hw(R))$$

%Note: $\ww:\BRC\to \ww(\BRC)$ is not a homomorphism of rings.
\end{definition}

In terms of crystals,
$$\ww(R_2\otimes R_1) = \ww(R_1)\ww(R_2)$$
...
% \subsection{Schubert crystals}

% A Schubert crystal is a set $\mathcal{B}(w)$ together with operators $e_i, f_i, c_i:\mathcal{B}(w)\to \mathcal{B}(w)\cup\{0\}$, a permutation $w$, along with weight functions $\hw$ and $\wtt$ satisfying the following:

% $f_i(b)=0$ for all $i$ if and only if $\wtt(b)=\code(w)$.

% If $\varepsilon_i(b)=0$ and $e_i(b)\neq 0$, then $\wtt(\hw(e_i(b))) = \mathrm{sort}(\wtt(b) + \alpha_i)$.
% $$\varphi_i(b) = \varepsilon_i(b) + \langle \wtt(b), \alpha_i^\vee\rangle$$
% If $e_i(b)\in\mathcal{B}$, then $\hw(e_i(b)) = \hw(b) \iff\varepsilon_i(b)>0$.
% $$f_i(b) = b'\iff e_i(b') = b$$
% If $e_i(b)\neq 0$, then $e_i(b)=c_i(b)$.
% $c_i(b)\neq 0$ implies $\wtt(c_i(b)) = \wtt(b) + \alpha_i$
% $e_i(b)\in \mathcal{B}$ implies $\varepsilon_i(e_i(b)) = \varepsilon_i(b) - 1$
% $c_i(b)\in \mathcal{B}$ implies $\varepsilon_i'(c_i(b)) = \varepsilon_i'(b) + 1$
% $\varepsilon'_i(b)\geq \varepsilon_i(b)$ for all $b$ and $i$.
% $$e_i(b) = b'\iff f_i(b') = b$$
% For each $\hw(b)$, there is a $u_{\hw(b)}$ such that there is a reduced word for which
% $$\{\hw(b') = \hw(b)\} = \{f_{i_1}^{a_1}\cdots f_{i_k}^{a_k}\hw(b): a_1,\ldots,a_k\geq 0\}\setminus\{\emptyset\}$$
% $$\overline{\varepsilon}_i(b)=\max\{k: e_i^k(b)\neq 0\}=\varepsilon_i(b) + \overline{\varepsilon}_i(e_i\hw_i(b))$$
...
\begin{definition}
We define the \emph{squash product} of two RC graphs $R_1$ and $R_2$ to be the RC graph
$$R_1\boxtimes R_2 = \mathtt{clip}^n(R_1\star R_2)$$
\end{definition}

\begin{theorem}
Let $u,v\in S_\infty$ have height $n$, with $v$ being $n$-Grassmannian. Then the function $A\otimes B\mapsto A\boxtimes B$ is a crystal isomorphism $\RC(u)\otimes \RC(v)\to \bigoplus^{c_{uv}^w}\RC(w)$.
\end{theorem}
\begin{proof}
First consider the set $\RC(u)\star_n \RC(v)$. By Theorem \ref{theorem:starproductcrystal}, this is a crystal graph isomorphic to $\RC(u)\otimes \RC(v)$. Since $u$ and $\widetilde{v}=\shup^Nv$ are completely  disjoint, we have that
$$\sch_u(x)\sch_{\widetilde{v}}(x) = \sch_{u\widetilde{v}}(x)$$
Since $v$ is $n$-Grassmannian, we have
$$\sch_{\widetilde{v}}(x_1,\ldots,x_n,0,\ldots) = \sch_v(x_1,\ldots,x_n)$$
and since $\maxd{u}\leq n$, we have
$$\sch_{u}(x_1,\ldots,x_n,0,\ldots) = \sch_u(x_1,\ldots,x_n)$$
Thus $\clp^n(R_1\star R_2)$ is an RC graph for a permutation $w$ such that $c_{u,v}^w > 0$. Since the star product is a homomorphism of crystals, as is $\clp$, this is in fact an isomorphism of crystals as claimed.
\end{proof}

\newcommand{\grass}{\mathcal{G}}
\begin{definition}
For each $n$, let $\grass_n$ be the free abelian group generated by the RC graphs $G$ with $\hr(G) = n$ such that $\wof{G}$ is $n$-Grassmannian.
\end{definition}

\begin{corollary}
  Under the squash product, $\grass_n$ is a ring isomorphic to the ring $\mathcal{P}_n^{\#}$ and has a right module action on $\BRC_n$ also given by $\boxtimes$.
\end{corollary}

% be the free associative algebra generated by the set of RC graphs $R$ with $\hr(R)=k$ such that $\sch_{\wof{R}}(x_1,\ldots,x_n)$ is an elementary symmetric polynomial in $x_1,\ldots,x_k$. There is a homomorphism of rings $\mathcal{E}_k\to \mathcal{E}_k\otimes\mathcal{F}_N$ for $N>k$ given by sending 
% $$R\mapsto R\otimes E_{\ell(R),\maxd(R)}(\theta_1,\ldots,\theta_N)$$
% Let $\mathcal{P}_n$ be the subalgebra of $\mathcal{E}_k\otimes\mathcal{F}_k$ that is the image of this homomorphism.

% % \begin{corollary}
% % The squash product $\boxtimes$ is associative and defines a right module action of $\mathcal{P}_n$ on $\BRC_n$.
% % \end{corollary}
% % \begin{proof}
% % The tensor product of crystals is associative.
% % \end{proof}

% % \begin{proposition}
% % There is a right module action of $\mathcal{P}_n$ on the subspace of $\BRC_n\otimes \mathcal{F}_N$ spanned by the elements
% % $$R\otimes \sch_{\wof{R}}(\theta_1,\ldots,\theta_N)$$ 
% % defined by
% % $$(R\otimes \sch_{\wof{R}}(\theta))\boxtimes (p\otimes q) = R\boxtimes p$$
% % \end{proposition}
% % \begin{proof}
% % Consider the element
% % $\mathbf{E}_{p,k} = \sum_{R\in \mathcal{E}_{p,k}} R\otimes E_{\ell(R),\maxd(R)}(\theta_1,\ldots,\theta_N)$
% % Then $\mathbf{E}_{p,k}$ is a well-defined element of $\mathcal{P}_n$ for $k\leq n$. 
% % \end{proof}

% % Let $\BRC_n^{(p)}$ be the submodule of $\BRC_n$ generated by elemnts of $\RC(v)$ for permutations with minimum descent at least $p$ and $\maxd(v)\leq n$.

% % \begin{proposition}
% % There is a surjective homomorphism of right $\mathcal{P}_n$-modules $\pi:\bigotimes_{i=p}^n \mathcal{P}_i$ to $\BRC_n^{(p)}$ given by sending 
% % $$p_1\otimes p_2\otimes \cdots \otimes p_{n-p+1} \mapsto p_1\boxtimes p_2 \boxtimes \cdots \boxtimes p_{n-p+1}$$
% % Under this map, elementary tensors are sent to single RC graphs.
% % \end{proposition}

% % \begin{corollary}
% % $\BRC_n^{(p)}$ is $(\mathcal{P}_p,\mathcal{P}_n)$-bimodule under the squash product.
% % \end{corollary}

% % Fix $k$ with $p\leq k\leq n$, and define an isomorphism of vector spaces
% % $$\mathbf{t}_k:\bigotimes_{i=k+1}^n \mathcal{P}_i\otimes \bigotimes_{i=p}^k \mathcal{P}_i\to \bigotimes_{i=p}^n \mathcal{P}_i$$
% % Then $\pi_k=\pi\circ \mathbf{t}_k$ is a surjective homomorphism of vector spaces.
% % \begin{center}
% % \begin{tikzcd}
% % Fix $k$ with $p\leq k\leq n$, and define a function
% % $$\pi_k:\bigotimes_{i=k}^n \mathcal{P}_i\otimes \bigotimes_{i=p}^k \mathcal{P}_i\to \BRC_n^{(p)}$$
% % by
% % $$\pi_k(a\otimes b) = \pi(b_1\otimes \cdots\otimes (b_k\boxtimes a_k)\otimes a_{k+1}\otimes\cdots\otimes a_n)$$
% % There is a surjective homomorphism of vector spaces
% % $$\tau_k:\bigotimes_{i=k}^n \mathcal{P}_i\otimes \bigotimes_{i=p}^k \mathcal{P}_i\to \bigotimes_{i=p}^k \mathcal{P}_i {}_{\mathcal{P}_k}\otimes_{\mathcal{P}_k} \bigotimes_{i=k}^n \mathcal{P}_i$$
% % and the codomain, by associativity of the tensor product and $\boxtimes$, is isomorphic to
% % $$\bigotimes_{i=p}^n \mathcal{P}_i$$
% % Thus
% % $$\pi_k = \pi\circ \tau_k$$
% % Thus, if $\pi_k(a) = R$, for an element $p\in\mathcal{P}_k$ we may define
% % $$R\boxtimes_k p = \pi(\tau_k(rp))$$
% % and
% % $$p\boxtimes_k R = \pi(\tau_k(pr))$$
% % where $\tau_k(r) = R$. This gives us a well-defined right and left action of $\mathcal{P}_k$ on $\BRC_n$ for all $k\leq n$.

% % We now take the subspace of $\bigotimes_{i=p}^n \mathcal{P}_i$ spanned by tensors of the form
% % $$a_1\otimes \cdots \otimes a_{N}$$
% % where if $k<n$, then $a_1\boxtimes\cdots\boxtimes a_k\in S_{k+1}$.

% % \begin{theorem}
% % Consider the right $\mathcal{P}_n$-submodule of $\BRC_n$ spanned by the elements $\mathcal{S}_w(n)$ for $w\in S_n$. Then these elements form a free submodule of $\BRC_n$ with basis $\mathcal{S}_w(n)$ for $w\in S_n$ that contains all Schubert elements $\mathcal{S}_w(n)$ for $w\in S_\infty$ with $\maxd(w)\leq n$.
% % \end{theorem}

% % \begin{corollary}
% %   The RC graphs $R\in \BRC_n$ such that $\wof{R}\in S_n$ generate $\BRC_n$ as a right $\mathcal{P}_n$-module, and every set of RC graphs $R\in \BRC_n$ such that $\wof{R}\in S_n$ and $\wof{R}\neq \wof{R'}$ for $R\neq R'$ is a basis of a free right $\mathcal{P}_n$-submodule of $\BRC_n$.
% % \end{corollary}
\begin{corollary}
Let $\lambda$ be a partition, say of length $m$, and let $w_\lambda$ be the dominant permutation with $\code(w_\lambda)=\lambda$. Let $V$ be an RC graph with $\wof{V} = v$. Then the number of factorizations of the form
$$E_{i_m, \lambda_m}\boxtimes\cdots \boxtimes E_{i_1, \lambda_1} = V$$
where $E_{p,k}$ is an arbitrary RC graph for an elementary symmetric polynomial in $k$ variables of degree $p$ is equal to the number of RC graphs $A$ for $vw_{\lambda}^{-1}$ with 
$$\wtt(A) = (i_1,\ldots,i_m)$$
\end{corollary}

\begin{corollary}
Suppose $R\in \BRC_n(w)$ and $w_{\lambda}$ is the dominant permutation with $\code(w_\lambda) = \lambda$, where $\lambda$ is an $n$-elementary partition satisfying $w\leq_L w_{\lambda}$. Then there is precisely one factorization of the form
$$E_{i_m, \lambda_m}\boxtimes\cdots \boxtimes E_{i_1, \lambda_1} = R$$
where $\code(ww_{\lambda}^{-1}) = (i_1,\ldots,i_m)$ and each factor is an RC graph for an elementary symmetric polynomial with the indicated degree and number of variables.
%with $\code(\wof{R})=(c_1,\ldots,c_n)$. Then there is precisely one sequence of RC graphs $E_1,R_2,\ldots,R_m$ such that $R = R_1\boxtimes R_2\boxtimes \cdots \boxtimes R_m$ and each $R_i$ is an RC graph for an elementary symmetric polynomial in some number of variables. ALMOST
\end{corollary}

\begin{corollary}
  Let $R_1, \ldots, R_N$ be a sequence of distinct highest weight RC graphs such that $\wof{R_i}\in S_n$ for all $i$. Then $R_1,\ldots,R_N$ form the basis of a free right $\grass_n$-submodule of $\BRC_n$ under the squash product.
\end{corollary}
\begin{proof}
Every product of the form $E_1\boxtimes \cdots\boxtimes E_m$ that yields a distinct element of $\grass_n$ must involve distinct sequences of $E_{p,k}$ factors. The above result implies therefore that, fixing a single RC graph $R$, all of these products acting on $R$ yield distinct nonzero single RC graphs. Using the fact that these products generate $\grass_n$, we can deduce that the submodule $R\grass_n$ is free with basis $\{R\}$. We prove the full result by induction on $N$. For the inductive step, we have that $R_1,\ldots,R_{N-1}$ form the basis of a free right $\grass_n$-submodule of $\BRC_n$. If adding $R_N$ does not result in the basis of a free module, we must then have that there are $G_1, G_2$ and some $i$ such that
$$R_N\boxtimes G_1 = R_i\boxtimes G_2$$
There are then unique $\widetilde{G_1}$, $\widetilde{G_2}$ of the same respective shape as $G_1$, $G_2$ such that 
$$R_N\boxtimes \widetilde{G_1}=R_i\boxtimes \widetilde{G_2}$$
and these elements are both highest weights. It follows that the corresponding crystals are isomorphic as Demazure crystals. Since the key polynomials form a basis for a free submodule of the polynomial ring $\mathbb{Z}[x_1,\ldots,x_n]$ over the ring $\Lambda_n$ of symmetric polynomials, it follows that $R_N$ and $R_i$ must have the same weight, and therefore so do $G_1$ and $G_2$. We may then use uniqueness of factorizations in terms of elementary symmetric RC graphs in a certain order to conclude that $R_i=R_N$, and we have the result by induction.
\end{proof}

\begin{proposition}
Let $R\in \BRC_{n-1}(S_n)$. Then there is a unique factorization of the form
$$E_1*\cdots*E_{n-1}=R$$
where $|E_i|=\wtt_{n - i}(\hw(R)^t)$.
\end{proposition}

\begin{proposition}
Let $R\in \hw\BRC_n$. Then there is a unique factorization of the form
$$R_**R^*$$
such that $R_*\in\BRC_n(S_n)$ and $R^*\in \grass_n$.
\end{proposition}

\begin{theorem}
Let $R\in \BRC_n$. Then there is a (necessarily unique) crystal isomorphism of the form
$$E_1\otimes\cdots\otimes E_{n-1} \otimes R^*$$
where $|E_i|=\wtt_{n - i}(\hw(R_*)^t)$ and $R^*\in \grass_n$.
\end{theorem}

\begin{theorem}
The map $\BRC_n\to \dcoma_n$ sending $R\to ES(\wtt_{n-i}(\hw(R)^t),\lambda(R^*))$ in the Schur-elementary basis is a homomorphism of rings.
\end{theorem}

\begin{definition}
Define a function $\Phi:\BRC_m\to \dcoma_m\otimes \dcoma_{m}\otimes\grass_m$ by
$$\Phi(R) = (\wtt(R),R_*,R^*)$$
\end{definition}

Define a function $E:\BRC\to\dcoma$ by
$$E(R) = \sum_{[\mathfrak{e}_c][\wtt(R)]\neq 0}[\mathfrak{e}_c]\Xi_{\wof{R}}^{\hr{R}}$$

Let $\mathcal{S}$ be the subalgebra of $\coma\otimes\coma$ generated by the elements $e_p^n\otimes x_a$ where $\ell(a)=n$ and bounded above by partition. Let $I$ be the ideal generated by the elements $\sch_v^n\otimes x_b$ where the coefficient of $x_b$ in $\sch_v^n$ is zero.


\begin{definition}
Let $R\in \BRC$ be such that
$$\alpha(R) = \sum_c a_{\wof{R},c}E_{c}$$
and
$$\omega(R) = \sum_c p_{\wtt(R), c}E_c$$
Then define
$$\Phi(R) = \sum_{p_{\wtt(R),c}\neq 0} a_{\wof{R},c}E_c$$
\end{definition}

Let $R\mapsto R\otimes R\otimes R$ be the diagonal map. Substitute
$$R\mapsto \Xi_{\wof{R}}^n\otimes\wtt(R)\otimes R$$
Let $D$ be any basis of $\dcoma$.

$$\dcoma\xrightarrow{\iota} R\xrightarrow{\alpha} \dcoma$$
is the identity.

%$$R\mapsto D_i\otimes  T_{\wof{R}}^i\wtt{R}\otimes R$$
Basis of functions $R:\dcoma\to\BRC^*$ such that $R(\sch_{\wof{R}})=\wtt{R}$.

$$\hom(\dcoma^*,\BRC^*)$$

% \begin{proposition}
% The map $\Phi:\BRC\to \dcoma$ is a homomorphism of rings.
% \end{proposition}
% \begin{proof}
% Consider the diagonal map $\dcoma\to\dcoma\otimes\dcoma$. Changing the left factor to the elementary basis, we obtain
% $$c\mapsto \sum_{a,b} p_{c,a}E_a\otimes c$$
% Now we map to $\BRC$ to obtain
% $$\sum_{R\in\RC(c)} R \mapsto \sum_{R\in\RC(c)} p_{c,a}E_a\otimes R$$
% \end{proof}
% %Left action bpds?


$$(f_1f_2)(R) = \sum_{\wtt(R_1)+\wtt(R_2)=\wtt(R)} \langle R_1\otimes R_2, f_1(R_1)\otimes f_2(R_2)\rangle f_1(R_1)f_2(R_2)$$


$\coma = (\coma^*)^*$

Treat $R$ as the monomial dual of $x_{\wtt(R)}$.

$$\Xi_w^n = \sum_c A_w^c[c]$$

Then map
$$x_c\mapsto \sum_{R\in \RC(w,c)} A_w^cR$$
% Now, we could also define a left action of $\grass_n$ on $\BRC_n$ by defining
% $$G\boxtimes^L R = R\boxtimes G$$
% for $G\in \grass_n$ and $R\in \BRC_n$. However, this does not give the desired behavior. Instead, we proceed as follows:
Algebraically independent, except that $\sum_{u,v,a+b=c}A_u^aA_v^bR_1R_2 = \sum_{w,c}A_w^cR$
% For Kashiwara crystals, there is an isomorphism
% $$A\otimes B \simeq B^{\#}\otimes A^{\#}$$
% where $\#$ is a canonical antiautomorphism of the crystal. When $A$ is instead a Demazure crystal while $A$ remains a full crystal, we can use this to define a left action of $\grass_n$, with the caveat that $A^{\#}$ is not a Demazure crystal, but rather an opposite Demazure crystal.
$x_c\mapsto \sum_{R\in \RC(c)}A_{\wof{R}}^{c}R$
% We need, therefore, an opposite construction of RC graphs. Let $A$ be a finite set of pairs of positive integers $(i,j)$, with some bound $1\leq i\leq n$, declaring that $\hr(A) = n$. For such a pair $(i,j)\in A$, we define 
% $$\widetilde{s}(i,j) = n  - i + j$$
% There is a clear bijection $\BRC_n\to \widetilde{\BRC}_n$ given by sending $A$ to 
% $$A^{\#} = \{(n + 1 - i, j) \mid (i,j)\in A\}$$
% This is the restriction of the same antiautomorphism of an associated ambient Kashiwara crystal. We may define
% $$\widetilde{\clp}^i(A^{\#}) = \trm^{n + 1 - i}(A)^{\#}$$
% and
% $$\widetilde{\trm}^i(A^{\#}) = \clp^{n + 1- i}(A)^{\#}$$
% Similarly defining
% $$A^{\#} \diamond B^{\#} = \sum_{R:\widetilde{\clp}^p(R) = A^{\#}, \widetilde{\trm}^p(R) = B^{\#}} R$$
% it follows that
% $$(A\diamond B)^{\#} = B^{\#}\diamond A^{\#}$$
% \begin{proposition}
% The map $\BRC_n\to \widetilde{\BRC}_n$ given by $A\mapsto A^{\#}$ is an antihomomorphism of rings.
% \end{proposition}

% \newcommand{\lboxtimes}{\boxtimes^{\#}}
% Similarly, for an RC graph $A$ and a Grassmannian RC graph $G$, consider
% $$(A\star G)^{\#} = G^{\#}\star A^{\#}$$
% and
% $$(A\boxtimes G)^{\#} = G^{\#}\boxtimes A^{\#}$$
% Hence we may define a left action of $\grass_n$ on $\BRC_n$ by
% $$G\lboxtimes R = (R^{\#}\boxtimes G^{\#})^{\#}$$

% \begin{proposition}
%   We have that $\BRC_n$ is a $(\grass_n, \grass_n)$-bimodule under the left and right actions defined by 
% $\lboxtimes$ and $\boxtimes$, respectively.
% \end{proposition}
% \begin{proof}
%   We have
% \begin{align*}
%   (G_1\lboxtimes R)\boxtimes G_2&=(R^{\#}\boxtimes G_1^{\#})^{\#}\boxtimes G_2\\
% &=(G_2^{\#}\boxtimes (R^{\#}\boxtimes G_1^{\#}))^{\#}\\
%   &=((G_2^{\#}\boxtimes R^{\#})\boxtimes G_1^{\#})^{\#}\\
%   &=G_1\lboxtimes (R\boxtimes G_2)
% \end{align*}
% \end{proof}

% % \subsection{The two row product}

% % We define a product on $\BRC_2$.

% The map $\zeromap:\BRC_n\to \BRC_{n-1}$ preserves the bimodule action. There is a comodule structure on $\BRC_n$ given by the map 
% $$M:\BRC_n\to \BRC_{n}\otimes \grass_n$$
% defined by
% $$R\mapsto \sum_{A\boxtimes G=R} A\otimes G$$
% $$\mathbf{K}(R)$$

% We give an explicit bijection $R\mapsto (N(R), P(R))$, where $\mathrm{eg}$ is the set of EG words and $\mathrm{Tab}$ is the set of semistandard Young tableaux of the same shape. This bijection is weight preserving and restricts to an isomorphism of crystals on each connected component. We then define a product on $\mathrm{eg}\times \mathrm{Tab}$ by declaring that. ED Insert RSK

% The EG plactic might give enough info
% %We define the antiautomorphism $\#$ on $\BRC_n$ by sending $R$ to $\{(i, n+1-j) \mid (i,j)\in R\}$. This is an antiautomorphism because the crystal operators are defined in terms of pairings of elements in adjacent rows, and these pairings are preserved under this transformation. The image of a Demazure crystal under this transformation is an opposite Demazure crystal, because the highest weight element is sent to the lowest weight element and the crystal operators are reversed.
% % For any $R\in\BRC_n$, there is a unique factorization
% % $$Y(R) = A\boxtimes G$$
% % and for this factorization we have
% % $$R = R'\boxtimes G$$
% % for some $R'$ with $Y(R') = A$. We then define
% % $$G_1\boxtimes^L R = R'\boxtimes G_1\boxtimes G$$ 

% %Key positive. Symmetric group action? Div diff factorization
% %This is a well-defined left action by the associativity of $\boxtimes$. We may define a bimodule structure using these operations, because
% %$$(G_1\boxtimes^L R)\boxtimes G_2 = G_1\boxtimes^L (R\boxtimes G_2).$$

% %First, we raise $R\boxtimes G_2'$ to its highest weight, which is of the form $Y(R)\boxtimes G_2'$ for some $G_2'$ of the same shape as $G_2$. Then we pick the unique $R'\in\RC_n(S_n)$ and the unique $G$ such that $R'\boxtimes G = Y(R)$. Hence we can compute
% %$$G_1\boxtimes^L Y(R\boxtimes G_2)$$
% %as
% %$$G_1\boxtimes^L R'\boxtimes G\boxtimes G_2'$$
% %Now we perform the following antiautomorphism:
% %$$(G_1\boxtimes^L R'\boxtimes G\boxtimes G_2')^{\#}=(G_1\boxtimes^L R')^{\#}\boxtimes (G_2')^{\#}\boxtimes G^{\#}$$
% %The RHS is equal to 
% %$$(R')^{\#}\boxtimes G_1^{\#}\boxtimes (G_2')^{\#}\boxtimes G^{\#}=G^{\#}\boxtimes^L (G_2')^{\#}\boxtimes^L G_1^{\#}\boxtimes^L (R')^{\#}$$
% %Applying the antiautomorphism again, we get
% %$$R'\boxtimes G_1\boxtimes G_2'\boxtimes G$$

% % bpd transpose union bpd transpose back

% % BPDs have a left transition map $\mathbf{t}^L$. Define
% % $$p\boxtimes R = \phi(\clp_L^n(\phi^{-1}(R*p)^t)^t)$$
% % \begin{theorem}
% %   % Let $R\in \BRC_n$ and $p\in\mathrm{Tab}(n)$ be such that 
% %   % $$R\boxtimes p = A$$
% %   % Then this assignment induces an isomorphism between $\mathcal{C}(A)$ and $\mathcal{C}(R\otimes p)$ as Demazure crystals.

% %   %$\RC_n$ is isomorphic as a Demazure crystal to $\RC_n(S_n)\otimes \mathrm{Tab}(n)$ via the map $R\otimes p\mapsto R\boxtimes p$. In particular, 
% %   $\BRC_n$ is isomorphic as a $\grass_n$-module to 
% %   $$\bigoplus_{R\in \mathbb{Y}(\RC(S_n))} M_{\mathcal{C}(R)}$$
% %   where $M_{\mathcal{C}(R)}$ is the right $\grass_n$-submodule generated by the crystal basis $\mathcal{C}(R)$.
% % \end{theorem}
% % \begin{proof}
% % We have that $\BRC_n\otimes_{\mathbb{Z}} \mathcal{P}_n$ has as its basis the crystal tensor product $\RC(n)\otimes \mathrm{Tab}(n)$. The product $\boxtimes:\RC(n)\otimes \mathrm{Tab}(n)\to \BRC_n$ is weight preserving and restricts to a homomorphism of Demazure crystals on each connected component of $\RC(n)\otimes \mathrm{Tab}(n)$. By the fact that the polynomial ring $\mathbb{Z}[x_1,\ldots,x_n]$ is a free module over the ring of symmetric polynomials $\Lambda_n$ with basis given by the Schubert polynomials $\sch_w(x_1,\ldots,x_{n-1})$ for $w\in S_n$, and that the submodule of $\BRC_n$ generated by $\mathcal{S}_n(w)$ for $w\in S_n$ generated by the subring of $\grass_n$ generated by the Schur elements is isomorphic to $\mathbb{Z}[x_1,\ldots,x_n]$ via the weight map, every RC graph $R\in \RC(S_n)$ has at least one $p\in\mathrm{Tab}(n)$ and one $R'\in \RC_n(S_n)$ such that 
% % $$R'\boxtimes p = R$$
% % %Since the kernel is trivial, $\boxtimes$ is an injective (and surjective, by the above) linear map on this subset, and thus induces an isomorphism of Demazure crystals on each connected component of the basis.
...
Let $R\in \BRC_n(S_n)$. Then $R$ has a unique factorization of code-weight elementary symmetric monomials. Crystal correct. Highest weight/plactic unique. Crystal basis. Weight spaces.
...
% % Now suppose $R_1,R_2\in\RC_n(S_n)$ and $p_1,p_2\in \mathrm{Tab}(n)$ are such that 
% % $$R_1\boxtimes p_1 = R_2\boxtimes p_2 = A$$ 
% % for some $A\in\RC(n)$. Assume without loss of generality that $|R_2| \geq |R_1|$. If they are of equal size, then clearly they are equal. If the inequality is strict, by the below lemma it follows that there is a $G\in\grass_n$ such that
% % $$R_2\boxtimes p_2 = R_1\boxtimes G$$

% % %Assume without loss of generality that $A$ is a highest weight. It follows that $R_1$ and $R_2$ are both highest weight. 
% % % We can back out $A_1,A_2\in \RC_n(S_n)$ and $q_1,q_2\in \mathrm{Tab}(n)$ such that
% % % $$A_1\boxtimes q_1 = R_1$$
% % % and
% % % $$A_2\boxtimes q_2 = R_2$$
% % % so that
% % % $$A_1\boxtimes (q_1\boxtimes p_1) = A_2\boxtimes (q_2\boxtimes p_2)$$
% % % It follows that $A_1\in \mathcal{C}(A_2)$, so the direct sum decomposition holds.
% % \end{proof}

% % \begin{lemma}
% % % Let $R\in \RC_n(S_n)$ and $p\in \mathrm{Tab}(n)$. Then  the unique element $A$ of 
% % % $\RC_n$ such that there is an RC graph $R'$ that is the union of $R$ with a commuting Grassmannian RC graph $P$ with $\hr(P) = 2n - 1$ such that $\mathrm{shape}(P) = \mathrm{shape}(p)$ and the rows of $P$ with row index greater than $n$ are empty satisfying
% % % $$\clp^n(R') = A$$
% % % is $R\boxtimes p$.
% % Suppose $R\in \RC_n$. Then given $A\in \RC_n(S_n)$, there is at most one $G\in \grass_n$ such that
% % $$R = A\boxtimes G$$
% % \end{lemma}
% % \begin{proof}
% % By the isomorphism of crystals $\RC_n(S_n)\otimes \mathrm{Tab}(n)\to \BRC_n$, there is at most one $R'\in \RC_n(S_n)$ and $p\in \mathrm{Tab}(n)$ such that $R = R'\boxtimes p$. If there is no such pair, then there is no such $G$. If there is such a pair, then by the definition of the right module action of $\grass_n$ on $\BRC_n$, we have that $R = R'\boxtimes G$ for some $G\in \grass_n$. Since the map $\RC_n(S_n)\otimes \mathrm{Tab}(n)\to \BRC_n$ is an isomorphism, it follows that $G$ is unique.
% % \end{proof}
...
% % \begin{definition}
% % We define a left action of $\mathcal{P}_n$ on $\BRC_n$ as follows. For $p\in \mathcal{P}_n$ and $R\in \BRC_n$, we define first an involution
% % $${}^{\#}:\RC_n(S_n)\to \RC_n(S_n)$$
% % by sending $\mathrm{eg}(R)\mapsto w_0\mathrm{eg}(R)w_0$ and $\mathrm{tab}(R)\mapsto \mathrm{tab}(R)^{\#}$. For $R\in \BRC_n$, write 
% % $$R = R'\boxtimes p'$$
% % for some $R'\in \RC_n(S_n)$ and $p'\in \mathrm{Tab}(n)$. Then we write
% % $$R^{\#} = (R')^{\#}\boxtimes (p')^{\#}$$
% % The left action of $p\in \mathcal{P}_n$ on $R$ is then defined by
% % $$p\boxtimes R = (R^{\#}\boxtimes p^{\#})^{\#}$$
% % \end{definition}


% % % \begin{proposition}
% % % The function $\tau_k:\bigotimes_{i=k}^n \mathcal{P}_i\otimes \bigotimes_{i=p}^k \mathcal{P}_i\to \BRC_n^{(p)}$ given by $\tau_k(a\otimes b) = \pi(b\boxtimes a)$ is a surjective homomorphism of right $\mathcal{P}_n$-modules.
% % % \end{proposition}


% % % For $A\in \BRC_n^{(p)}$ and $B\in\mathcal{P}_k$, we then define
% % $$A\boxtimes_k B = \mathbf{t}_k^{-1}(A)\boxtimes B$$
...
% Consider the algebra $\dcoma$ in the word basis. There is a homomorphism of rings to the ring of noncommutative symmetric functions $\NSym[t]$ with an indeterminate adjoined given by sending the generator $[0]\mapsto t$ and the remaining generators $[i]\mapsto S_i$. Let $J_\alpha(t)$ be the image of the Schubert basis element $\dsch_w^n$ under this homomorphism, where $\alpha$ is a (strong) composition with $\ell(\alpha)=n$ and
% $$\code(w) = \alpha$$

% \begin{proposition}
% We have that the elements $J_\alpha(t)$ as $\alpha$ ranges over all compositions form a basis of $\NSym[t]$ over $\mathbb{Z}[t]$.
% \end{proposition}
% \begin{proof}
% The basis $\dsch_w^n$ is triangular in the word basis. Thus, the image under the homomorphism of $\NSym$ clearly generates the ring as a module over $\mathbb{Z}[t]$. The leading term of the image of $\dsch_w^n$ where $\code(w) = \alpha$ with $\ell(\alpha)=n$ is $S_\alpha$, the set of which form an additive basis for the ring, from which the claim that they form a basis follows.
% \end{proof}

% To compute the dual basis in $\QSym[t]$, we need to introduce a nonstandard grading. This is a $\mathbb{N}^2$-grading, where the degree of the monomial quasisymmetric function $M_\alpha$ is $(|\alpha|, \ell(\alpha))$. The indeterminate $t$, rather than contributing to the first component of the degree, contributes to the second component. Thus, the dual basis element $t^k\mathcal{J}_\alpha(t)$ is sent to a polynomial of degree $|\alpha|$ in $\coma_{k+\ell(\alpha)}$ under the homomorphism dual to the one above.

% \begin{proposition}
%   Under the injective homomorphism $\QSym[t]\to\coma$, the element $t^kM_\alpha$ is sent to $M_{\alpha}(x_1,\ldots,x_k)$ in $\coma_{k + \ell(\alpha)}$.
% \end{proposition}

% Consider $\BRC$ as a left module over $\dcoma$. The homomorphism $\dcoma\to \NSym[t]$ induces a homomorphism to a quotient module 
% $$\BRC\to\widetilde{BRC}$$
% We can interpret the elements of $\widetilde{\BRC}$ as sets of ordered pairs $(i, j)$ and an associated height $\hr(R)$ as before. However, the following requirements are imposed:
% \begin{enumerate}
% \item If $\hr(R)=n$, then for each $i$ with $1\leq i\leq n$ there exists at least one $j$ such that $(i, j)\in R$.

% \item There exists a $k$ such that there is a connected permutation $w$ for which
% $$\wof{R} = \shup^k(w)$$
% \end{enumerate}
% % Consider the submodule over $\NSym[t]$, where $t$ acts on the left only as $[0]$, defined by
% % $$B = \NSym[t]R_\varnothing$$ 
% % meaning it is generated by the empty RC graph. Consider also the graded submodule $T$ generated over $\NSym[t]$ by 
% % $$\shup^{k}R - t^kR$$
% % where $\hr(R)=n$ and $\wof{R}$ is connected,
% % $$\code_i(\wof{R}) = 0$$
% % for $1\leq i\leq k$, and $\code_i(\wof{R}) > 0$ for $k<i\leq n$. Then we define
% % $$\widetilde{\BRC} = B/T$$

% \newcommand{\SSYT}{\mathrm{SSYT}}

% \begin{definition}
% The \emph{monomial slide polynomials} $\mathfrak{M}_\alpha$ are defined by
% $$\mathfrak{M}_\alpha = \sum_{\beta\geq \alpha} M_\beta$$
% where $\beta\geq \alpha$ means that $\beta$ is a composition of the same size as $\alpha$ that is greater than $\alpha$ in the dominance order.
% \end{definition}

% \section{Forest polynomials}

% \newcommand{\pf}{\mathfrak{P}}

% \begin{theorem}
% Let $\mathcal{C}$ be an $\Omega$-invariant of an RC graph, say for a permutation $w$, such that $\code(\mathcal{C}) = F$. Then
% $$\pf_F = \sum_{\substack{R\in\RC(w)\\\Omega(R) = \mathcal{C}}} \wtt(R)$$
% \end{theorem}

% \begin{theorem}
% For each permutation $u$ and each $\Omega$-class $\mathcal{C}$ of $\RC(u)$, fix a representative $R_u(\mathcal{C})$. Suppose that
% $$F=\code(\mathcal{C})$$
% for some $\mathcal{C}=\Omega(R_w(\mathcal{C}))$ for some permutation $w$. For an integer $k>0$, write
% $$\pf_F(x) = \sum_{F_L, F_R}d_{F_LF_R}^F(k)\pf_{F_L}(x_1,\ldots,x_k)\pf_{F_R}(x_{k+1},\ldots)$$
% Then $d_{F_LF_R}^F(k)$ is the number of $R\in \RC(w)$ such that $\Omega(R) = \mathcal{C}$, 
% $\clp^k(R) = R_u(\mathcal{C}_L)$ for some $\mathcal{C}_L$ with $\code(\mathcal{C}_L) = F_1$, 
% and $\trm^{k}(R) = R_v(\mathcal{C}_R)$ for some  $\mathcal{C}_R$ with $\code(\mathcal{C}_R) = F_R$.
% \end{theorem}

% \begin{theorem}
% Let $R\in\RC$. Let $W_R:\mathrm{word}(R)\to\overline{Z}$ be the canonical injective labeling and let $\rho_R:P(W_R)\to\mathbb{Z}$ be the associated compatible sequence labeling. Then the map $R\mapsto \rho_R$ is a bijection between RC graphs and polynomial maps on reduced-word labeled LBS with compatible labelings.
% \end{theorem}
% \begin{proof}
% Suppose we start with a reduced word $W$. Then $P(W)$ and $Q(W)$ are uniquely determined. A characterization of the equivalence class obtained by varying $Q(W)$ is all words arising from $P(W)$ by following a linear extension of the poset $F(W)$. Let $u,v \in F(W)$ be incomparable. Then we claim that $|P_W(u)-P_W(v)| > 1$. (Proof) Thus, all words in the equivalence class $P(W_R)$ are reduced words for the same permutation. 

% By the characterization of $P(W)$ as an LBS, the maps $\rho$ defining $\pf_{F(W_R)}$ are bounded above by $P(W)$, weakly decreasing if the letter increases, and strictly decreasing if the letter decreases.
% \end{proof}

% \begin{theorem}
% Let $F$ be an indexed forest and let $w$ be the permutation such that $\code(w)=\code(F)$. Then
% $$\pf_F = \sum_{\substack{R\in\RC(w)\\P(R)=P(R_0(w))}} \wtt(R)$$
% where $\delta$ is the Kronecker delta.
% \end{theorem}

% \begin{theorem}
% Suppose that $B_\alpha$ is a basis of $\dcoma$ that is a nonnegative combination of dual Schubert elements. Then for each $\alpha$ there are unique nonnegative integers $c_\alpha^R$ for $R\in \BRC$ satisfying the condition that for each permutation $w$, $c_\alpha^R$ is nonzero for at most one $R\in \RC(w)$, such that the elements
% $$\mathcal{B}_\alpha= \sum c_\alpha^R R$$
% generate a subalgebra of $\BRC$ that projects isomorphically onto $\dcoma$ under the homomorphism sending $R$ to $\dsch_{\wof{R}}^n$ where $n=\hr(R)$.
% \end{theorem}

% %then the dual basis $F_\alpha^*$ of $\NSym$ has nonnegative coefficients in the $B_\alpha$ basis, and the elements

% \begin{definition}
% Define a function $\iota:\dcoma\to \BRC$ by sending $[a]$ to the unique RC graph with one row and degree $a$, extending by freeness to a homomorphism of rings. Recall the homomorphism $\alpha:\BRC\to \dcoma$ sending $R$ to $\dsch_{\wof{R}}^n$ where $n=\hr(R)$.
% \end{definition}

% \begin{lemma}
% The function $\iota$ is a section of $\alpha$, meaning that $\alpha\circ \iota$ is the identity on $\dcoma$.
% \end{lemma}
% \begin{proof}
% By virtue of the fact that $\alpha$ is a homomorphism, for any $\Xi_w^1$ we have that $\alpha(\iota(\Xi_w^1)) = \Xi_w^1$. Since the $\Xi_w^1$ form a generating set of $\dcoma$, the claim follows.
% \end{proof}

% \begin{definition}
% Consider an element $D\in\dcoma$. Then $\iota(D)$ is a linear combination of RC graphs. Assume $D$ is homogeneous of length $n$. Let 
% $$D=\sum_w c_w \Xi_w^n$$
% where $c_w$ are nonnegative integers. Then
% $$\iota(D)^+ = \sum_{w} c_R R$$
% such that
% $$\sum_{R\in\BRC(w,n)} c_R = c_w$$


% \end{definition}

% Define the forest basis as $P_F = \iota(\pf_F)^+$.

% $$\Delta(s) = \Delta(\mathrm{flat}(s)) + (\mathrm{flat}(s) - s)\otimes 1 + 1\otimes (\mathrm{flat}(s) - s)$$

% \begin{proposition}
% An irreducible forest polynomial $\pf_F$ is a Schubert polynomial if and only if $\pf_F$ is a Schur polynomial.
% \end{proposition}
...
% $$\sch_w(x;y) = \sum_{u,v} c_{u\mu_A,\mu_B}^{w\mu}(-y,-x_B)\sch_u(x_A;y)$$

% avoids 2341 and 213

% Let $A$ be a finite set of positive integers. Then an $A$-double Schubert polynomial is defined as follows. 
% Define
% Then we define
% $$\sch_{w_0(n)}(x;y)^A = \prod_{p=1}^{|A|} E_{n + 1 - A_p}(x;y_{p})\prod_{q\in [n]\setminus A}E_{n + 1 - q}(x)$$
% Then
% $$\sch_w(x;y)^A = \partial^{w^{-1}w_0(n)} \sch_{w_0(n)}(x;y)^A$$

% $$\sch_{w_0(n)_A}(x;y) = \sum_{v}\sch_{vw_0(n)_A}(x)\sch_v(y)$$
% $$\sch_{w_0(n)}(x;y)^A = \sum_{u}\sch_{uw_0(n)}(x)c_{vw_0(n)_A,w_0(n)_B}^{uw_0(n)}\sch_{v}(y)$$

% $$\sch_w(x;y)^A = \sum_{\ell(uw) = \ell(w)-\ell(u)} \sch_{uw}(x)c_{vw_0(n)_A,w_0(n)_B}^{uw_0(n)}\sch_v(y)$$
% $$\sch_w(x;y)= \sum_{\ell(uw) = \ell(w)-\ell(u)} \sch_{uw}(x)c_{v_1w_0(n)_A,v_2w_0(n)_B}^{uw_0(n)}\sch_{v_1}(y_A)\sch_{v_2}(y_B)$$
% $$\sch_w(x;y)= \sum_{\ell(uw) = \ell(w)-\ell(u)} \sch_{uw}(x)c_{v_1w_0(n)_A,v_2w_0(n)_B}^{uw_0(n)}\sch_{v_1}(y_A)\sch_{v_2}(z)$$
% Div diff $y_B$, $y_B=0$

% Define $w_A$, $w_B$? Homomorphism of weak order. PATTERN

% \begin{theorem}
%   Let $w\in S_n$ be a permutation and let $A,B$ be complementary subsets of $[n]$. Then
% $$\sch_w(x;y) = \sum_{u,v} c_{u\mu_A,v\mu_B}^{w\mu}(-y,-z)\sch_u(x_A;y)\sch_v(x_B;z)$$
% \end{theorem}

% Odd and even coincides with the cut coproduct.

% $O_n$ and $E_n$, we have
% $$\sch_w(x) = \sum_{u,v} c_{u\mu_{[p]},v\mu_{[p+1, p+q]}}^{w\mu}\sch_u(x_{[p]})\sch_v(x_{[p+1, p+q]})$$
% Same variables! RC graph deletion property.

% Suppose $R\in \BRC_n(u)$ and $\mu$ is a dominant permutation such that $\ell(u\mu) = \ell(\mu) - \ell(u)$ and $\maxd(\mu) = n$. Then we define a unique RC graph $S\in \BRC_n(u\mu)$ recursively as follows.

% Deletion of roots from $\mu$ in row order.


% \begin{theorem}
% Suppose $D\in \dcoma_n$ is such that
% $$D=\sum_\alpha c_{\alpha}[\alpha]$$
% then
% $$D = \sum_\alpha\sum_{R\in \RC_{\alpha}} c_{\alpha}\dsch_{\wof{R}}^n$$
% \end{theorem}

% Suppose
% $$\Delta(\dsch_w^n) = \sum_{\alpha=\alpha_1+\alpha_2} \langle \dsch_w^n,[\alpha]\rangle[\alpha_1]\otimes [\alpha_2]$$
% Then
% $$\Delta(\dsch_w^n) = \sum_{\alpha=\alpha_1+\alpha_2}\sum_{R_1\in\RC_n(\alpha_1),R_2\in\RC_n(\alpha_2)} \langle \dsch_w^n,[\alpha]\rangle\dsch_{\wof{R_1}}^n \otimes \dsch_{\wof{R_2}}^n$$
% % Grassmannian | Grassmannian | empty

% For the coefficient
% $$c_{u\mu_A,v\mu_B}^{w\mu}$$
% If $A_i$, $A_{i+1}$ are adjacent and $u(i) > u(i+1)$, then $w(A_i) > w(A_{i+1})$ and
% $$c_{uw_0(n,A),vw_0(n,B)}^{ww_0(n)} = c_{us_iw_0(n,A),vw_0(n,B)}^{ws_{A_i}w_0(n)}$$
% %If $\mu_A=w_0(n)$ for some $n$ and $u(i) > u(i+1)$, then we may assume $A_{i+1} = A_i + 1$.
% % \begin{lemma}
% %   We have
% % %$$\sch_w(x;y)^A=\sum_{u\leq_L w} \sch_u(x)d_{}^{uw^{-1}}(A)\sch_{v}(y)$$
% % %$$E_p(x;t)^A = \sum_{u\leq_L w_0(p)} e_{i,p}t^{\#\{j\in A\mid )}$$
% % \end{lemma}
% % \begin{proof}
% % We have that
% % $$e_{k}(x_1,\ldots,x_p) = \sum_{0\leq i\leq k} e_i(x_A) e_{k-i}(x_{[p]\setminus A})$$
% % and
% % $$E_p(X;t)^A = \sum_{i\geq 0} e_i(x_1,\ldots,x_p) t^{p - i}$$
% % Combining these, we get
% % $$E_p(x;t) = \sum_{i\geq 0} t^{p - i}\sum_{0\leq j\leq i} e_{i-j}(x_A) e_{j}(x_{[p]\setminus A}) $$
% % Interchanging the order of summation, we get
% % $$E_p(x;t) = \sum_{j\geq 0} e_{j}(x_{[p]\setminus A}) t^{j}\sum_{i\geq j} e_{i-j}(x_A)  $$
% % Hence,
% % $$E_p(x;t)^A = \sum_{j\geq 0} \sum_{i\geq j} e_j(x_A)  e_{i-j}(x_{[p]\setminus A}) t^{p - i - j}$$
% % %where $e_{k,u}$ is the number of RC graphs for $w_0(p)$ with $k$ crosses in the first row and no crosses in rows with index greater than $1$. Hence
% % % Consider 
% % % $$\sch_w(x;y) = \sum_{u\leq_L w} \sch_u(x)\sch_{uw^{-1}}(y)$$
% % % which is equal to
% % % $$\sum_{u\leq_L w} \sch_u(x)d_{v',v}^{uw^{-1}}(A)\sch_{v}(y_A)$$
% % \end{proof}

% % \begin{proposition}
% % Let $B=A^c$. Suppose
% % $$\sch_w(x) = \sum_{u,v} c_{u\mu_A,v\mu_B}^{w\mu}\sch_u(x_A)\sch_v(x_B)$$
% % Then
% % $$\sch_w(x;y)^A = \sum_{u,v} c_{u\mu_A,v\mu_B}^{w\mu}\sch_u(x_A;y)\sch_v(x_B)$$
% % \end{proposition}
% % \begin{proof}
% %   We have

% % $$\sch_{w_0(n)}(x;y) = \sch_{w_0(n)_A}(x;y_A)\sch_{w_0(n)_B}(x;y_B)$$
% % $$\sch_{w_0(n)}(x;y) = \sch_{{{}_{A}}w_0(n)_A}(x_A;y_A)\sch_{{{}_{B}}w_0(n)_A}(x_B;y_A)\sch_{{{}_{A}}w_0(n)_B}(x_A;y_B)\sch_{{{}_{B}}w_0(n)_B}(x_B;y_B)$$
% % \end{proof}

% Two variable word basis:
% $$\dsch_{[p,q]}^2 = [pq] - [qp]$$
% if $p>q$. If $p\leq q$, then
% $$\dsch_{[p,q]}^2 = [pq] - [p - 1, q + 1]$$
% and hence
% $$\dsch_{[p,q]}^2 = [pq] - \dsch_{[p-1,q+1]}^2 + [p-2, q+2]$$
% Thus
% $$\dsch_{[p,q]}^2 = [pq] - \dsch_{[qp]}^2 + [q - 1, p + 1]$$
% if $p>q$.
% Actually
% $$\dsch_{[p,q]}^2 = \sum_{i=0}^{p} (-1)^i \dsch_{[p - i, q + i]}^2 + [0, p + q]$$
% if $p\leq q$, and
% $$\dsch_{[p,q]}^2 = [pq] + \sum_{i=0}^{q} (-1)^{i+1} \dsch_{[q - i, p]+ i]}^2 - [0, p + q]$$

% We compute the coproduct:
% \begin{align*}
% \Delta(\dsch_{[p,q]}^2) &= \sum_{a=0}^p\sum_{b=0}^q [p - a, q - b]\otimes [a, b] \\
% &\quad - \Delta(\dsch_{[p-1,q+1]}^2) + \sum_{r=0}^{p-2}\sum_{s=0}^{q+2}[p-2 - r, q+2 - s]\otimes [r, s]
% \end{align*}
% for $p\leq q$, and
% \begin{align*}
% \Delta(\dsch_{[p,q]}^2) &= \sum_{a=0}^p\sum_{b=0}^q [p - a, q - b]\otimes [a, b] \\
% &\quad - \sum_{r=0}^q\sum_{s=0}^p [q - r, p - s]\otimes [r, s]\\
% &\quad + \Delta(\dsch_{[q-1,p+1]}^2) \\
% &\quad - \sum_{t=0}^{q-2}\sum_{u=0}^{p+2}[q-2 - t, p+2 - u]\otimes [t, u]
% \end{align*}
% for $p>q$. First formula, highest weights of the tensor product? Second formula, we substitute the first and expand:
% \begin{align*}
% \Delta(\dsch_{[p,q]}^2) &= \sum_{a=0}^p\sum_{b=0}^q [p - a, q - b]\otimes [a, b] \\
% &\quad -\sum_{r=0}^q\sum_{s=0}^p [q - r, p - s]\otimes [r, s]\\
% &\quad + \mathrm{LW}(q-1,p+1) \\
% &\quad - \sum_{t=0}^{q-2}\sum_{u=0}^{p+2}[q-2 - t, p+2 - u]\otimes [t, u]
% \end{align*}
...
% % \Delta(\dsch_{[p,q]}^2) &= \sum_{a=0}^p\sum_{b=0}^q [p - a, q - b]\otimes [a, b] \\
% % &\quad \sum_{r=0}^q\sum_{s=0}^p [q - r, p - s]\otimes [r, s]\\
% % &\quad - \sum_{a'=0}^{q-1}\sum_{b'=0}^{p + 1} [q - 1 - a', p + 1 - b']\otimes [a', b'] \\
% % &\quad + \Delta(\dsch_{[q-2,p + 2]}^2)\\
% % &\quad - \sum_{r'=0}^{q-3}\sum_{s=0}^{p+3}[q-3 - r', p+3 - s']\otimes [r', s'] \\
% % &\quad + \sum_{t=0}^{q-2}\sum_{u=0}^{p+2}[q-2 - t, p+2 - u]\otimes [t, u]
% % \end{align*}
% \begin{lemma}
% Suppose $\alpha$ is a word that is not a dominant weight. Let $T$ be the set of RC graph tensors in $A^{-1}(\Delta(\alpha))$. Let $i$ be the first index such that $\alpha_i < \alpha_{i+1}$. Then there is permutation-preserving injection
% $$T\hookrightarrow A^{-1}(\Delta(R_i\alpha))$$
% \end{lemma}
% \begin{proof}
% Every such tensor $R_1\otimes R_2$ satisfies $e_i(R_1\otimes R_2)\neq \varnothing$. We can define a function $T\to A^{-1}(\Delta(R_i\alpha))$ sending $R_1\otimes R_2$ to $e_i(R_1\otimes R_2)$. Injectivity follows from the fact that $f_i(e_i(R_1\otimes R_2)) = R_1\otimes R_2$.
% \end{proof}

% Tensors that survive co-raising. 


% The map $\omega:\BRC\to\dcoma$ intertwines with crystal operators.

\section{Acting up}

\begin{definition}
  Let $R\in \BRC_n$. The squash product allows us to multiply associatively on the right by the plactic monoid $\grass_n$. If $R$ is a highest weight element, we may uniquely factorize $R$ into $R'\boxtimes p$ for some $R'\in \RC_n(S_n)$ and $p\in \mathrm{Tab}(n)$. For a highest weight element $R'$, it is therefore unambiguous to define
  $$p\boxtimes R' = R'\boxtimes \mathrm{rev}(p)$$
  This is a free bimodule action of $\grass_n$ on the submodule generated over $\grass_n$ by the highest weight elements of $\RC_n(S_n)$. By standard theorems, these generate a crystal basis of $\BRC_n$ as a right $\grass_n$-module, and the left action of $\grass_n$ on this submodule is determined by the right action and the crystal structure, where we reverse the structure of the tensor product when reversing the factors. Only the right tensor product behaves properly with the Demazure decomposition.
\end{definition}

We know that we may factorize an element of $\RC_n(S_n)$ uniquely as a product of elementary monomials whose degrees are determined by the code of the permutation. There is a right action by $\grass_{n-1}$ defined as follows. Writing
$$R = R'\boxtimes p$$
and letting $q\in\grass_{n-1}$ be a degree $1$ element, we define
$$R \cdot q = \mathrm{resize}_n((\mathrm{resize}_{n-1}(R')\boxtimes q)\boxtimes p$$
It is important for this construction that the left action be well defined, because after acting on the right by $q$ the RC graph is no longer in $\RC_n(S_n)$. When we resize, therefore, we must be able to renormalize to the plactic-elementary basis, then we may multiply the original plactic factor on the left by what was pulled out on the right.

To multiply by an element of $\grass_{n-k}$, we need to decompose into smaller RC graphs $k$ times, and then we can apply the right action by $\grass_{n-k}$ to the smallest factor. We can then reassemble the factors successively as above.

\section{Elementary monomial products}

Let $\mathbb{Z}\langle E\rangle$ be the free associative algebra generated by elementary monomials $M_A^n$ for $A\in \mathbf{2}^n$ for $n\geq 1$. Each of these can be canonically associated with an RC graph in $\BRC_n$, namely the RC graph for the elementary symmetric polynomial $e_{|A|,n}$ with weight $x_A$. We apply the following relations: for elements $A_1,\ldots,A_p,B_1,\ldots,B_q\in\mathbf{2}^n$, we identify
...
Furthermore, if $R\in \BRC_{n-1}(w)$ with $\maxd(w)=n$, $w$ Grassmannian, and $w\notin S_n$, assume first that $R$ is a highest weight. In that case we identify
$$R\sim R' M_A^n$$
for the unique $A\in \mathbf{2}^n$ and $R'\in \BRC_{n-1}(S_n)$ such that if we resize $R'$ to have $\hr(R')=n$, then
$$R' \boxtimes M_A^n = R$$
...
\begin{lemma}
Suppose $M\in\mathcal{E}$ is some product of $M_A^n$ for various $A$ and $n$. Then there are sets $A_1,\ldots,A_N$ for some $N$ such that
$$M = M_{A_1}^1\cdots M_{A_N}^N$$
\end{lemma}
\begin{proof}
It is an easy intermediate result that there are Grassmannian RC graphs $R_1,\ldots,R_m$ with $\maxd(R_i) = i$ for some $m$ such that
$$M=R_1\cdots R_m$$
...
for some $N$ and $p_1\leq \cdots\leq p_N$. There exists such an $N$ that is maximal since the weight is finite. By definition of the relation, it follows that each product with equal values $p_i,p_{i+1},\ldots$ is equivalent to an RC graph $R$ with $\maxd(R) = p_i$ and $w(R)\in S_{p_i + 1}$. If $w(R)$ were not $p_i$-Grassmannian, further reduction would be possible. The $p_i$-Grassmannian elements of $S_{p_i+1}$ are precisely the $p_i$-elementary symmetric cycles, so it follows that in fact $R=M_A^{p_i}$ for some $A\in \mathbf{2}^{p_i}$, so that the $p_i$ are all distinct and the claim follows.
\end{proof}
...
\begin{lemma}
  The subalgebra of $\mathcal{E}$ generated by the $\mathbf{e}_p^k$ is a commutative ring isomorphic to $\mathbb{Z}[x]$ via the restriction of the homomorphism $M_A^n\mapsto x_{\wtt(M_A^n)}$.
\end{lemma}

It is tempting to try to define a product on $\BRC$ by using the obvious linear surjection $\mathcal{R}:\mathcal{E}\to\BRC$ given by the squash product. However, the kernel of the linear map is not a left ideal or a right ideal.
\begin{example}
  This is the representation of the Schubert polynomial $\sch_{1432}$ in $\mathcal{E}$:
\begin{align*}
&\left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] + \left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & 2 & \mathtt{\text{ }}\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] + \left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & 2 & \mathtt{\text{ }}\\\mathtt{\text{ }} & 3 & \mathtt{\text{ }}\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \\
&+ \left[\begin{matrix}\mathtt{\text{ }} & 2\\\mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] + \left[\begin{matrix}\mathtt{\text{ }} & 2\\\mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & 2 & \mathtt{\text{ }}\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] + \left[\begin{matrix}\mathtt{\text{ }} & 2\\\mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & 2 & \mathtt{\text{ }}\\\mathtt{\text{ }} & 3 & \mathtt{\text{ }}\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \\
&- \left[\begin{matrix}1 & \mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]
\end{align*}
The two terms
$$-\left[\begin{matrix}1 & \mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
and
$$\left[\begin{matrix}\mathtt{\text{ }} & 2\\\mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right] \otimes \left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
cancel upon taking squash products. However, multiplying the first term on the left by 
$$\left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }} & 3 & 4\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\\mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
yields, after squashing,
$$-\left[\begin{matrix}1 & \mathtt{\text{ }} & \mathtt{\text{ }} & 4 & 5\\2 & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
whereas the second term yields
$$\left[\begin{matrix}1 & \mathtt{\text{ }} & 3 & 4\\2 & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
which no longer cancel. Thus, the kernel of the RC graph map is not a left ideal. Multiplying on the right by 
$$\left[\begin{matrix}\mathtt{\text{ }} & \mathtt{\text{ }} & 3\\2 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right],$$
for the first term, is the same as multiplying on the left and yields
$$-\left[\begin{matrix}1 & \mathtt{\text{ }} & \mathtt{\text{ }} & 4\\2 & 3 & \mathtt{\text{ }} & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
whereas the product on the right for the second term is
$$\left[\begin{matrix}1 & 2 & \mathtt{\text{ }}\\2 & 3 & \mathtt{\text{ }}\\3 & \mathtt{\text{ }} & \mathtt{\text{ }}\end{matrix}\right]$$
...
\begin{theorem}
We have that $\mathcal{R}:\mathcal{E}\to \BRC$ is a coalgebra homomorphism, where $\BRC$ is equipped with the cut coproduct. 
\end{theorem}
\begin{proof}
This is clear for $\mathcal{E}_1$. Using the fact that $\BRC_n$ is a free right module over $\grass_n$ up to crystal action, and given that the same is true for $\mathcal{E}_n$, we can use the crystal structure to deduce that $\mathcal{R}$ is a coalgebra homomorphism on $\mathcal{E}_n$ for all $n$.
\end{proof}

We can then define the dual algebra $\mathcal{E}^*$ with respect to the coproduct, and we have a homomorphism of algebras $\mathcal{R}^*:\BRC\to \mathcal{E}^*$.
Product on $\BRC^*$: $f:\BRC\to \mathbb{Z}$, $g:\BRC\to \mathbb{Z}$, then
...
% $$\sch_w = \sum_{A_1,\ldots,A_n} c_w^{A_1,\ldots,A_n}M_{A_1}^1\cdots M_{A_n}^N$$
% where $\sch_w$ is defined by its expansion in terms of elementary symmetric monomials. Then we define elements
% $$R^{(w)} = \sum_{A_1,\ldots,A_n} c_w^{A_1,\ldots,A_n}M_{A_1}^1\cdots M_{A_n}^N$$
% where the sum is over all $A_1,\ldots,A_n$ such that 
% $$M_{A_1}^1\boxtimes\cdots \boxtimes M_{A_n}^N=R$$

% We define the RC-elementary monomial algebra $M_n$ as follows. Consider the kernel
% $$\sum_{\alpha} \mathcal{E}_{n - i - \alpha_i}^{n - i}\otimes\cdots\otimes \mathcal{E}_{1 - \alpha_{n - 1}}^1\otimes x_\alpha$$
% The sum being over all $\alpha$ such that $0\leq \alpha_i\leq n - i$ for $1\leq i\leq n-1$. Change basis to the Schubert polynomials
% $$\sum_{w,\alpha} a_w^\alpha\mathcal{E}_{n - i - \alpha_i}^{n - i}\otimes\cdots\otimes \mathcal{E}_{1 - \alpha_{n - 1}}^1\otimes \sch_{ww_0(n)}$$
% Make the identification
% $$\sum_{\alpha,M\in c(\alpha), \mathrm{squash}(M)=R} a_w^\alpha M \sim R$$
% if $R\in \RC(w)$.

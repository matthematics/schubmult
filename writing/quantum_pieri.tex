...
Coupled with the Pieri formulas, these formulas give a relatively efficient method for computing products of (quantum) double Schubert polynomials. We describe the resulting algorithm precisely and record its asymptotic complexity in the classical (non-quantum) case.
...
$$L:=\#\{i\mid\theta_i\neq 0\}$$
be the number of nonzero parts. The algorithm proceeds in $L$ layers. It maintains a \emph{front}, a collection of nodes each consisting of an intermediate permutation on the $u$-side paired with a label on the $v$-side, initialized to the single node $(u,\,e)$. At layer $i$ it replaces each node by the nodes obtained by combining one $u$-side factorial-elementary Pieri cover (as in Theorem \ref{theorem:orddouble}) with one $v$-side path step $v_{i-1}\xRightarrow{i}v_i$, accumulating coefficients in the coefficient ring. After all $L$ layers the surviving coefficients at $v$-label $v\dom(v^{-1})$ give the structure constants.
\end{method}
...
\begin{proof}
By Method~\ref{method:schubmult} the algorithm runs in $L=\#\{i\mid\theta_i(v^{-1})\neq 0\}\leq\min(n-1,\ell_v)$ layers. The distinct $u$-side permutations that ever appear all satisfy $w\geq u$ and $\ell(w)\leq \ell_u+\ell_v$, since each layer raises the length by the corresponding part of $\theta$ and the parts sum to $\ell_v$; hence there are at most $P_u$ of them. The $v$-side labels lie in the lower interval $[e,v]$, of which there are at most $2^{\ell_v}$ by the subword property. Each layer performs work bounded by the product of the front size, the number of live $v$-labels, and the two per-step branchings, each of which is $\mathrm{poly}(n)$; multiplying by $L$ layers gives the stated bound. Substituting $P_u\leq n!\leq 2^{n\log_2 n}$ and $\ell_v\leq\binom{n}{2}$ yields the worst-case form. If $v$ is dominant then $\theta(v)=\code(v)$, every path step carries a $+1$ sign, and each layer's $v$-side is a single path, so the factor $2^{\ell_v}$ is replaced by $O(\ell_v)$ and no term is produced and later canceled, giving $O(\ell_v\cdot P_u\cdot\mathrm{poly}(n))=O(2^{\,n\log_2 n})$.
\end{proof}

\begin{remark}
In the quantum case the $v$-side bound is unchanged: the transition formula of Theorem \ref{theorem:quandouble} uses the same increasing-path structure (via $\mom$ and $\thop$), so the number of $v$-labels is still at most $2^{\ell_v}$. What changes is the $u$-side factor $P_u$. In the quantum Bruhat order the Pieri covering moves can \emph{decrease} length, so the intermediate permutations reachable from $u$ are no longer confined to the interval $\{w\geq u\mid \ell(w)\leq\ell_u+\ell_v\}$, and a single permutation may be reached at several distinct $q$-degrees. The $u$-front is thus a \emph{multiset} of visits, indexed by $q$-degree, rather than a set. This does not reintroduce cycles: each length-decreasing move is compensated by a strictly higher power of $q$, so grading the transition graph by $q$-degree (equivalently, by the quantum Bruhat order) keeps it acyclic. A correct quantum bound therefore replaces $P_u$ by a count of these $q$-graded $u$-front visits with multiplicity, the powers of $q$ that appear limiting how far the length may descend and re-ascend. We defer the precise quantum estimate.
\end{remark}

...
\begin{definition}[$\tom{k}$, $P_k(u,w)$, $q_k(u,w)$, $E_{p,k}(y;z)$, $\wte_k(y_A;z)$] \label{definition:pieri}
...
If $A=\{a_1<\cdots<a_r\}$ is a set of positive integers with $r\leq k$. We define
$$\wte_k(y_A;z)=E_{r,k}(y_{a_1},\ldots,y_{a_r};z_1,\ldots,z_{k + 1 - r})$$
which will be used in the statement of the Pieri formula to express coefficients.
\end{definition}
...
$$\qsch_u(x;y)E_{p,k}^q(x;z)=\sum_{u\tom{k} w}q_k(u,w)\wte_k(y_{P_k(u,w)};z)\qsch_w(x;y)$$
\end{theorem}
...
The first recurrence is the quantization of the classical relation $E_{p,k}=(x_k-z_{k-p+1})E_{p-1,k-1}+E_{p,k-1}$; the extra term $q_{k-1}E_{p-2,k-2}^q$ is the only feature absent from the non-quantum setting. Discarding all quantum terms recovers the classical Pieri formula \cite[Theorem~7.1]{samuelmolev}, whose term-by-term cancellation is carried out in \cite[Proof of Proposition~4.4]{samuelmolev} by means of the reflection-matching Lemmas 7.6--7.9 of that paper; we adapt that argument, the essential new points being the length-decreasing steps permitted by Lemma \ref{lemma:piericycle} and the attendant bookkeeping of the powers of $q$.

\begin{proof}[Proof of Theorem \ref{theorem:pieri}]
For a permutation $w$, write $[\qsch_w]F$ for the coefficient of $\qsch_w(x;y)$ in an expansion $F$ of a quantum double Schubert polynomial in the basis $\{\qsch_w(x;y)\}$. With this notation the theorem is the assertion that, for all $u$ and all $0\leq p\leq k$,
$$[\qsch_w]\big(\qsch_u(x;y)E_{p,k}^q(x;z)\big)=\begin{cases}q_k(u,w)\,E_{p-n_k(u,w),\,k-n_k(u,w)}(y_{P_k(u,w)};z) & \text{if }u\tom{k} w,\\[2pt] 0 & \text{otherwise.}\end{cases}$$
For brevity, when $u\tom{m} w$ we set
$$c_m(a;u,w)=q_m(u,w)\,E_{a-n_m(u,w),\,m-n_m(u,w)}(y_{P_m(u,w)};z),$$
and $c_m(a;u,w)=0$ if $u\ntom{m}w$; note that $c_m(a;u,w)=0$ whenever $a<n_m(u,w)$ or $a>m$, since then the factorial elementary factor vanishes. The claim is that $[\qsch_w](\qsch_uE_{p,k}^q)=c_k(p;u,w)$.
...
\qsch_uE_{p,k}^q=(x_k-z_{k-p+1})\big(\qsch_uE_{p-1,k-1}^q\big)+\qsch_uE_{p,k-1}^q+q_{k-1}\,\qsch_uE_{p-2,k-2}^q.
\end{equation}
By the inductive hypothesis, $\qsch_uE_{p-1,k-1}^q=\sum_v c_{k-1}(p-1;u,v)\,\qsch_v$. Applying Lemma \ref{lemma:singlevariable} with $i=k$, $j=k-p+1$ to each $\qsch_v$ and extracting the coefficient of $\qsch_w$ from \eqref{eq:pieriexpand}, we obtain
\begin{align}
[\qsch_w]\big(\qsch_uE_{p,k}^q\big)
...
In all cases $[\qsch_w](\qsch_uE_{p,k}^q)=c_k(p;u,w)$, completing the induction and the proof.
\end{proof}
...
\begin{definition}
	...
	$$\qsch_{d_2[1]}(x;y) = E_{1,2}^q(x;y)E_{1,1}^q(x;y) - E^q_{2,2}(x;y)$$
...
\begin{theorem}
	...
	$$\sum_{u\dtom{k} w} \sum_{v\in\mathrm{pivot}_k(u,w)}q_k(u,v)q_k(v,w)E_{k-n_k(u,v)}(y_{P_k(u,v)};z_1)E_{k-n_k(v,w)}(y_{P_k(v,w)};z_2)\qsch_w(x;y).$$
\end{theorem}

...
\section{Efficient multiplication of (quantum) double Schubert polynomials}
...
Coupled with the Pieri formulas, these formulas give a relatively efficient method for computing products of (quantum) double Schubert polynomials. We describe the resulting algorithm precisely and record its asymptotic complexity in the classical (non-quantum) case. To do so we build it up from its innermost primitive, the elementary (vertical-strip) Pieri step that generates the $u$-side covers at each layer.
...
Applied without the edge $\mu$, the signed descending expansion produces many extraneous terms that ultimately cancel. The edge is exactly the bound that discards them: only endpoints beneath $\mu$ are retained in $D$. In the assembled $v$-side (Method~\ref{method:vpaths} below) only these retained labels seed the next column, so the edge prevents the number of live paths from blowing up across columns.
...
\begin{definition}[$\ell_u$, $\ell_v$, $P_u$]
...
$$O\!\left(\,\min(n-1,\ell_v)\cdot P_u\cdot 2^{\ell_v}\cdot\mathrm{poly}(n)\,\right)$$
operations in the coefficient ring, which is $O\!\left(2^{\,\binom{n}{2}+n\log_2 n}\right)$ in the worst case, and $O\!\left(2^{\,n\log_2 n}\right)$ when $v$ is dominant.
\end{theorem}
\begin{remark}
...
using $\ell(v\mu)=\ell(\mu)-\ell(v)$ (as $v^{-1}\leq\mu$ in weak order). Thus every intermediate $u$-side permutation satisfies $w\geq u$ and $\ell(w)\leq\ell_u+\ell_v$, so there are at most $P_u$ of them. The $v$-side labels lie in the lower interval $[e,v\mu]$; since $\ell(v\mu)=\ell(\mu)$ can exceed $\ell_v$, we bound their number by $2^{\ell(\mu)}$, but the edge filter of Lemma~\ref{lemma:edgefilter} retains only those beneath the running dominant edge, of which there are at most $2^{\ell_v}$ live at once. Each layer performs work bounded by the product of the front size, the number of live $v$-labels, and the two per-step branchings, each of which is $\mathrm{poly}(n)$; multiplying by $L$ layers gives the stated bound. Substituting $P_u\leq n!\leq 2^{n\log_2 n}$ and $\ell_v\leq\binom{n}{2}$ yields the worst-case form. If $v$ is dominant then $\theta(v)=\code(v)$, $\ell(\mu)=\ell_v$, every path step carries a $+1$ sign, and each layer's $v$-side is a single path, so the factor $2^{\ell_v}$ is replaced by $O(\ell_v)$ and no term is produced and later canceled, giving $O(\ell_v\cdot P_u\cdot\mathrm{poly}(n))=O(2^{\,n\log_2 n})$.
\end{proof}

\begin{remark}
The quantum algorithm has the same shape, running the transition of Theorem \ref{theorem:quandouble} in place of Theorem \ref{theorem:orddouble}, with the quantum factorial-elementary Pieri operator at each layer. Two things change relative to the classical case. First, the $v$-side is unaffected: Theorem \ref{theorem:quandouble} uses the same increasing-path structure (via $\mom$ and $\thop$), so the number of $v$-labels is still at most $2^{\ell_v}$. Second, on the $u$-side the quantum covers of Remark \ref{remark:qkbruhat} may \emph{decrease} length, so a node is now a pair $(w,d)$ of a permutation together with a $q$-degree, and the same $w$ may be reached at several $q$-degrees. This does not reintroduce cycles: each length-decreasing cover raises the $q$-degree $\sum_i d_i$ by a strictly positive amount (by $b-a$ when the transposition is $t_{ab}$), so grading nodes by $q$-degree---equivalently, by the quantum $k$-Bruhat order---keeps the transition graph acyclic.
\end{remark}

\begin{definition}[$\widehat P_u$, $Q_{u,v}$] \label{def:quantumfront}
For $u,v\in S_n$ set
$$\widehat P_u:=\#\{\,w\in S_n\mid \ell(w)\leq \ell_u+\ell_v\,\},\qquad Q_{u,v}:=\binom{\lfloor(\ell_u+\ell_v)/2\rfloor+n-1}{n-1}.$$
\end{definition}
...
$$O\!\left(\,\min(n-1,\ell_v)\cdot \widehat P_u\cdot Q_{u,v}\cdot 2^{\ell_v}\cdot\mathrm{poly}(n)\,\right)$$
operations in the coefficient ring. In the worst case this is at most
$$2^{\binom{n}{2}}\cdot n!\cdot Q_{u,v}\cdot\mathrm{poly}(n),$$
whose leading exponent is $\binom{n}{2}$, the same as in the classical bound of Theorem \ref{thm:complexity}.
\end{theorem}

\begin{proof}
The layer count $L\leq\min(n-1,\ell_v)$ and the $v$-side bound $2^{\ell_v}$ are as in Theorem \ref{thm:complexity}, since the $v$-side transition is unchanged. On the $u$-side, the pairing $\sum_i u_i=\ell(\mu)-\ell(v\mu)=\ell_v$ of Theorem~\ref{thm:complexity} still bounds the net length added when all covers are length-increasing; the quantum covers of Remark \ref{remark:qkbruhat} can additionally \emph{decrease} length, which only lowers $\ell(w)$, so every reachable permutation still satisfies $\ell(w)\leq\ell_u+\ell_v$ and there are at most $\widehat P_u$ of them. Each such permutation is reached with a $q$-degree $d=(d_1,\ldots,d_{n-1})$; by the grading, a nonzero contribution requires $\ell(u)+\ell(v)-\ell(w)+2\sum_i d_i\geq 0$, and specializing $y=z=0$ (equivalently, comparing with the small quantum cohomology of the flag variety \cite{fulton2004quantum}) forces $\sum_i d_i\leq\tfrac12(\ell_u+\ell_v)$ at every layer. The number of such $q$-degrees is at most $\binom{\lfloor(\ell_u+\ell_v)/2\rfloor+n-1}{n-1}=Q_{u,v}$, so the $u$-front is a multiset of at most $\widehat P_u\cdot Q_{u,v}$ visits. Multiplying the per-layer work as before gives the stated bound. For the worst case use $\widehat P_u\leq n!$ and $\ell_v\leq\binom{n}{2}$; since $\log_2\!\big(n!\,Q_{u,v}\big)=O(n\log n)=o\!\big(\binom{n}{2}\big)$, the leading exponent is $\binom{n}{2}$.
\end{proof}
...
$$w=uc_1\cdots c_n$$
where for each $i$ there exist pairwise distinct positive integers $a_{i,1},\ldots,a_{i,p_i}$ and $b_i$ such that we may write the cycle $c_i$ as
$$c_i=(a_{i,p_i},a_{i,p_i-1},\ldots, a_{i,1},b_i)$$
with $a_{i,j}\leq k$ for all $1\leq j\leq p_i$, $b_i>k$, and $b_i>b_j$ if $i<j$, and such that for each $m$ we have
$$\ell(u,uc_1\cdots c_m) = \sum_{i=1}^m \left(p_i - 2\sum_{a\in \desc_u(c_i)}(b_i-a)\right)$$
and
$$\ell(u,w)+\deg q_k(u,w)=p_1+\cdots+p_n$$
\end{lemma}
%Note that $\ell(u,w)=\ell(u,uc_1\cdots c_n)$ need not be positive: each descent $a\in\desc_u(c_i)$ contributes a length drop of $2(b_i-a)-1$. These drops are recorded in the grading, where $q_k(u,w)$ has $q$-degree $\sum_{i=1}^n 2\sum_{a\in\desc_u(c_i)}(b_i-a)$ under $\deg q_{ab}=2(b-a)$, so that

\begin{proof}
Suppose first that $u\tom{k}w$. By Remark \ref{remark:qkbruhat} there are transpositions $t_{a_1b_1},\ldots,t_{a_pb_p}$ with $a_i\leq k<b_i$, the $a_i$ pairwise distinct, and $b_1\geq\cdots\geq b_p$, such that
$$u=u_0\lessdot_k^{\mathbf q}u_1\lessdot_k^{\mathbf q}\cdots\lessdot_k^{\mathbf q}u_p=w,\qquad u_i=u_{i-1}t_{a_ib_i},$$
is a saturated chain in the quantum $k$-Bruhat order. Grouping the transpositions that share a common top $b$ assembles them, exactly as in the non-quantum case \cite[Lemma~7.3]{samuelmolev}, into a cycle
$$c=(a_{i+r},\ldots,a_i,b)=t_{a_ib}\cdots t_{a_{i+r}b};$$
ranging over the distinct values of $b$ in decreasing order yields pairwise disjoint cycles $c_1,\ldots,c_n$ with $w=uc_1\cdots c_n$ of the asserted form. To compute the length, set $c^{(j)}=t_{a_ib}\cdots t_{a_jb}=(a_j,\ldots,a_i,b)$; the step from $uc^{(j-1)}$ to $uc^{(j)}=uc^{(j-1)}t_{a_jb}$ exchanges the value $u(a_{j-1})$ in position $b$ with the value $u(a_j)$ in position $a_j$. By the cover characterization of Remark \ref{remark:qkbruhat} this step is a classical cover, of length increment $+1$, when $u(a_j)<u(a_{j-1})$, and a quantum cover, of length increment $-(2(b-a_j)-1)$, when $u(a_j)>u(a_{j-1})$, that is, precisely when $a_j\in\desc_u(c)$. Summing these increments over the steps constituting $c_1,\ldots,c_m$ gives
$$\ell(u,uc_1\cdots c_m)=\sum_{i=1}^m\left(p_i-2\sum_{a\in\desc_u(c_i)}(b_i-a)\right),$$
and, recording each quantum step in the $q$-degree as in Definition \ref{definition:pieri}, $\ell(u,w)+\deg q_k(u,w)=p_1+\cdots+p_n$.

Conversely, suppose pairwise disjoint cycles $c_1,\ldots,c_n$ as in the statement are given, and write $c_i=t_{a_{i,1}b_i}\cdots t_{a_{i,p_i}b_i}$. Applying these transpositions in order, the step from $uc^{(j-1)}$ to $uc^{(j)}=uc^{(j-1)}t_{a_{i,j}b_i}$ exchanges the values $u(a_{i,j})$ and $u(a_{i,j-1})$ in positions $a_{i,j}\leq k<b_i$. Any transposition $t_{ab}$ changes the length by at least $+1$ when it places the larger of the two exchanged values in the smaller position $a$, and by at least $-(2(b-a)-1)$ in every case, with equality in either bound if and only if the step is a classical, respectively quantum, cover in the sense of Remark \ref{remark:qkbruhat}. Applying the first bound at the non-descent steps and the second at the descents therefore gives
$$\ell(u,uc_1\cdots c_n)\geq p_1+\cdots+p_n-2\sum_{i=1}^n\sum_{a\in\desc_u(c_i)}(b_i-a),$$
and the length hypothesis of the statement forces equality throughout, so every step is a cover of the quantum $k$-Bruhat order and hence $u\tom{k}w$.
\end{proof}

\newcommand{\lup}{\triangleleft}

\begin{definition}[$\lup$]
If $u,u'\in S_\infty$, we define $u\lup u'$ if $\ell(u,u')=1$ or $u'=ut_{ab}$ where $\ell(u,u')=1-2(b-a)$. Equivalently, $u\lup ut_{ab}$ exactly when $u\lessdot_k^{\mathbf q}\mathbf q^{\gamma}ut_{ab}$ is a cover of the quantum $k$-Bruhat order for some (equivalently, any) $k$ with $a\leq k<b$, in the sense of Remark \ref{remark:qkbruhat}.

%Define $\nabla_u^w(F(x))$ to be the coefficient of $\qsch_w(x;y)$ in the product $F(x)\qsch_u(x;y)$, replacing $y$ with $x$ and quantizing.

%Same as unquantizing, doing the div diff, and requantizing.

%$$\mathcal{Q}\partial^w\mathcal{Q}^{-1}$$
%$$\mathcal{Q}\partial^w\mathcal{Q}^{-1}(\qsch_u(x;y)\qsch_v(x;z))$$
%
%$$\mathcal{Q}\partial^w\mathcal{Q}^{-1}(c_{u,v}^{w,d}(y;z)q^d\qsch_w(x;y))$$
%
...
%$$(\sch^q_u(\chi;y)\qsch_v(\chi;y))\cdot 1=c_{u,v}^w(x;x)\sch_w(y;x)$$
%$$F(x)\qsch_u(x;y)=\sum_w \nabla_u^w(F(x))|_{x=y}\qsch_w(x;y)$$
%$$\nabla^u(\qsch_v(x;z))=\nabla^u(\qsch_{v_2}(x;y)\sch_{v_1}(y;z)$$
\end{definition}
...
Having excluded the cases $a_p>a_i$ and $a_p<a_i$, by the assumption that $a_p\neq a_i$ it follows by induction that there can be at most one descent.
\end{proof}
...
The uniqueness of $b$ among the indices $r>k$ with $\ell(ut_{kr})=\ell(u)+1$, and the nonexistence of an index $q<k$ with $\ell(ut_{qk})=\ell(u)+1$ and $ut_{qk}\tom{k-1}w$, now follow exactly as in the proof of \cite[Lemma~7.6]{samuelmolev}. Those arguments use only the disjoint cycle characterization of the Pieri relation, which in the present setting is supplied by Lemma \ref{lemma:piericycle} in place of \cite[Lemma~7.3]{samuelmolev}: in each case the reflection $t_{kr}$ (respectively $t_{qk}$) either merges the cycle $c_i$ containing $k$ with another cycle or splits it so that $k$ and a second index exceeding $k-1$ lie in a common cycle of $(ut_{kr})^{-1}w$ (respectively $(ut_{qk})^{-1}w$), whence $ut_{kr}\ntom{k-1}w$ (respectively $ut_{qk}\ntom{k-1}w$) by Lemma \ref{lemma:piericycle}.
\end{proof}
...
The heart of the induction is the following cancellation, which replaces the informal ``the sums are empty'' step of the classical argument. When $u(k)=w(k)$ the diagonal term alone already reproduces $c_k(u,w)$, by adjoining the variable $y_{u(k)}$ to the product $\wte(y_{\fx_{{k-1}}};z)$; the reflection sums and the quantum term must therefore cancel among themselves.
...
$$\fx_{{k-1}}(u,v)=\fx_{{k-2}}(u,w),\qquad n_{k-1}(u,v)=n_{k-2}(u,w)+1$$
(using $n_m=m-|\fx_{m}|$). Hence $c_{k-1}(u,v)$ and $c_{k-2}(u,w)$ carry the \emph{same} factorial elementary polynomial $\wte(y_P;z)$ with $P=\fx_{{k-2}}(u,w)$. Finally the $q$-weights match. Write $\kappa_{a^\ast k}=1$ if $t_{a^\ast k}$ is length-increasing and $\kappa_{a^\ast k}=q_{a^\ast k}$ if length-decreasing. As the $q$-weight is multiplicative over the cycles of $u^{-1}(\cdot)$ and only the cycle receiving $k$ is altered (from top $k-1$ to top $k$), the definition of the weights yields
$$\kappa_{a^\ast k}\,q_{k-1}(u,v)=q_{k-1}\,q_{k-2}(u,w).$$
Therefore the contributing summand is
$$\mathrm{sign}(a^\ast-k)\,\kappa_{a^\ast k}\,c_{k-1}(u,v)=-\,\kappa_{a^\ast k}\,q_{k-1}(u,v)\,\wte(y_P;z)=-\,q_{k-1}\,q_{k-2}(u,w)\,\wte(y_P;z)=-\,q_{k-1}c_{k-2}(u,w),$$
...
$$\fx_{{k-1}}(u,v)=\fx_{{k-2}}(u,w),\qquad n_{k-1}(u,v)=n_{k-2}(u,w)+1,\qquad \kappa_{ak}q_{k-1}(u,v)=q_{k-1}q_{k-2}(u,w),$$
...
\end{lemma}
\begin{proof}
Suppose first that $u(k)=w(k)$. Then by Lemma \ref{lemma:leveldichotomy}(i) also $u\ntom{k-1}w$, so $c_{k-1}(u,w)$ vanishes; hence the diagonal term is $0$, and the remaining reflection sums and the quantum term cancel by Lemma \ref{lemma:reflectioncancel}. This settles the case $u(k)=w(k)$ completely.

Now suppose $u(k)\neq w(k)$. Since $u\ntom{k}w$, Lemma \ref{lemma:piericycle} shows the cycle of $u^{-1}w$ containing $k$ has a top $b>k$, so $c_k(u,w)=0$; writing $F$ for the right-hand side \eqref{eq:diag}--\eqref{eq:rec}, we must show $F=0$.
...
\]
Fix $M$ and let $\mathcal{G}_M$ be the collection of summands of $F$ whose $q$-weight equals $q^M$; here the diagonal \eqref{eq:diag} carries weight $q_{k-1}(u,w)$, the quantum term carries $q_{k-1}q_{k-2}(u,w)$, and the Monk summand of \eqref{eq:up}--\eqref{eq:down} reaching $w'=wt_{ak}$ or $w'=wt_{kb}$ carries $\kappa\,q_{k-1}(u,w')$ with $\kappa\in\{1,q_{a k},q_{k b}\}$ its drop-weight. We show $F_M=0$ in two exhaustive cases.

\emph{Case (i): $\mathcal{G}_M$ contains the diagonal.} It carries weight $q_{k-1}(u,w)$, so $q^M=q_{k-1}(u,w)$; in particular $u\tom{k-1}w$. Write $P=\fx_{{k-1}}(u,w)$ and $n=n_{k-1}(u,w)$. Set $a_0=u^{-1}(w(k))$; since $u(k)\neq w(k)$ and $w(k)$ is a value of $u$ occurring at a position $<k$ (this holds whenever the diagonal is nonzero), we have $a_0<k$. The Monk summand indexed by $t_{a_0 k}$ then satisfies $u\tom{k-1}wt_{a_0 k}$ with
$$\fx_{{k-1}}(u,wt_{a_0k})=P\cup\{w(k)\},\qquad n_{k-1}(u,wt_{a_0k})=n-1,$$
carries weight $q_{k-1}(u,w)$, and is the \emph{only} Monk summand in $\mathcal{G}_M$. Dividing $\mathcal{G}_M$ by the common factor $q_{k-1}(u,w)$, its two contributions read
$$(y_{w(k)}-z_1)\,\wte(y_P;z)-\wte(y_{P\cup\{w(k)\}};z),$$
which vanish since $\wte(y_{P\cup\{w(k)\}};z)=(y_{w(k)}-z_1)\wte(y_P;z)$ by adjoining the variable $y_{w(k)}$. Hence $F_M=0$.

\emph{Case (ii): $\mathcal{G}_M$ does not contain the diagonal.} Then $q^M\neq q_{k-1}(u,w)$, and $\mathcal{G}_M$ consists of Monk summands of \eqref{eq:up}--\eqref{eq:down} together with the quantum term when $q^M=q_{k-1}q_{k-2}(u,w)$. By Lemma \ref{lemma:movedcancel} these summands sum to $0$, so $F_M=0$.
...
with the conventions $\qelem_{0,0}^q=1$ and $\qelem_{k,k}=0$ if $k<0$. Moreover, the ordinary polynomials $\wte(y_A;z)=\prod_{i\in A}(y_i-z_1)$ satisfy, for any set $A$ and any $c\notin A$, the single-variable identity
$$\wte(y_{A\cup\{c\}};z)=(y_c-z_1)\,\wte(y_A;z).$$
\end{lemma}

The first recurrence is the quantization of the classical relation $E_{k,k}=(x_k-z_1)E_{k-1,k-1}$; the extra term $q_{k-1}\qelem_{k-2,k-2}$ is the only feature absent from the non-quantum setting. Discarding all quantum terms recovers the classical Pieri formula \cite[Theorem~7.1]{samuelmolev}, whose term-by-term cancellation is carried out in \cite[Proof of Proposition~4.4]{samuelmolev} by means of the reflection-matching Lemmas 7.6--7.9 of that paper; we adapt that argument, the essential new points being the length-decreasing steps permitted by Lemma \ref{lemma:piericycle} and the attendant bookkeeping of the powers of $q$.
...
and $c_m(u,w)=0$ if $u\ntom{m}w$. The claim is that $[\qsch_w](\qsch_u\qelem_{k,k})=c_k(u,w)$.
...
the second equality holding because no reflection with first index $k$ occurs. The diagonal contribution \eqref{eq:diag} then gives
$$(y_{u(k)}-z_1)\,c_{k-1}(u,w)=q_{k-1}(u,w)\,(y_{u(k)}-z_1)\,\wte(y_{\fx_{{k-1}}(u,w)};z)=q_k(u,w)\,\wte(y_{\fx_{k}(u,w)};z)=c_k(u,w),$$
by adjoining the variable $y_{u(k)}$ as in Lemma \ref{lemma:elemrecursion}. The remaining contributions are the reflection sums \eqref{eq:up}, \eqref{eq:down} and the quantum term $q_{k-1}c_{k-2}(u,w)$ of \eqref{eq:rec}, which sum to $0$ by Lemma \ref{lemma:reflectioncancel}. Hence the total is $c_k(u,w)$, as required. If instead $u(k)=w(k)$ but $u\ntom{k}w$ (equivalently $u\ntom{k-1}w$), then $c_k(u,w)=0$ and the diagonal term vanishes, while the reflection sums and quantum term again cancel by Lemma \ref{lemma:reflectioncancel}; the total is $0=c_k(u,w)$.

\emph{Case 2: $u\tom{k}w$ but $u\ntom{k-1}w$.} By Lemma \ref{lemma:pieriknotkn1} there is a unique $b>k$ with $wt_{kb}\lup w$ and $u\tom{k-1}wt_{kb}$; set $v=wt_{kb}$. This is exactly the unique nonvanishing summand of \eqref{eq:up}, and by Lemma \ref{lemma:pieriknotkn1} the sums \eqref{eq:down} vanish, as does the diagonal \eqref{eq:diag} and the quantum term \eqref{eq:rec} (since $u\ntom{k-1}w$ and $u\ntom{k-2}w$). Moreover $\fx_{k}(u,w)=\fx_{{k-1}}(u,v)$, so
$$\wte(y_{\fx_{k}(u,w)};z)=\wte(y_{\fx_{{k-1}}(u,v)};z).$$
If $\ell(w)=\ell(v)+1$ then the reflection $t_{kb}$ is length-increasing, so by Definition \ref{definition:pieri} $q_k(u,w)=q_{k-1}(u,v)$ and the contribution is $c_{k-1}(u,v)=c_k(u,w)$. If instead $\ell(w)=\ell(v)-2(b-k)+1$ then $t_{kb}$ is length-decreasing, Definition \ref{definition:pieri} gives $q_k(u,w)=q_{k-1}(u,v)\,q_{kb}$, and the contribution is $q_{kb}\,c_{k-1}(u,v)=c_k(u,w)$. In both cases the coefficient equals $c_k(u,w)$, as required. This is the step at which the quantum drop terms of Lemma \ref{lemma:singlevariable} and the recursive definition of $q_k(u,w)$ interlock.

\emph{Case 3: $u\ntom{k}w$ (with $u(k)\neq w(k)$).} Here $c_k(u,w)=0$, so we must show the coefficient \eqref{eq:diag}--\eqref{eq:rec} vanishes. This is exactly the assertion of Lemma \ref{lemma:vanishing}. (The subcase $u(k)=w(k)$ with $u\ntom{k}w$ was already treated at the end of Case 1.)

In all cases $[\qsch_w](\qsch_u\qelem_{k,k})=c_k(u,w)$, completing the induction and the proof.
\end{proof}
...
\begin{theorem}
	...
	$$\sum_{u\dtom{k} w} \sum_{v\in\mathrm{pivot}_k(u,w)}q_k(u,v)q_k(v,w)E_{k-n_k(u,v)}(y_{\fx_{k}(u,v)};z_1)E_{k-n_k(v,w)}(y_{\fx_{k}(v,w)};z_2)\qsch_w(x;y).$$
\end{theorem}

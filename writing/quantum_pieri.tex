...
\emph{Case (ii): $\mathcal{G}_M$ contains neither the diagonal nor the straight term.} Then $\mathcal{G}_M$ consists of Monk summands together with, possibly, the quantum term, and we show these cancel in pairs of equal factorial elementary coefficient and opposite sign. If the quantum term lies in $\mathcal{G}_M$, then $q^M=q_{k-1}q_{k-2}(u,w)$ and, exactly as in Lemma \ref{lemma:reflectioncancel} (whose Steps 1--3 use only $u\tom{k-2}w$ and the cycle structure at indices $\leq k$, never the hypothesis $u(k)=w(k)$), there is a unique Monk summand in $\mathcal{G}_M$, indexed by a reflection $t_{a^\ast k}$ with $a^\ast<k$, whose neighbour $v$ satisfies $P_{k-1}(u,v)=P_{k-2}(u,w)$, $n_{k-1}(u,v)=n_{k-2}(u,w)+1$ and $\kappa_{a^\ast k}q_{k-1}(u,v)=q_{k-1}q_{k-2}(u,w)$; it equals $-q_{k-1}c_{k-2}(p-2;u,w)$ and cancels the quantum term. The remaining Monk summands of $\mathcal{G}_M$ pair off through position $k$: a down-neighbour $wt_{rk}$ ($r<k$) with an up-neighbour $wt_{sk}$ ($s>k$), the two reflections differing by a $3$-cycle through $k$. Such a pair shares the agreement set $P_{k-1}$ and multiplicity $n_{k-1}$, hence the same factorial elementary coefficient, while the opposite signs $\mathrm{sign}(r-k)=-1$ and $\mathrm{sign}(s-k)=+1$ make the two contributions cancel. For the classical skeleton $M=1$ this is precisely the fixed-point-free reflection cancellation underlying the classical Pieri rule \cite{samuelmolev}. For $M\neq1$ the same through-$k$ pairing persists on the (now length-decreasing) neighbours of common $q$-weight $q^M$; that this pairing exhausts $\mathcal{G}_M$ has been verified by direct computation for all $u,w\in S_n$ with $n\leq5$, and we isolate it as Proposition~\ref{prop:qreflection} below. Therefore $\mathcal{G}_M$ sums to $0$ and $F_M=0$.

As the two cases are exhaustive, every $F_M=0$, so $F=0$.
\end{proof}

The reflection–reflection cancellation used in Case (ii) of the previous proof is the sole point at which we appeal to computation. We isolate it, together with its verified range, as the following statement; its $M=1$ instance is the classical reflection cancellation of \cite{samuelmolev}, and its quantum-term instance is Lemma \ref{lemma:reflectioncancel}.

\begin{proposition}[Quantum reflection cancellation] \label{prop:qreflection}
Let $u,w\in S_\infty$ with $u\ntom{k}w$ and $u(k)\neq w(k)$, and let $q^M\neq q_{k-1}(u,w)$ be a $q$-monomial, so that no diagonal or straight term of \eqref{eq:diag}--\eqref{eq:rec} has weight $q^M$. Then the Monk summands of \eqref{eq:up}--\eqref{eq:down} of $q$-weight $q^M$, together with the quantum term of \eqref{eq:rec} when $q^M=q_{k-1}q_{k-2}(u,w)$, admit a fixed-point-free pairing in which paired contributions carry the same agreement set $P_{k-1}$, multiplicity $n_{k-1}$ and factorial elementary coefficient but opposite sign; consequently they sum to $0$. When $q^M=q_{k-1}q_{k-2}(u,w)$ the pairing is that of Lemma \ref{lemma:reflectioncancel}; otherwise it sends a down-neighbour $wt_{rk}$ ($r<k$) to an up-neighbour $wt_{sk}$ ($s>k$) via a $3$-cycle at position $k$. The statement holds for all $u,w\in S_n$ with $n\leq 5$.
\end{proposition}

The quantum-term case of Proposition~\ref{prop:qreflection} is proved in Lemma~\ref{lemma:reflectioncancel} and the classical skeleton in \cite{samuelmolev}; the general length-decreasing case is the natural common generalization, and a uniform cycle-surgery proof is the subject of ongoing work.

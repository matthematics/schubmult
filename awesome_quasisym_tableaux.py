...
if __name__ == "__main__":
    ...
    def J(qsym_elem):
        ...
        QSym = QuasiSymmetricFunctions(ZZ['t'])
        M = QSym.M()
        qsym_elem1 = M(qsym_elem)
        dct_result = {}
        used = set()
        while qsym_elem1 != QSym.zero():
            # print(f"{qsym_elem1=}")
            dct = {tuple([int(a) for a in k]): v for k,v in dict(qsym_elem1).items() if v != v.parent().zero()}
            if len(dct.keys()) == 0:
                break
            key = next(iter(sorted([tuple(k) for k in dct.keys()],key=lambda k: (len(k),tuple([a for a in k])))))
            # print(f"{key}, {dct[key]}")
            if key in used:
                raise Exception(f"{key} double used {used}")
            used.add(key)
            val = dct[key]
            # ding = val * FA.j_quasisymmetric(tuple(key))
            # print(f"{ding=}")
            qsym_elem1 -= val * FA.j_quasi_recurse(tuple(key))
            dct_result[key] = dct_result.get(key, val.parent().zero()) + val
            # print(len(dict(qsym_elem1).keys()))

        return dct_result
    QS = QSym.QS()
    tt = ZZ['t']
    #fat = J(FA.j_quasi_recurse((1,1,2))*FA.j_quasi_recurse((2,1)))
    Jip = FreeAlgebra(JBasis)

    def is_horiz_strip(beta, alpha):
        if len(beta) <= 1:
            if len(alpha) == 0:
                return True
            if alpha[0] <= beta[0]:
        ...
        if len(alpha) < len(beta):
            alpha = [*([0]*(len(beta)-len(alpha))),*alpha]
        above = set()
        above.update(tuple([(0,a) for a in list(range(alpha[0],beta[0]))]))
        for i in range(1,len(beta)):
            if beta[i] < alpha[i]:
                return False
            new_above = set(tuple([(i, a) for a in list(range(alpha[i],beta[i]))]))
            if not all(all(ia + a + 1 < ib + b for (ia, a) in above) for (ib, b) in new_above):
                return False
            
            above.update(new_above)
        return True

    def is_left_descent(alpha, j):
        if any(a <= 0 for a in alpha):
            raise Exception(f"Bad alpha {alpha}")
        print("is_left_descent", alpha, j)
        if j >= len(alpha):
            return False
        if j == 0:
            return True
        for i in range(1, j + 1):
            if alpha[j] + j - i <= alpha[i-1]:
                return False
        print("yep")
        return True
    ...
    def skew(beta, alpha):
        if len(alpha) < len(beta):
            alpha = [*([0]*(len(beta)-len(alpha))),*alpha]
        skew_shape = [a - b for a, b in zip(beta, alpha)]
        return skew_shape

    def all_horiz_strips(alphagod):
        if len(alphagod) == 0:
            return {alphagod}
        stack = [alphagod]
        res = set()
        while len(stack) > 0:
            top = stack.pop()            
            print(f"{top=}")
            skew_shape = tuple(skew(alphagod, top))
            print(f" {skew_shape=}")
            if skew_shape in res:
                continue
            res.add(skew_shape)
            for j in range(len(top)):
                if is_left_descent(top, j):
                    new_top = [a - 1 if i == j else a for i, a in enumerate(top)]                    
                    while len(new_top) > 0 and new_top[0] == 0:
                        new_top = new_top[1:]
                    print(f" {new_top=}")
                    print(f" {alphagod=}")
                    print(f" {is_horiz_strip(alphagod, new_top)=}")
                    if is_horiz_strip(alphagod, new_top):                        
                        print(f"Affing the bagel {new_top=}")
                        stack.append(tuple(new_top))
        return res
    
    def all_derived(alphagod):
        if len(alphagod) == 0:
            return {alphagod}
        stack = [alphagod]
        res = set()
    ...
    def tableaux(alphagod):
        if len(alphagod) == 0:
            return {(): 1}
        res = {}
        for der in all_derived(alphagod):
            print(f"Considering derived {der}")
            if der == alphagod:
                continue            
            # if len(hstrip) < len(alphagod):
            #     ahstrip = (*([0]*(len(alphagod)-len(hstrip))),*hstrip)
            # else:
            #     ahstrip = hstrip
            skew_shape = skew(alphagod, der)
            # while len(skew_shape) > 0 and skew_shape[0] == 0:
            #     skew_shape = skew_shape[1:]
            skew_shape = tuple(skew_shape)
            if len(skew_shape) == 0 or sum(skew_shape) == 0:
                new_tab = (der, )
                res[new_tab] = res.get(new_tab, 0) + 1
                continue
            for tab, val in tableaux(der).items():
                new_tab = (skew_shape, *tab)
                res[new_tab] = res.get(new_tab, 0) + val
        return res
        
    alphagod = (2,1,2)
    print(f"Tableaux of shape {alphagod}:")
    tabset = tableaux(alphagod)
    ret = 0
    for tab, val in tabset.items():
        print(f"{tab}: {val}")
        comp = [sum(t) for t in tab]
        ret += val * M[*comp]
    print(f"{ret - FA.j_quasisymmetric(alphagod)}")
    
    #print(f"Number of tableaux: {len(tabset)}")
    #print(next(iter(["\n".join(dir(sympy_converter(v))) for v in fat.values()])))
    # fatfk = Jip.from_dict({tuple([int(a) for a in k]): sympify(sum([int(v2)*Symbol("t")**int(k2) for k2, v2 in v.monomial_coefficients().items()])).subs({Symbol("t"): S.One}) for k, v in fat.items()})
    # print(fatfk)

    # sys.exit(0)
    # dingbat = FA.j_quasi_recurse((2,2,3,1)).coproduct()
    # print(dict(dingbat))
    
    # def polify(v):
    #     return Add(*[int(v2)*Symbol("t")**int(k2) for k2, v2 in v.monomial_coefficients().items()])
    # JBunk = FreeAlgebra(JBasis)
    # R = JBunk @ JBunk
    # pinworm = R.zero
    # for (k1, k2), v in dict(dingbat).items():
    #     dct1 = {k: polify(v*v2) for k, v2 in dict(J(M[*k1])).items()}
    #     dct2 = {k: polify(v2) for k, v2 in dict(J(M[*k2])).items()}

    #     pinworm += R.ext_multiply(JBunk.from_dict(dct1),JBunk.from_dict(dct2))

    # print(R.from_dict(pinworm))
    ...
    for i in range(10,12):
        for c in Compositions(i):
            alphagod = tuple([int(a) for a in c])
            bagel0 = FA.j_quasi_recurse(alphagod)
            bagel = QSym.zero()
            for k, v in dict(bagel0).items():
                bagel += v.subs(1)*QS[*k]
            print(f"J{list(alphagod)} = {QS(bagel)}")
            # ret0 = 0
            # # for k, v in dict(ret).items():
            # #     ret0 += v.subs(1)*M[*k]
            # # ret = ret0
            # # #print(f"J({alphagod}) = {ret}")
            # # print(f"J({alphagod}) = {QS(ret)}")
            # for k, v in dict(ret).items():
            #     kk = tuple([int(a) for a in k])
            #     fa_elem = FA(*kk).change_basis(JTBasis)
            #     for k2, v2 in fa_elem.items():
            #         if k2[0] == alphagod:
            #             assert v == tt.gens()[0]**(k2[1])*v2, f"Fuck the donkey {alphagod=} {k=} {v=} {k2=} {v2=}"
            #             break
            # print(f"plefkit {c}")
            # fa_elem = FA(*alphagod).change_basis(JTBasis)
            # for (tup, tpow), v in fa_elem.items():
            #     polydict[tup] = polydict.get(tup,0) + v * tt.gens()[0]**tpow * M[*alphagod]
    # for tup in polydict:
    #     # print(f"{tup} correct: {polydict[tup]}")
    #     # print(f"{tup} maybewr: {gendict[tup]}")
    #     # fatdiff = polydict[tup] - gendict[tup]
    #     # print(f"fatdiff = {fatdiff}")
    #     bagel = QSym.zero()
    #     bagel0 = QS(gendict[tup])
    #     for k, v in dict(bagel0).items():
    #         bagel += v.subs(1)*QS[*k]
    #     print(f"J{list(tup)} = {QS(bagel)}")
    #     # assert fatdiff == 0
    sys.exit(0)
    def comp(p, perm):

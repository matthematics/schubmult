...
\begin{lemma}
	We have that $\mathtt{tab}(R)$ is a semi-standard Young tableau of shape $\mathrm{shape}(Q)$, and the map
...
\end{theorem}
\begin{proof}
By \cite[Theorem~4.11]{morse2016crystal}, if $e_i(R)\neq \emptyset$, then $R$ and $e_i(R)$ have the same Edelman-Greene insertion tableau. Since conjugation by $w_0$ is an automorphism of Coxeter-Knuth equivalence, so do $\ege(R)$ and $\ege(e_i(R))$. However, the shape
\end{proof}

\begin{definition}
Let $\RC(w, n)^0$ be the set of bounded RC graphs $(R, n)$ such that $\wof{R} = w$ and row $n$ is empty. We define a function $\zeromap_*^R:I(w)\to I(w')$, where $\zeromap(R,n) = (R',n-1)$ and $w'=\wof{R'}$, as follows. Suppose $\code_n(w) = m$. Let $c=s_{n + m -1}\cdots s_n$. Then there exist unique reflections $t_{a_1b_1}, \ldots, t_{a_mb_m}$ such that
...
\begin{lemma}
  The map $\zeromap_*^R:I(w)\to I(w')$ depends only on $w$ and $w'$, not on the choice of $R$ (besides the fact that it determines $w'$).
\end{lemma}
\begin{proof}
  This is clear if $\maxd(R)<n$. Otherwise, we prove this by induction on $\code_n(\wof{R})$. If $\code_n(\wof{R})=1$, then given $w'$ the $(n,n+1)$ root from $I(w)$ can only map to one root in $I(w')$, namely the root $(a,b)$ such that $ws_n = w't_{ab}$. Now suppose the result holds for $\code_n(\wof{R})<m$ for some $m>1$, and let $\code_n(\wof{R})=m$. Let $R_1 = R\setminus \{(i,j)\}$ where $\rtt_R(i,j) = (n, n + m)$, and let $\zeromap(R_1, n) = (\widetilde{R}, n-1)$. By the inductive hypothesis, the map $\zeromap_*^{R_1}:I(\wof{R_1})\to I(\wof{\widetilde{R}})$ is independent of the choice of $R_1$. By construction of the algorithm, we have that $\widetilde{R} =  R'\setminus\{(i,k)\}$ for some $k$. Hence $\zeromap_*^R$ is determined by $\zeromap_*^{R_1}$ along with the image of $(n, n + m)$, which is determined by $w$ and $w'$ alone.
\end{proof}
...
\end{theorem}
\begin{proof}
Suppose $\zeromap(R, n) = (R', n-1)$. We claim that for any $R''\in \RC(\wof{R'}, n-1)$, we may define a function $T_{R''}^*$ by
$$T_{R''}^*(i, j) = T_{R''}(\zeromap_*^R(i,j))$$
and $T_{R''}^*$ is an inversion tableau for $\wof{R}$. Since $e_i$ and $f_i$ preserve the permutation $\wof{R'}$, it follows that there is at least a crystal raising operator $e_i^*:\RC(w, n)^0\to \RC(w, n)^0$ defined by
$$T_{e_i(R'')} = e_i^*(T_{R''}^*)$$
and similarly for $f_i^*$. It is clear that these operators are inverses of each other when defined, and hence we have a crystal structure on $\RC(w, n)^0$ transported from $\bigcup_{w\downvar{n} w'}\RC(w', n-1)$. Since $\zeromap$ is weight-preserving bijection and crystals do not have any nontrivial automorphisms that preserve weight, it follows that in fact $e_i=e_i^*$ for all $i$, and hence $\zeromap$ is an isomorphism of crystal graphs.
\end{proof}

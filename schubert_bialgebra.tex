...
\begin{lemma} \label{lemma:doublediff}
  ...
  $$\partial_u^w(E_k(x;-y_p))=\sum_{C\in\achain{p}(u,w)}(-1)^{\sigma(C)}e_{k-\ell(u,w)}^k+\mathcal{O}(y)$$
\end{lemma}
...
where $1 = u_0 \leq u_1\leq \cdots \leq u_{\ell(\lambda)} = ww_\lambda^{-1}$.
Define, for convenience,
$$\sigma(P) = \sum_{i=1}^{\ell(\lambda)} \sigma(C_i)$$
for such a $P$. Then we define
$$E^{\alpha;n}_w = \sum_{P = (C_1,\ldots,C_{\ell(\lambda)})\atop C_i\in\achain{i}(u_{i-1},u_i)}\sum_{(\lambda_{\ell(\lambda) + 1 - i}-|C_{\ell(\lambda) + 1 - i}|)_{i=1}^{\ell(\lambda)}=\alpha}(-1)^{|\lambda|-\ell(w)+\sigma(P)}$$
\end{definition}

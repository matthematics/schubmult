...
Associativity follows. $\omega:\BRC\to \dcoma$ is easily seen to be a homomorphism of rings, since the product is weight preserving. For the homomorphism $\alpha:\BRC\to \dcoma$, consider a bounded RC graph $(R, n)$ and an empty bounded RC graph $(\emptyset, 1)$. Then by definition of $\zeromap$ we have that
$$(R, n)\diamond (\emptyset, 1) = \sum_{\zeromap(R', n+1) = (R,n)}(R', n + 1)$$
and
$$\alpha*(R, n)\alpha(\emptyset, 1) = \dsch_{\wof{R}}^n\dsch_{e}^1 = \sum_{\substack{w'\downvar{n+1} w\\\ell(w)=\ell(w')}}\dsch_{w'}^{n+1}=\sum_{\zeromap(R',n+1) = (R,n)}\alpha(R', n+1)$$
More generally,
$$\alpha(R, n)\alpha(r_p, 1) = \dsch_{\wof{R}}^n[p] = \sum_{\substack{w'\downvar{n+1} w\\\ell(w,w') = p}}\dsch_{w'}^{n+1}=\sum_{\zeromap(R',n+1) = (R,n)}\alpha(R'\cup \shup^{n}r_p, n+1)$$
Associativity gives us the result.
\end{proof}

...
\begin{lemma}[\cite{WEIGANDT2021105470}]
  Let $(B, N)$ be an ordinary reduced BPD for a permutation $w$, so that $w(M) = M$ for all $M>N$ and $B$ is $N\times N$. Let $n\geq \maxd(w)$ be an integer. Then $w(B', n) = w(B, N)$, where $B'$ is equal to the first $n$ rows of $B$ without modification.
\end{lemma}
...
$$\bzeromap(B, n) = (B', n-1)$$
One may ask oneself here whether anything changes at all. Indeed, all that changes are the labels of the incoming pipes, as shown in Figure \ref{figure:transition}.
\end{definition}


There is a canonical bijection $\varphi:\BPD(w,n)\to \mathcal{RC}(w,n)$ defined in \cite{GaoHuang2023} that preserves weight.
...
$$B\looparrowright C = \Delta(((a_1 + n, r_1), (a_2 + n, r_2), \ldots, (a_m + n, r_m)),C\shup^n)$$

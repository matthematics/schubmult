...
\begin{definition}
Define the homomorphism $\iota_n:\mathrm{Schub}_n\to \mathcal{T}_n$ by sending $\mathcal{S}_w(n)$ to $\mathbf{S}_w(n)$ for all $w$ with $\maxd(w)\leq n$ and imposing the $\Lambda_n$-module structure.

Let $\widetilde{T}_n$ be the quotient of $\mathcal{T}_n$ by imposing the squash product relations on the subring generated by the $n$-elementary symmetric polynomial RC graphs. Define $\mathcal{U}_n$ to be the submodule of $\widetilde{T}_n$ generated by the $\mathbf{S}_w(n)$ for $w\in S_n$ with the product
$$\mathbf{S}_u(n)\cdot \mathbf{S}_v(n) = \iota_n(\RC_n(\mathbf{S}_u(n)\mathbf{S}_v(n)))$$
\end{definition}
...
$$\mathfrak{F}(R) = \sum_{A\in\RC_n(\wof{R})}\RC_n^\QY(A)$$
Suppose $\mathbf{A},\mathbf{B}\in \mathcal{T}_n$ are such that $\RC_n(\mathbf{A}) = A$ and $\RC_n(\mathbf{B}) = B$. Then we define
$$\mathfrak{F}(A)\cdot \mathfrak{F}(B) = \mathfrak{F}(\RC_n(\mathbf{A}\mathbf{B}))$$
\end{definition}

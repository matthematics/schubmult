...
Fix integers $p,q\geq 0$. Suppose the following data are given.
\begin{itemize}
\item[(1)] A two-sided ideal $I\subseteq \BRC$ such that $I\subseteq \Omega$, where $\Omega=\ker(\alpha)$ from Theorem~\ref{theorem:semidirect}.
\item[(2)] A set $\Lambda_n$ for each length $n$, and for each $\lambda\in\Lambda_n$ a multiplicity-free element
$$E_\lambda = \sum_{R\in\mathcal{C}_\lambda} R \in \BRC/I$$
where the sets $\{\mathcal{C}_\lambda\}_{\lambda\in\Lambda_n}$ partition the basis elements of $\BRC/I$ (lifted from $\RC_n$ in the quotient), each $R\in\mathcal{C}_\lambda$ appearing with coefficient 1. The family $\{E_\lambda\}_{\lambda\in\Lambda_n}$ then forms a linearly independent set spanning a subring $\mathcal{E}^{(I)}\subseteq \BRC/I$.
\item[(3)] A basis $\{b_\lambda^*\}_{\lambda\in\Lambda_n}$ of $\dcoma_n$ such that
$$\alpha_I(E_\lambda)=b_\lambda^*,$$
where $\alpha_I:\BRC/I\to\dcoma$ is the induced homomorphism.
\item[(4)] For every $\lambda\in\Lambda_{p+q}$, choose a representative set $\mathcal{C}_\lambda\subseteq \RC_{p+q}$ whose image in $\BRC/I$ sums to $E_\lambda$.
\item[(5)] Membership conditions of the form
$$\clp^p(R)\in \mathcal{C}_\mu^{\mathrm{top}},\qquad \trm^p(R)\in \mathcal{C}_\nu^{\mathrm{bot}}$$
that depend only on the class of $R$ in $\BRC/I$ and characterize the products $E_\mu E_\nu$ by counting classes.
\end{itemize}
Then the structure constants
$$b_\mu^*b_\nu^*=\sum_{\lambda\in\Lambda_{p+q}} c_{\mu,\nu}^{\lambda}\, b_\lambda^*$$
satisfy
$$c_{\mu,\nu}^{\lambda}=\#\Bigl\{R\in\mathcal{C}_\lambda:\ \clp^p(R)\in \mathcal{C}_\mu^{\mathrm{top}},\ \trm^p(R)\in \mathcal{C}_\nu^{\mathrm{bot}}\Bigr\},$$
hence $c_{\mu,\nu}^{\lambda}\geq 0$.
\end{theorem}
...
By hypothesis (2), the sets $\mathcal{C}_\lambda$ partition the basis elements of $\BRC/I$ in each length, so the $E_\lambda$ are automatically linearly independent (disjoint supports, unit coefficients). By hypothesis (3), $\alpha_I(E_\lambda)=b_\lambda^*$, so
$$\alpha_I(E_\mu E_\nu)=b_\mu^*b_\nu^*=\sum_\lambda c_{\mu,\nu}^{\lambda}b_\lambda^*.$$
Because the $b_\lambda^*$ are linearly independent, $c_{\mu,\nu}^{\lambda}$ is the coefficient of $E_\lambda$ in the product $E_\mu E_\nu$ in $\BRC/I$.
...
$$\Bigl\{R\in\mathcal{C}_\lambda:\ \clp^p(R)\in \mathcal{C}_\mu^{\mathrm{top}},\ \trm^p(R)\in \mathcal{C}_\nu^{\mathrm{bot}}\Bigr\}.$$
Since this is a cardinality, $c_{\mu,\nu}^{\lambda}\geq 0$.

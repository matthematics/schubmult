...
Let $\mathcal{E}$ be the free associative algebra generated by elementary monomials $M\in \binom{[n]}{k}$ for $1\leq k\leq n$ and $n\geq 1$ with the following relations. We assert that if $M_1\in\binom{[n]}{k}$ and $M_2\in \binom{[m]}{l}$ and $m\neq n$, then $M_1M_2 = M_2M_1$. Within the subalgebra generated by $\binom{[n]}{*}$, we apply an equivalence relation according to the squash product. Across different $n$, equivalence is applied up to squash product with a full elementary symmetric sum.

\begin{lemma}
  The subalgebra of $\mathcal{E}$ generated by full elementary symmetric sums is a commutative ring isomorphisc to $\mathbb{Z}[x]$.
\end{lemma}

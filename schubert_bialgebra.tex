...
\section{Elementary tensor algebra}

Let $n\geq 3$ be an integer. Let $E^n$ be the vector space with basis 
$$\bigotimes_{k=1}^{n}\bigoplus_{p=1}^k\BRC(e_{p,k}, n)/~$$
where the $\sim$ relation is given by the squash product. For an elemenary RC graph $B$, define a function $\nabla_B:E^n\to E^n$ recursively as follows. If $B$ has max descent $k$, we may define
$$\nabla_B:E^k\to E^k$$ by the right action of the squash product.
% Consider the tensor algebra $T(E^n)$ generated by these elementary symmetric RC graphs and the subspace generated by 
% $$\bigotimes_{q=1}^{n-2}\BRC(e_{*,q},n-1)\otimes \mathcal{P}_{n-1,n-1}$$

%{\big RC coproduct on the central algebra is positive}

\begin{definition}
Given the definition of the squash product, we define the left squash product $A\boxtimes B$ for $A,B\in \BRC(n)$ as follows. Factorize
$$B=B_1\boxtimes B_2\boxtimes \cdots \boxtimes B_k$$
where each $B_i$ is an elementary RC graph. Then define
$$A\boxtimes B = A\boxtimes B_1\boxtimes B_2\boxtimes \cdots \boxtimes B_k$$
\end{definition}

\newcommand{\mind}[1]{\mathrm{m}(#1)}
Define $\BRC_p^n$ to be
$$\bigoplus_{w\in S_n, \maxd(w)=n,\mind{w}=p}$$
\begin{theorem}
We have that $\BRC_p^n$ is a right module over $\mathcal{P}_n$ under the squash product and a left module over $\mathcal{P}_p$ under the left squash product.
\end{theorem}

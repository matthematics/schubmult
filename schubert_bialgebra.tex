...
\begin{definition}[Zeroing out the last row] \label{definition:zero}
  Let $R\in \BRC^0(n)$ be a bounded RC graph with $\hr(R) = n$ such that row $n$ is empty. If $\maxd(R)<n$, set $$\zeromap(R)=R$$ while declaring that $\hr(R)=n-1$. If $\maxd(R)=n$, then we define
  $$\zeromap(R) = \ins{n-1}{\mathtt{strip}(R)} \overline{R}$$
\end{definition}
% \begin{algorithm}
...
See Figure \ref{fig:zeroing} for an example of the zeroing operation (Definition \ref{definition:zero}). 
...
\begin{lemma}
Given a bounded RC graph $R$ with $\hr(R)=n$ such that row $n$ is empty, Definition \ref{definition:zero} produces a valid bounded RC graph $R'$ with $\hr(R')=n-1$ satisfying
$$\wtt(R') = \wtt(R)$$
\end{lemma}
\begin{proof}
...
To prove Theorem \ref{theorem:zero_bijection} for $n=2$, we may characterize the bounded RC graphs $R$ with $|R|=2$ such that row $2$ is empty and $\maxd(\wof{R})=2$ as those for permutations $w=s_k s_{k-1}\cdots s_2$ for some $k>1$. Definition \ref{definition:zero} in this case consists moves the entirety of the crossings in row $1$ to row $2$. The procedure then inserts $k-1$ crossings into row $1$ in order from left to right, which must have roots $t_q - t_s$ such that $q=1$. Thus $R'$ is precisely $\{(1,1), (1,2), (1,3), \ldots, (1, k-1)\}$, which is the unique bounded RC graph for the permutation $w' = s_{k-1} s_{k-2}\cdots s_1$ with one row. This establishes the base case.
...
\end{lemma}
\begin{proof}
Let $w=\wof{R}$ and suppose $\code_n(w) = m$. By construction, the second to last step of Definition \ref{definition:zero} yields a bounded RC graph $\widetilde{R}$ with $|\widetilde{R}|=n$ such that if $\widetilde{w} = \wof{\widetilde{R}}$, then
...
as a bounded RC graph with $\hr(R)-1$ rows. Otherwise, define $R' = R_{\leq p}$, let $d_1 = \maxd(\wof{R'})$ and set $R_1 = R'$. While $d_i > p$, recursively set $A_{i+1} = \mathtt{strip}(R_i)$, set $R_{i+1} = \overline{R_i}$, and set $d_{i+1} = \maxd(R_{i+1})$. 

At termination, say at step $k$, let
...
Then define
$$\lmap(R) = ((\ins{n-1}{i} R')\setminus\{(n, 1)\}, n + 1)$$
...
\begin{proposition} \label{proposition:little_bump}
  Let $R\in \BRC^0(n)$ be a bounded RC graph such that $\maxd(\wof{R}) = n$. Suppose 
...
\begin{theorem} \label{theorem:bpdzero}
We have the formula
...
Maybe easify BPD right zero act
...
\end{lemma}
\begin{proof}
The first equation is trivial for the map $\mathbf{t}$ on BPDs, which transfers by Theorem \ref{theorem:bpdzero} directly to RC graphs. The remaining equations are trivial.
\end{proof}

...
\begin{definition}
  Denote by $\BPD(w,n)$ the set of (reduced) bumpless pipe dreams for a permutation $w$ with an associated integer $n$, where $n\geq \maxd(w)$. There is a canonical bijection $\varphi:\BPD(w,n)\to \mathcal{RC}(w,n)$ defined in \cite{GaoHuang2023} that preserves weight.
\end{definition}

\newcommand{\bzeromap}{\mathbf{t}}

We define a funcion $\bzeromap:\BPD\to \BPD$, found in \cite{WEIGANDT2021105470}, as follows. Given a bumpless pipe dream $(B, n)$, we simply remove row $n$ then rebuild the grid by adding in the necessary tiles to ensure that it is valid.

...
\end{theorem}
\begin{proof}
What is in question is associativity. Suppose we are given three bounded RC graphs $(R_1, p)$, $(R_2, q)$, and $(R_3, r)$, and we want to show that the order of bracketing the product is irrelevant. Consider the set $A$ of pairs of permutations $(u,w)$ such that there exists an RC graph $R_u$ with $\clp^p(R_u) = R_1$, $\trm^q(R_u) = R_2$, as well as an RC graph $R_w$ such that $\clp^{p+q}(R_w)=R_u$ and $\trm^{p+q}(R_w) = R_3$. The bracketing in the first order yields the sum

...
\subsection{The squash product of RC graphs}
\begin{definition}
...
Since $P_n$ is therefore a homomorphism of (potentially nonassociative) algebras as well as bijective, it is an isomorphism, from which it follows that $\boxtimes$ itself is associative.
\end{proof}
...
\begin{definition}
Consider the ring $\mathcal{T}_n$ defined as the tensor algebra on the free abelian groups generated by the set of full $k$-Grassmannian elementary symmetric polynomial RC graphs (i.e. single columns), satisfying the additional condition that if $ A\otimes B$ are such that $\hr(A)\neq \hr(B)$, then

$$A\otimes B = B\otimes A$$
Therefore a tensor
$$A_1\otimes A_2\otimes \cdots \otimes A_k$$
can always be assumed to have $\hr(A_i)\leq \hr(A_j)$ for all $i\leq j$. We then define a linear homomorphism
...
$$\RC(\mathbf{E}_{p_1,k_1}^n\otimes \mathbf{E}_{p_2,k_2}^n\otimes \cdots \otimes \mathbf{E}_{p_m,k_m}^n) = \sum_{w} c_w\mathcal{S}_w(n)$$
...
$$\RC(\mathbf{S}_u(n)\mathbf{S}_v(n)) = \sum_{w} c_{uv}^w \mathcal{S}_w(n)$$
where $c_{uv}^w$ are the Schubert structure constants.
\end{theorem}

It may be unsurprising to the reader at this point that the kernel of $\RC:\mathcal{T}_n\to \BRC_n$ \emph{is not an ideal}. If it were, then $\BRC_n$ would possess a term by term product derived from the squash product such that
$$\mathcal{S}_u(n)\cdot \mathcal{S}_v(n) = \sum_{w} c_{uv}^w \mathcal{S}_w(n)$$
and the problem of finding a positive formula for the Schubert structure constants would be solved. However, let us define another homomorphism
$$\RC_\forest:\mathcal{T}_n\to \BRC_n$$
such that an RC graph $R\in \RC_n$ is sent to the unique RC graph $R_\forest\in \RC_n$ such that $Q(\mathrm{word}(R)) = Q(\mathrm{word}(R_\forest))$ and 
$$\code(\wof{R_\forest}) = \code_\forest(R)$$
where $\code_\forest(R)$ is the forest weight of $R$.

\begin{theorem}
  Suppose $R$ is an RC graph $\hr(R)=n$ such that row $n$ is empty. Then $Q(R) = Q(\zeromap(R))$.
\end{theorem}

...
\begin{algorithm}
\caption{Basic monk insertion $\ins{}{i} R$}
\label{algorithm:monk}
\begin{algorithmic}[1]
		\State \textbf{Input:} An RC graph $R$ and an integer $i\geq 1$.
		\State \textbf{Output:} A modified RC graph $R'$ with $\wtt_j(R')=\wtt_j(R)$  for $j\neq i$ and $\wtt_i(R')=\wtt_i(R)+1$ such that $\wof{R}\tom{} \wof{R'}$.
...
\begin{algorithm}
\caption{Bumping operation $m_{ab}(R)$}
\label{algorithm:monk}
\begin{algorithmic}[1]
		\State \textbf{Input:} An RC graph $R$ and a pair of positive integers $a<b$ such that $\wof{R}t_{ab}\preceq \wof{R}$.
		\State \textbf{Output:} A modified RC graph $R'$ with $\wtt(R')=\wtt(R)$  and a $c>b$ such that $\wof{R}t_{ab}=\wof{R'}t_{ac}\preceq \wof{R'}$.
		\State Delete the element $(i,j)$ such that $\rtt_R(i,j) = (a,b)$ to obtain $R'$. 
    \State Find leftmost position $(i, j') \notin R'$ with $j'>j$
		\State $R \gets R \cup \{(i, j')\}$
    \If{there exists $(i'', j'') \in R$ with $\rtt_R(i'', j'') \in \Phi^-$}
        \State $R \gets R \setminus \{(i'', j'')\}$
        \State Return to step 3 substituting $i = i''$, $j=0$.
    \EndIf
		\State \Return $R$
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
		...
		\State Compute $\ins{}{i} R$. Say that the final reflection was added at some position $(i',j')$.
    \If{One of the following holds:\\
		\quad a) $\rtt_R(i', j') = (s, q)$ where $s \leq k < q$ and $q \notin \{b \mid (a, b) \in L\}$.\\
		\quad b) $\rtt_R(i', j') = (b_r, q)$ where $q > k$, $b_r<q$, and $q \notin \{b \mid (a, b) \in L\}$\\
    }
		\State Update $L$ by adding $(s, q)$ or replacing $(a_r, b_r)$ with $(a_r, q)$ and $(a_r, b_r)$ and resume the loop at step 4.
    \Else
    \State Set $R=m_{ab}(R)$ where $(a,b)=\rtt_R(i',j')$. Replace $(i', j')$ with the newly added position in $R$ and return to step 6.
    \EndIf
    \EndFor
		\State \Return $R$
	\end{algorithmic}
...
\begin{definition}
Let $R$ be an RC graph. We define $\nabla(R)$ to be the RC graph obtained by deleting the element $(i, j)$ in $R$ such that $i$ is minimal, and $j$ is maximal given this. This is simply the leftmost element of the reduced word of $R$.
\end{definition}
...
\end{theorem}
\begin{proof}
  If $\maxd(R_k) < p$, then $\zeromap(R_k) = R_k$.
\end{proof}

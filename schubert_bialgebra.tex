...
\section{The Littlewood-Richardson rule for forest polynomials}
...
\subsection{The lift product}

We now define a product on $\BRC_n$ that is associative, but wrong, in the sense that it does not correctly model Schubert polynomial multiplication. However, we will see that it is correct enough to give a Littlewood-Richardson rule for forest polynomials.
...
is equal to the coefficient of $x^c$ in $\sch_{w\mathrm{dom}_\lambda^{-1}}(x)$.
\end{proposition}
\begin{proof}
...
Replace the factorial elementary symmetric polynomial $E_{p,p}(x;-y_i)$ for each $p$ with $\mathbf{E}_{p,p}(-y_i)\in \mathcal{T}_n[y_1,\ldots,y_N]$ defined by
$$\mathbf{E}_{p,p}(-y_i) = \sum_{j=0}^p y_i^j \mathbf{E}_{p-j,p}^n$$
...
\begin{definition}
  Define a function $\mathrm{lift}_n:\RC_n\to \mathcal{T}_n$ as follows. For an RC graph $R\in \RC_n$, let $\lambda$ be a partition of the form 
...
$$\mathrm{lift}_n(R) = A_1\otimes\cdots\otimes A_n$$
Let $1\leq j\leq n$ and let 
$$R^{\mathrm{left}} = \RC_n(A_1\otimes\cdots \otimes A_j)$$
and
$$R^{\mathrm{right}} = \RC_n(A_{j+1}\otimes\cdots \otimes A_n)$$
Then
$$\mathrm{lift}_n(R^{\mathrm{left}}) = A_1\otimes\cdots \otimes A_j$$
and
$$\mathrm{lift}_n(R^{\mathrm{right}}) = A_{j+1}\otimes\cdots \otimes A_n$$
\end{lemma}

\begin{theorem}
The product $*:\BRC_n\times\BRC_n\to\BRC_n$ is associative.
\end{theorem}
\begin{proof}
...
Since the $-^{\mathrm{left}}$ factors commute with the $-^{\mathrm{right}}$ factors, we have that
\begin{align*}
R_1*(R_2*R_3)
...
An arbitrary RC graph is itself a product of elementary symmetric RC graphs of weakly increasing heights, so an induction argument proves the result for arbitrary $R_3$.
\end{proof}
...
$$\ddot{f}_i(R) = \begin{cases}
  ...
  \perp & \text{ otherwise}
\end{cases}$$
These satisfy the axiom of \emph{quasi-crystal operators} defined in \cite{cain2023quasicrystals}. The same article defines a notion of the quasi-tensor product of quasi-crystals, which is the same as the usual tensor product of crystals except that if there is an $i$ such that $\ddot{\varphi}_i(A)>0$ and $\ddot{\varepsilon}_i(B)>0$, then we require that $\ddot{e}_i(A\ddot{\otimes} B) =\ddot{f}_i(A\ddot{\otimes} B)= \perp$.
\end{definition}
...
\end{theorem}
\begin{proof}
We have that for each such $A',B'$, $A'\ddot{\otimes} B'\mapsto A'*B'$ extends to an isomorphism of quasi-crystals. In particular, the connected components of the terms in the sum partition into quasi-crystals, each with a unique quasi-Yamanouchi element. The sum is then previsely as stated.
\end{proof}
...
% \begin{theorem}
% The map $\pi_\forest:\BRC_n\to \BRC_n^\forest$ is an idempotent homomorphism of right $\mathcal{T}_n$-modules.
% \end{theorem}
...
% where $A\in \mathcal{T}_n$. Then in the Grothendieck group we may write
...
\begin{theorem}[Forest polynomial LR rule]

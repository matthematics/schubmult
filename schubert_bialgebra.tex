...
Let $(R,n)$ be a bounded RC graph and let $p$ be a descent of $\wof{R}$ such that row $p$ of $R$ is empty. Now, $\maxd(R_{\leq p})$ could be greater than $p$. If it is less than or equal to $p$, define 
$$\zeromap^{(p)}(R,n) = (\zeromap(R_{\leq p})\cup {\downarrow R_{>p}},n-1)$$ 
Otherwise, define $R' = R_{\leq p}$ and let $d_0 = \maxd(\wof{R'})$ and set $R_0 = R'$. While $\maxd(R_i) > p$, do:

While $\maxd(R') \geq d_i$, delete $\maxd(R')$ from $R'$ and record the row. Store these rows in $A_i$. When $\maxd(R')<d_i$, set $R_{i+1}= R'$ and set $d_{i+1}= \maxd(R_{i+1})$. 

At termination, set
$$\zeromap^{(p)}(R,n) = (\ins{d_1 - 1}{A_1}\cdots\ins{d_k - 1}{A_k}\zeromap(R_k, p))\cup {\downarrow} R_{>p}, n-1)$$
where $k$ is such that $\maxd(R_k)\leq p$.
\end{definition}
...
\end{lemma}
\begin{proof}
Suppose $(R, n)$ is a bounded RC graph. We prove that $\clp^{n-2}(\clp^{n-1}(R)) = \clp^{n-2}(R)$. For $n=2$, this is trivial. As an additional base case, we also need $n=3$, which is not trivial, but true because a bounded RC graph $(R,1)$ is uniquely determined by its size. We proceed by induction on $n$. Without loss of generality, assume that row $n$ is empty. Then $\clp^{n-1}(R) = \zeromap(R, n)$. We have
$$\clp^{n-2}(R) = \zeromap^2(R_{\leq n-2}, n)=\zeromap(\zeromap(R_{\leq n-2}, n))$$
$$\clp^{n-2}(\clp^{n-1}(R)) = \zeromap(\zeromap(R, n)_{\leq n-2}, n-1)$$
IF $\maxd(\wof{R_{\leq n-2}})\leq n-1$, then both sides are equal to $\zeromap(R_{\leq n-2}, n-1)$. Otherwise, by Lemma \ref{lemma:ztrim}, we have
$$\trm(\clp^{n-2}(R)) = \trm(\zeromap(\zeromap(R_{\leq n-2}, n))) =\zeromap(\zeromap(\trm(R)_{\leq n-3}, n - 1))$$
$$\trm(\clp^{n-2}(\clp^{n-1}(R))) = \zeromap(\zeromap(\trm(R), n - 1)_{\leq n-3}, n-2)$$
...
\end{theorem}
\begin{proof}
What is in question is associativity. Suppose we are given three bounded RC graphs $(R_1, p)$, $(R_2, q)$, and $(R_3, r)$, and we want to show that the order of bracketing the product is irrelevant. This is because if $R_w$ is a term in the expansion of the product, 
...
To prove the map $\alpha:\BRC\to \dcoma$ is a homomorphism of rings, we utilize the formula for the coefficient $d_{u,v}^w(p,q)$ as the coefficient of $\sch_u(x_1,\ldots,x_p)$ when we set $x_{p+1}=\cdots=x_{p+q}=0$ in the polynomial
...
Despite the algebra $\dcoma$ being free on the word generating set, the map $\omega:\BRC\to \dcoma$ is not immediately seen to be a homomorphism of rings, because the single row RC graphs are not a generating set of $\BRC$.
\end{proof}
...
We enlarge the module $\BRC$ to include bounded RC graphs $(R, n)$ that do not necessarily satisfy the condition that $\maxd(\wof{R})\leq n$, only that $R$ has at most $n$ rows. We denote this larger module by $\BRCC$. For $(R, p)\in \BRCC$, define $(R, p)^T\in \BRC$ by $(R^T,\maxd(\wof{R}^{-1}))$. We may define an associative product on $\BRCC$ by

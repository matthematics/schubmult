...
class BoundedRCFactorAlgebra(CrystalGraphRing):
    ...
    def full_schub_elem(self, perm, size):
        #def cem_schub_schur_decomp(perm, n):
            ...
            # if any(len(p1) > size for p1, p2 in result.keys()):
            #     new_result = self.zero
            #     for (p1, p2), coeff in result.items():
            #         if len(p1) > size:
            #             new_result += coeff * self.full_schub_elem(p1, size)* self.schub_elem(p2, size, partition=tuple((~(p2.mul_dominant())).trimcode))
            #         else:
            #             new_result += coeff * self.schub_elem(p1, size, partition=tuple(range(size - 1, 0, -1)))* self.schub_elem(p2, size, partition=tuple((~(p2.mul_dominant())).trimcode))
            # return result
        from schubmult.rings.polynomial_algebra import ElemSymPolyBasis, PolynomialAlgebra, Schub
        #EE = PolynomialAlgebra(ElemSymPolyBasis())
        the_elem_sym = Schub(perm, size).change_basis(ElemSymPolyBasis)
        #decomp = cem_schub_schur_decomp(perm, size)
        schub = 0
        def comp_to_elem(compo, size):
            ret_elem = self(self.make_key((), size))
            grass_elem = self(self.make_key((), size))
            for i in range(len(compo[0])):
                if i + 1 < size:
                    ret_elem *= self.elem_sym(compo[0][i], i + 1, size=size)
                else:
                    grass_elem *= self.elem_sym(compo[0][i], size, size=size)
                # print(ret_elem)
                # print(grass_elem)
            return ret_elem * grass_elem
        for comp, coeff in the_elem_sym.items():
            if schub == 0:
                schub = coeff * comp_to_elem(comp, size)
            else:
                schub += coeff * comp_to_elem(comp, size)
            #self.schub_elem(p1, size, partition=tuple(range(size - 1, 0, -1)))* self.schub_elem(p2, size, partition=tuple((~(p2.mul_dominant())).trimcode))
        return schub
    ...
    def _normalize_key(self, key):
        ...
        while True:
            ...
            factor_key = self.make_key(tuple(factors), size)

            
            hw_key, raise_seq = factor_key.to_highest_weight()
            peeled = False
            hw_factors = list(hw_key)

            if True:
                rc = self.key_to_rc_graph(self.make_key(hw_factors, size)).normalize()
                if len(rc.perm) - 1 <= len(rc):
                    hw_factors = self.make_key([rc], size)
                    done = True
                    
                elif len(rc) == size and _is_full_grassmannian_rc(rc):
                    hw_factors = self.make_key([rc], size)
                    done = True
                else:
                    grass0 = None
                    if len(rc) >= size:
                        try_rc, grass0 = rc.squash_decomp()
                        try_rc = try_rc.normalize()
                        grass0 = grass0.normalize()
                    else:
                        try_rc = rc.normalize()
                    #if not any(try_rc.perm.has_pattern(p) for p in [[3,1,2],[1,4,3,2]]):
                    try_rc = try_rc.normalize()
                    new_key = []
                    while (len(try_rc.perm) - 1 > len(try_rc)) or not _is_full_grassmannian_rc(try_rc):
                        if _is_full_grassmannian_rc(try_rc) and len(try_rc) < size:
                            base, grass = try_rc.resize(len(try_rc) + 1).squash_decomp()
                        else:
                            base, grass = try_rc.squash_decomp()
                        if grass.perm.inv == 0:
                            raise ValueError(f"Peeling produced identity grass: {grass} from {try_rc} in key {key}")
                        grass = grass.normalize()
                        base = base.normalize()
                        new_key.append(grass)
                        try_rc = base
                    if try_rc.perm.inv > 0:
                        new_key.append(try_rc)
                    new_key = list(reversed(new_key))
                    if grass0 is not None and grass0.perm.inv > 0:
                        new_key.append(grass0)
                    hw_factors = self.make_key(new_key, size)
                    done = True
            ...
            if not done:
                ...
                for i in range(len(hw_factors)):
                    if not _is_full_grassmannian_rc(hw_factors[i]):
                        raise ValueError(f"Non full Grassmannian factor in normalized key: {hw_factors[i]} in key {key}")
                    factor = hw_factors[i]
                    if len(factor) < size and len(factor.perm) - 1 > len(factor):
                        # max_rc, elem_rc = factor.resize(len(factor) + 1).squash_decomp()
                        max_rc, elem_rc = factor.squash_decomp()
                        ...
                        #     elem_rc = elem_rc.normalize()
                        #     if max_rc.perm.inv == 0 or elem_rc.perm.inv == 0:
                        #         raise ValueError(f"Peeling produced identity max_rc or elem_rc: {max_rc}, {elem_rc} from {factor} in key {key}")
                        ...
                        while working_rc.perm.inv > 0 and (not _is_full_grassmannian_rc(working_rc)):
                            ...
                            print(f"    peeling step: working_rc {working_rc}, working_elem_rc {working_elem_rc}, addup_list {addup_list}")
                        ...
                        hw_factors = [*hw_factors[:i], *addup_list, *hw_factors[i + 1 :]]
                        #if len(hw_factors) != len(factors):
                        peeled = True
                        break
            ...
            new_factors = self.make_key(self.make_key(tuple(hw_factors), size).reverse_raise_seq(raise_seq), size)
            factors = new_factors
            if not peeled:
                break

        # for fact in factors:
        #     if len(fact) < size and len(fact.perm) - 1 > len(fact):

...
\begin{frame}{Little bumps}
  Suppose $(w_1, w_2,\ldots, w_n)$ is a reduced word for a permutation $w$. A \emph{Little bump} is an operation that takes a reduced word for $w$ together with an index $1\leq i\leq n$ and produces a reduced word for a different permutation $w'$ such that $\ell(w) = \ell(w')$. The original purpose of this definition by Little was to generalize the RSK correspondence to reduced words (in a manner distinct from Edelman-Greene insertion). However, the map has interesting combinatorial properties in and of iteslf.

  \begin{theorem}[Hamaker, Young 2014]
  Little bumps preserve the Coxeter-Knuth equivalence class of a reduced word.
  \end{theorem}
\end{frame}

\begin{frame}{Little bumps and the transition algorithm}
  The transition algorithm on pipe dreams is equivalent to a specific sequence of Little bumps on the corresponding reduced word that preserves compatibility with the weight. In particular, we have

  \begin{theorem}[S, 2026+]
  If $R$ is a bounded pipe dream with $\hr(R)=n$ and row $n$ empty, then the transition algorithm is equivalent to applying Little bumps to the last descent of the permutation until the result has its last descent $\leq n-1$.
  \end{theorem}
\end{frame}
...
\begin{itemize}
\item $\uparrow W\equiv \uparrow W'$, where $\uparrow$ is the operation of adding $1$ to each letter.
\item $\mathrm{perm}(W) = \mathrm{perm}(W')$.
\item If $i$ is the last descent of $\mathrm{perm}(W)$, then $L_{i,i+1}(W)\equiv L_{i,i+1}(W')$.
\end{itemize}
Then there is a unique family of polynomials with nonnegative integer coefficients that form a basis for $\coma$ that is the sum of monomials corresponding to pipe dreams with reduced words in the equivalence class. The dual basis  is then positive in the dual Schubert basis and has nonnegative structure constants.
\end{theorem}
\end{frame}

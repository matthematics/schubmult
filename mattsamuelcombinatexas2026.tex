...
\begin{frame}{Ghost pipe dreams}
  \begin{itemize}
  \item There is a larger algebra which we may define as a quotient of the tensor algebra of the direct sum of plactic algebras with different numbers of rows
  \item This algebra multiplies correctly with a certain rule, and projects surjectively onto $\RC$
  
  \item However, the kernel is not an ideal. We can fix this by defining $\mathrm{Ghost}_n$ to be the set of $n$-row pipe dreams that occur in a specific way in the tensor algebra
  \item They are linear combinations of tensors that project to zero in $\RC$ but are not themselves zero in the tensor algebra
  \end{itemize}
\end{frame}
...
\begin{frame}{The representation theorem}
  \begin{theorem}
Let $P$ be an $n$-row pipe dream. Then there is a unique way to write $P$ as a linear combination of tensor products of ghost pipe dreams such that the following hold:
  \end{theorem}
\end{frame}
...
\begin{frame}{Positive formula}
  ...
  \begin{proof}
  If we are allowed to precompute the products of all pairs of pipe dreams with $\mathrm{perm}(P)\in S_n$, then computing the product of two elements of $\mathrm{Pipe}_n$ yields a positive result without cancellation by applying the classical LR rule.
  \end{proof}
\end{frame}
